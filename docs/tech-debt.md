...
### 105. 브랜치별 DB 설정 자동 전환이 **`main`·`develop`에서는 아직 안 돈다** — 훅 파일이 그 브랜치에 없기 때문 (2026-07-21 등재, 🟡 조건부)
...
- **⚠️ 왜 지금 당장 위험하지는 않은가:** 어긋나는 방향이 **안전한 쪽**이다. `main`에서 로컬을 보는 것은 데이터가 안 보이거나 접속 오류가 날 뿐이고, **운영 DB를 실수로 건드리는 반대 방향은 일어나지 않는다.** `use-env.sh`의 브랜치 규칙 자체도 "main만 운영, 나머지 전부 로컬"이라 같은 방향으로 설계했다.
- **해소 조건:** 이 두 파일이 `develop`·`main`에 도달하면 자동으로 해소된다. 즉 **별도 작업이 아니라 병합 시점의 문제**다.
- **📅 `test/bmad-loop`를 `develop`에 병합할 때 확인** — 병합 후 `main`↔`develop` 체크아웃을 실제로 해보고 위 표의 `main` 행이 운영 주소로 바뀌는지 재측정한다. 안 바뀌면 이 항목을 다시 연다.
