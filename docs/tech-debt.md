...
| 🟢 품질/테스트 | **17** | AI 정규화 회귀테스트(#13) · FR17 경로(#14) · Riverpod 컨트롤러(#17) · 누수테스트 의존(#32) · DSN 포트(#33) · anon 테스트 중복(#34) · 시드 self-heal(#35) · FocusTrap 2종(#36·#37) · ErrorState tone(#38) · error useState(#39) · 런북 §8 누락(#41) · **§10.1 정렬 서술 미래형(#59 — 9.5에도 남음)** · **randomUUID secure context(#64)** · *(📅 예약: #15 LIMIT→13-1 · #16 거리컷오프→13-6 · #40 Pretendard→11-0 · **#59→9-5**)* *(✎ 2026-07-19 Story 9.4 Task 0: **#57·#58·#63 해소** — 행에서 제거. #59는 9.4 몫만 이행, 9.5 갤러리가 남아 유지)* |
...
- **🔸 2026-07-19 Story 9.4 — 절반 이행(목록 카드).** 약속을 **실행되는 검사로 옮겼다**: `conventions.md §10.2`(읽는 쪽 규칙 신설) + `coverImages.test.ts`가 정렬·tie-break를 고정한다. 정렬은 **두 층에 다 박았다** — DB 쿼리(`attachCoverImages`)와 순수 함수(`coverImages`) 양쪽. 한쪽이 빠져도 화면이 흔들리지 않는다.
