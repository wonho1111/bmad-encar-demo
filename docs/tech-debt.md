...
| 우선순위 | 건수 | 한 줄 |
|---|---|---|
| 🔴 필수 | 2 | 시드 평문 비번 · 안드로이드 서명 (※ 앱 픽셀 E2E·AI 라이브 호출은 2026-07-11 실폰 검증으로 해소 → 부록) |
...
### 1. 시드 계정 평문 비밀번호 커밋
- **위치:** `supabase/seed.sql` (admin 시드 1.5, seller 시드 2-5)
- **내용:** 데모 계정(`seller-seed@test.com / seller123` 등)이 평문 비밀번호로 저장소에 커밋됨. ⚠️ 주석에 "운영 전 교체" 명시돼 있음.
- **왜 위험:** 저장소 접근자가 곧바로 로그인 가능. 운영 DB에 그대로 적용되면 계정 탈취 경로.
- **해소:** 운영 배포 전 비밀번호 교체 **또는** 시드 환경 분리(운영 DB에는 시드 미적용). 환경별 게이팅 도입.
...
1. **운영 전 보안 마감** — 🔴 #1 시드 비번, #2 안드로이드 서명. (남은 🔴 2건)
