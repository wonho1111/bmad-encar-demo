...
### 77. 버킷에 **이전부터 있던 고아 오브젝트 3건** (코드리뷰 2026-07-19 발견, 🟢 품질 — 이번 변경이 만든 것 아님)
...
- **왜 지금 안 지웠나:** 이번 변경이 만든 고아가 아니다(A3 — 내가 만든 것만 치운다). 지우려면 사용자 판단이 필요하고, 특히 `.emptyFolderPlaceholder`는 지우면 대시보드 폴더 표시가 사라질 수 있다.
- **해소 방향:** 사용자 승인 후 관리자 토큰으로 webp 2건 삭제. 표식 파일은 그대로 두는 편이 낫다.
