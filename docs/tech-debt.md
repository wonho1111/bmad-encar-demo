...
### 183. 화면 1개를 여는 데 인증 서버(`/auth/v1/user`)를 17~27번 부른다 — 링크 프리페치가 `getUser()`를 증폭시킨다 (2026-07-28 실측, 🟡 조건부)
...
### 193. `#185`(마이그레이션 게이트 프로브가 chat/realtime을 안 봄)의 트리거가 도래했다 — 프로브 확장 대신 전용 실DB 테스트로 대체하기로 판단 (2026-07-28 Story 12-2, 🟢 품질, 기존 항목 무수정)
...
### 194. `#188`(conftest.py 부재)이 이번 스토리로 "해소"된 것으로 오독되면 안 된다 — 신규 `conftest.py`는 새 파일 1개만 쓰고, 기존 5개는 여전히 복제 상태다 (2026-07-28 Story 12-2, 🟢 품질, 기존 항목 무수정)
...
### 195. Realtime Broadcast 전달 실패가 조용히 삼켜지거나(플랫폼 `realtime.send()`) 트랜잭션 전체를 되돌린다(`realtime.broadcast_changes()` 자체가 못 불릴 때) — 어느 쪽도 관측되지 않는다 (2026-07-28 Story 12-2 코드리뷰 adversarial+edge-case 교차 지적, 🟡 기능, defer)
...
### 196. 0023이 서는 전제 3가지가 **원격에서만 답할 수 있는데 아직 안 물었다** — 스텁은 로컬 기준 실측이고, 실제 `realtime.messages`는 파티션 테이블이며, Realtime이 불가용하면 채팅 저장 자체가 막힌다 (2026-07-28 Story 12-2 후속 코드리뷰 실측, 🟡 기능, 기존 항목 무수정)
...
### 199. `#195` 재판단(트리거 도래) — Story 12.3(폴링 제거) 착수 시점에도 브로드캐스트 실패율 관측 인프라를 만들지 않기로 결정 (2026-07-29 Story 12-3, 🟡 기능, defer, 기존 항목 무수정)
...
### 200. 관리자 대화 열람 화면이 채팅 본문을 `break-words` 없이 렌더한다 — 사용자 화면에서 실측으로 닫은 `#169`(390px 가로 오버플로)가 같은 데이터로 관리자 쪽에만 남았다 (2026-07-29 Story 12-3 후속 코드리뷰 verification-gap 지적, 🟢 품질, defer)
...
### 201. Broadcast Replay의 ≤25건/72시간 한도는 이 데모 규모에서 사실상 도달하지 않는다 — 그래도 커서 재조회를 항상 병행하기로 판단 (2026-07-29 Story 12.4, 🟢 품질, 기존 항목 무수정)
...
### 202. Broadcast Replay가 이미 삭제된 `chat_messages` 행의 방송을 되살릴 수 있다 — 이번 스토리의 verification 중 실제로 걸림(E2E red), 로컬 DB 정리로 해소 (2026-07-29 Story 12.4 verification 중 실측, 🟡 기능, defer)
...
### 203. `deferred-work.md`의 `DW-6`(12-3 review-budget-followup)이 대장에 등재돼 있지 않다 (2026-07-29 Story 12-4 3차 후속 리뷰 착수 시 대장 점검, 🟢 품질, bmad-loop DW-6 이관)
- **위치:** `_bmad-output/implementation-artifacts/deferred-work.md`의 `DW-6`(status: open) — *"Review budget (2 cycles) was exhausted with the story finalized (status: done, verify green) while the review pass kept recommending an independent follow-up"*(origin: `review-budget-followup`, source_spec: `spec-12-3-실시간-송수신-전환-폴링-제거.md`, 작성 주체 = bmad-loop 엔진 run `20260729-003659-be13`).
- **내용:** `#148`·`#149`·`#164`·`#184`·`#192`와 **완전히 같은 종류(짝)** 다. `deferred-work.md`는 2026-07-15에 동결된 경위 보관용 파일이라 "지금 뭐가 열려 있나"의 정본이 아닌데(B8 — 정본은 이 파일 하나), bmad-loop 엔진은 훅·주입의 사정거리 밖에서 거기에 `DW-N`을 계속 직접 쓴다(구조 자체는 `#129`). 이번엔 12-4의 3차 후속 리뷰 세션이 착수 점검에서 `DW-6` 미이관을 발견했다.
- **12-3의 후속 리뷰 권고가 실체다:** 12.3(실시간 송수신 전환·폴링 제거)은 리뷰 2사이클이 **둘 다 `status: done`** 이면서 **둘 다 "독립 후속 패스가 더 필요하다"** 고 했고, `max_review_cycles = 2` 상한에 걸려 *"예산 소진 → 커밋하고 권고는 DW로 이관"* 경로로 마감됐다. 코드가 나쁘다는 뜻이 아니라, **리뷰가 아직 볼 게 남았다고 말한 채로 닫혔다**는 뜻이다.
- **왜 지금 안 고치나:** 이관(등재)이 이 항목의 실체다 — 실제 "12-3에 대한 독립 후속 리뷰"를 돌리는 것은 완전히 별개의 작업이라 여기서 하지 않는다(`#184`·`#192`와 동일한 처리). 동결 파일 자체는 무수정이다.
- **트리거:** 사용자가 12-3(`spec-12-3-실시간-송수신-전환-폴링-제거.md`)의 독립 후속 리뷰를 실행할 의사결정을 내릴 때 — 그 diff를 다시 본다. 참고로 이번 12-4 3차 리뷰가 `ChatRoomMessages.tsx`의 인접 코드를 넓게 훑었으므로 겹치는 축(구독 콜백·pending·에러 칸 분할)은 이미 상당 부분 재검토됐다.

### 204. 오프라인 큐 flush가 **영구히 멈출 수 있다** — supabase-js 호출에 타임아웃이 없어 `isFlushing` 가드가 풀리지 않는 경로 (2026-07-29 Story 12.4 3차 후속 리뷰 adversarial 지적, 🟡 기능, defer)
...
### 205. 오프라인 큐잉의 2000자 길이 가드가 **실제로 도달 가능한 경로인지 확인되지 않았다** — `maxLength`가 이미 막고 있을 수 있다 (2026-07-29 Story 12.4 3차 후속 리뷰 verification-gap 지적, 🟢 품질, defer)
...
### 207. 오프라인 큐잉 기능 전체가 **라이브러리 한 버전의 소스 독해** 위에 서 있는데 의존성은 캐럿 범위다 (2026-07-29 Story 12.4 3차 후속 리뷰 verification-gap 지적, 🟡 조건부, defer)
...
### 208. `deferred-work.md`의 `DW-7`(12-4 review-budget-followup)이 대장에 등재돼 있지 않다 (2026-07-29 Story 12-5 착수 시 대장 점검, 🟢 품질, bmad-loop DW-7 이관)
- **위치:** `_bmad-output/implementation-artifacts/deferred-work.md`의 `DW-7`(status: open) — *"Review budget (2 cycles) was exhausted with the story finalized (status: done, verify green) while the review pass kept recommending an independent follow-up"*(origin: `review-budget-followup`, source_spec: `spec-12-4-재연결-배너-갭-보정.md`, 작성 주체 = bmad-loop 엔진 run `20260729-003659-be13`).
- **내용:** `#148`·`#149`·`#164`·`#184`·`#192`·`#203`과 **완전히 같은 종류(짝)** 다. `deferred-work.md`는 2026-07-15에 동결된 경위 보관용 파일이라 "지금 뭐가 열려 있나"의 정본이 아닌데(B8 — 정본은 이 파일 하나), bmad-loop 엔진은 훅·주입의 사정거리 밖에서 거기에 `DW-N`을 계속 직접 쓴다(구조 자체는 `#129`). 이번엔 12-5 착수 점검에서 `DW-7` 미이관을 발견했다.
- **12-4의 후속 리뷰 권고가 실체다:** 12.4(재연결 배너 + 갭보정)는 리뷰 사이클이 **매번 `status: done`** 이면서 **매번 "독립 후속 패스가 더 필요하다"** 고 했고(1·2·3차 세 패스 모두 그랬다 — 3차 패스 본문의 `followup_review_recommended: true`), `max_review_cycles` 상한에 걸려 *"예산 소진 → 커밋하고 권고는 DW로 이관"* 경로로 마감됐다. 코드가 나쁘다는 뜻이 아니라, **리뷰가 아직 볼 게 남았다고 말한 채로 닫혔다**는 뜻이다.
- **왜 지금 안 고치나:** 이관(등재)이 이 항목의 실체다 — 실제 "12-4에 대한 독립 후속 리뷰"를 돌리는 것은 완전히 별개의 작업이라 여기서 하지 않는다(`#184`·`#192`·`#203`과 동일한 처리). 동결 파일 자체는 무수정이다.
- **트리거:** 사용자가 12-4(`spec-12-4-재연결-배너-갭-보정.md`)의 독립 후속 리뷰를 실행할 의사결정을 내릴 때 — 그 diff를 다시 본다. 이 스토리(12-5)가 `chat/[roomId]/page.tsx`·`chat/page.tsx`·내비 쪽만 건드리고 `ChatRoomMessages.tsx`의 재연결·큐잉·갭보정 로직 자체는 손대지 않으므로(스펙 Never), 겹치는 축은 없다.

### 209. `AppHeader`의 안읽음 RPC 호출이 `#183`(getUser 증폭) 층에 하나 더 얹힌다 (2026-07-29 Story 12.5, 🟢 품질, defer)
...
### 210. 방이 열린 채 실시간으로 본 메시지는 재진입 전까지 안읽음 배지에 남는다 (2026-07-29 Story 12.5, 🟢 품질, 의도적 범위 제외)
...
### 211. 로컬 Supabase 스택에서 `authenticated`/`anon`의 테이블 기본 권한이 통째로 빠져 있었다 — 전 기능이 로컬에서 42501로 죽는 상태였다 (2026-07-29 Story 12.5 구현 검증 중 실측, 🔴 인프라, 세션 내 임시 조치)
...
### 212. `chat_unread_count()`가 시간 창 없이 사용자의 전체 대화 이력을 매번 스캔한다 (2026-07-29 Story 12.5 코드리뷰 adversarial 지적, 🟡 조건부, defer)
...
### 213. `last_read_at`을 DB(`now()`)가 아니라 web 서버 시계(`new Date()`)가 정한다 (2026-07-29 Story 12.5 후속 코드리뷰 adversarial+edge-case 교차 지적, 🟢 품질, defer)
...
### 214. `last_message_at` 트리거가 INSERT만 본다 — 관리자가 마지막 메시지를 지우면 방이 목록 상단에 고정된다 (2026-07-29 Story 12.5 후속 코드리뷰 edge-case-hunter 지적, 🟢 품질, defer)
...
### 216. `markChatRoomRead`의 upsert가 **단조증가가 아니다** — 같은 컬럼 축에 넣은 `last_message_at` 가드와 비대칭이다 (2026-07-29 Story 12.5 3차 후속 리뷰 edge-case-hunter 지적, 🟢 품질, defer)
...
### 217. `chat_rooms` 시드 삽입문도 명시 컬럼 목록으로 바뀌어 `#146`이 예고한 "조용한 누락" 쪽에 합류했다 — `#146` 본문의 현황 서술이 낡았다 (2026-07-29 Story 12.5 3차 후속 리뷰 adversarial+edge-case 교차 지적, 🟢 품질, 기존 항목 무수정 정정)
...
### 218. 관리자가 **소비자** 채팅방 라우트를 열면 Realtime 구독이 계속 CHANNEL_ERROR로 끊긴다 (2026-07-29 Story 12.5 3차 후속 리뷰 검증 중 실측, 🟢 품질, defer)
...
### 219. `deferred-work.md`의 `DW-8`(12-5 review-budget-followup)이 대장에 등재돼 있지 않다 (2026-07-29 Story 12-6 착수 시 대장 점검, 🟢 품질, bmad-loop DW-8 이관)
- **위치:** `_bmad-output/implementation-artifacts/deferred-work.md`의 `DW-8`(status: open) — *"Review budget (2 cycles) was exhausted with the story finalized (status: done, verify green) while the review pass kept recommending an independent follow-up"*(origin: `review-budget-followup`, source_spec: `spec-12-5-안읽음-배지-방-목록-정렬.md`, severity: low, 작성 주체 = bmad-loop 엔진 run `20260729-003659-be13`).
- **내용:** `#148`·`#149`·`#164`·`#184`·`#192`·`#203`·`#208`과 **완전히 같은 종류(짝)** 다. `deferred-work.md`는 2026-07-15에 동결된 경위 보관용 파일이라 "지금 뭐가 열려 있나"의 정본이 아닌데(B8 — 정본은 이 파일 하나), bmad-loop 엔진은 훅·주입의 사정거리 밖에서 거기에 `DW-N`을 계속 직접 쓴다(구조 자체는 `#129`). 이번엔 12-6 착수 점검에서 `DW-8` 미이관을 발견했다.
- **왜 지금 안 고치나:** 이관(등재)이 이 항목의 실체다 — 실제 "12-5에 대한 독립 후속 리뷰"를 돌리는 것은 완전히 별개의 작업이라 여기서 하지 않는다(`#184`·`#192`·`#203`·`#208`과 동일한 처리). 동결 파일 자체는 무수정이다.
- **트리거:** 사용자가 12-5(`spec-12-5-안읽음-배지-방-목록-정렬.md`)의 독립 후속 리뷰를 실행할 의사결정을 내릴 때 — 그 diff를 다시 본다.

### 220. "다시 보내기" 재시도 버튼에 진행 중 표시(로딩 상태)가 없다 (2026-07-29 Story 12.6 코드리뷰 adversarial+edge-case 교차 지적, 🟢 품질, defer)
...
### 221. "다시 보내기" 재시도 버튼에 연타 방지(쿨다운)가 없다 (2026-07-29 Story 12.6 코드리뷰 adversarial 지적, 🟢 품질, defer)
...
### 222. `ChatListBfcacheRefresh`가 Next.js의 문서화되지 않은 popstate 처리 순서에 의존하는데 이를 지켜주는 자동 테스트가 없다 (2026-07-29 Story 12.6 코드리뷰 adversarial+verification-gap 교차 지적, 🟡 조건부, defer)
...
### 223. `#215` 수정(popstate 가드)이 루트 레이아웃에 있어 채팅과 무관한 모든 페이지의 뒤로가기에서도 실행된다 (2026-07-29 Story 12.6 코드리뷰 adversarial 지적, 🟢 품질, defer)
...
### 224. 큐가 다른 경로로 비워지면 "N건 미전송" 안내가 화면에 박제되고 재시도 버튼으로도 못 지운다 (2026-07-29 Story 12.6 후속 코드리뷰 adversarial+edge-case 교차 지적, 🟡 조건부, defer)
...
### 225. `#215` 수정이 `/chat`에서만 돌아 다른 페이지로 뒤로가기하면 안읽음 배지가 여전히 옛값이다 (2026-07-29 Story 12.6 후속 코드리뷰 verification-gap+edge-case 교차 지적, 🟡 조건부, defer)
...
### 226. 재시도 버튼의 끊김 가드가 실시간 웹소켓 상태를 보는데 정작 전송은 HTTP다 (2026-07-29 Story 12.6 후속 코드리뷰 adversarial 지적, 🟢 품질, defer)
...
### 227. 방을 옮기면 아직 못 보낸 오프라인 큐가 **경고 없이 통째로 버려진다** (2026-07-29 Story 12.6 2차 후속 리뷰 edge-case-hunter 지적, 🟡 기능, defer)
...
### 228. `#215` 수정이 기대는 **문서화되지 않은 Next 내부 동작이 `#222`가 적은 것 말고도 2건 더** 있다 (2026-07-29 Story 12.6 2차 후속 리뷰 adversarial 지적, 🟡 조건부, defer)
...
### 229. web CI에 `next build` 단계가 없어 **RSC 경계 위반이 CI 초록으로 통과**한다 (2026-07-29 Story 12.6 2차 후속 리뷰 verification-gap 지적, 🟡 조건부, defer)
...
### 230. 재시도 버튼 배선의 **실행 순서 불변식**이 검사로 고정돼 있지 않다 (2026-07-29 Story 12.6 2차 후속 리뷰 verification-gap 뮤테이션 실증, 🟢 품질, defer)
