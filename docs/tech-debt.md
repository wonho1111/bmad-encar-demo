...
- **✅ 해소 (Story 11-0, 2026-07-27):** `next/font/local`로 self-host 전환 완료. `web/src/app/fonts/PretendardVariable.woff2`(≈1.96MiB) + `LICENSE.txt`(OFL 1.1) 추가, `layout.tsx`의 CDN `<link>`+`preconnect` 제거, `globals.css` `--font-sans`가 로컬 폰트 변수를 우선 참조. 실측: `document.fonts.check()` 400/800 weight true, 네트워크 탭에 `jsdelivr` 요청 0건, `<Logo>` 라이트/다크 회귀 없음, `next build`/`lint` 통과. 상세: `_bmad-output/implementation-artifacts/spec-11-0-pretendard-self-host-전환.md`.
...
### 128. #40 본문이 실측과 어긋난 채 닫혔다 — 가변축 값 오기 + 근거가 실제보다 강하게 읽힘 (2026-07-27 Story 11-0 3차 리뷰 defer, 🟢 품질)
