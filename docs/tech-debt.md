...
- **왜 없나(추측 아님, 실측):** `playwright.config.*` **0건**, `web/`에 Playwright 의존성 **없음**, `app/integration_test/` **없음**. `.github/workflows/tests.yml` 주석이 이미 인정하고 있다 — *"E2E — Playwright 스펙이 레포에 0개다. docs/e2e-*.md는 사람이 읽는 수동 대본이지 코드가 아니다."* (그 `docs/e2e-*.md`조차 지금은 존재하지 않는다.)
...
### 127. 폰트 CDN 재유입·자산 비대를 잡는 실행 검사가 없다 — #40이 조용히 되돌아갈 수 있다 (2026-07-27 Story 11-0 후속리뷰 defer, 🟢 품질)
...
### 160. 내비의 상호작용(760px 접힘·바깥클릭·Esc·포커스복귀·상호배타·리사이즈 강제닫기)에 자동 회귀검사가 하나도 없다 (2026-07-28 Story 11-2 3차 코드리뷰 defer, 🟡 조건부)
...
### 172. Story 11.5 후속 리뷰가 대장에서 **고치지 못하고 남긴 것 2건** — 닫힌 항목의 제목 표기, `#168` 트리거 형태 (2026-07-28 Story 11.5 후속 코드리뷰, 🟢 품질)
...
### 174. 대장 기존 항목 3곳에 "Playwright 스펙이 레포에 0개다"라는 **이미 거짓이 된 주장**이 남아 있다 (2026-07-28 Story 11.5 3차 코드리뷰 defer, 🟢 품질)
