...
| 🟡 조건부 | **19** | 재오픈 차단(#9) · 타입(#12) · **GRANT(#18 — Epic 9)** · 관리자 운영 실사용 미확인(#19) · 거래일 `sold_at`(#20) · **게이트 구조적 대가 3종(#22~24 — Epic 13)** · psycopg 버전핀(#25) · 폐기토큰 401(#26) · **시드 멱등 delete(#27 — Epic 9·10)** · **앱 사진 업로더 계획 공백(#52 — Epic 16)** · **수정 중 sold 전환 관통(#54)** · **서명 URL 만료 미복구(#55)** · **동시 편집 10장 초과 고아(#56)** · **서명 URL 위치매칭(#62 — 9.5)** · *(📅 예약: #6 멱등키→12-1 · #7 폴링배너→12-4 · #11 옵션쉼표→10-3 · **#54→9-7 · #55→9-5 · #56→Epic 9 회고**)* *(✎ 2026-07-19: `#51`은 아래 ✅ 행에서 닫힌 항목인데 여기 "원인 미상"으로도 남아 있었다 — 모순이라 제거)* |
| 🟢 품질/테스트 | **20** | AI 정규화 회귀테스트(#13) · FR17 경로(#14) · Riverpod 컨트롤러(#17) · 누수테스트 의존(#32) · DSN 포트(#33) · anon 테스트 중복(#34) · 시드 self-heal(#35) · FocusTrap 2종(#36·#37) · ErrorState tone(#38) · error useState(#39) · 런북 §8 누락(#41) · **WebP 폴백 미검증(#57)** · **애니메이션/알파 손실 무고지(#58)** · **§10.1 정렬 서술 미래형(#59)** · **제출 중 drop 미차단(#63 — 9.4)** · **randomUUID secure context(#64)** · *(📅 예약: #15 LIMIT→13-1 · #16 거리컷오프→13-6 · #40 Pretendard→11-0 · **#57→9-4 착수 전 · #58→9-4 · #59→9-4·9-5**)* |
...
| — | — | ⚠️ **이 표에 기존 누락이 있다(9.3 리뷰에서 발견, 미수정):** 9.1 리뷰 이월 **#43~#50**과 **#53**이 위 행들의 나열·건수에 반영돼 있지 않다. 본문 섹션에는 전부 존재한다 — **표만 늙었다.** 9.3 리뷰가 만든 드리프트가 아니라서 고치지 않고 표시만 남긴다(A3). 대장을 세는 쪽은 표가 아니라 본문 `### N.` 항목을 세라. |
...
**합계 47건** = 🔴1 + 🟡19 + 🟢20 + 🔒1 + ✅6. *(📅 3(+6) · ⚪ 3은 이 합계에 넣지 않는다 — 위 행들과 겹친다.)*
...
- **📅 예약: Epic 9 회고에서 재평가.** #60 수정으로 생긴 사진 정리 함수(`deleteListingPhotoObjects`)를 재활용할 수 있는지 그때 판단한다. 지금 손대면 근거 없이 설계를 늘리게 된다.
### 57. WebP→JPEG 폴백 경로가 테스트·실측 0건 (9.3 리뷰 이월, 🟢 품질)
...
### 58. 애니메이션 WebP 첫 프레임만 저장 · 투명 PNG 알파 손실이 무고지 (9.3 리뷰 이월, 🟢 품질)
...
### 59. `conventions.md` §10.1의 "읽는 쪽은 `order by sort_order, id`" 서술이 현재형 약속 (9.3 리뷰 이월, 🟢 문서 부채)
...
### 63. 제출 중에 시작된 드래그의 drop이 차단되지 않는다 (9.3 코드리뷰 2차 이월, 🟢 품질)
