...
> **건수가 21 → 39로 늘어난 건 부채가 늘어서가 아니다.** `deferred-work.md`에 흩어져 있던 열린 항목을 여기로 흡수했고(대장 일원화), 이번 검토에서 문서부채 5건(#28~31·#41~42)을 새로 등록했다. **미룬 것도 적는 게 대장의 일이다** — 안 적으면 "안 한 것"과 "했는지 모르는 것"이 구별되지 않고, 후자가 더 비싸다.
...
- ✅ **해소 완료 (2026-07-16)** — 두 대본 모두 §8 계약(열람/행동)에 맞춰 재작성. **한쪽만 고치면 다시 어긋나므로 둘 다 고쳤다**: `e2e-checklist.md` §1.1(열람 3항목 + 행동 3항목 + 어포던스) · `e2e-test-cases.md` `W-AUTH-01`(→ "열람 허용"으로 의미 반전) + `01b~01e` 신설 + `W-AUTH-07` 정정. 각 대본 머리에 계약 정본(`conventions.md` §8)과 코드(`proxy.ts`)를 명시해 다음 변경 때 따라오게 했다.
- 📌 **교훈**: 계약이 바뀐 건 8.5(2026-07-14)인데 대본은 안 따라왔다. **문서는 조용히 늙는다** — 코드였다면 그 자리에서 빨간불이 났다. 근본 해소는 P0 케이스를 Playwright 스펙으로 옮기는 것(별도 스토리 규모).
