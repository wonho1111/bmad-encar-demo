...
- **✅ 해소 (Story 9.0, 2026-07-19 — `0014_listing_images_public_bucket.sql`):** `insert ... do nothing`이 아니라 **`update storage.buckets set public, file_size_limit, allowed_mime_types`** 로 값을 덮어쓴다. 버킷이 어떤 상태로 먼저 존재했든 이 문장에서 확정되므로 "조용히 무효" 경로가 사라졌다. *(단 위 ⚠️는 유효하다 — 5MB·MIME를 **집행**하는 주체는 여전히 Storage API 서버이고 게이트는 그걸 증명하지 못한다. 이 마이그가 고친 것은 "값이 안 들어가는 것"이지 "값이 집행되는지"가 아니다.)*
...
- **✅ 해소:** 관리자 경로가 판매자 경로와 **같은 함수**(`deleteListingPhotoObjects`)를 호출하고, 정리 실패 시 매물 삭제를 중단한다. DB 쪽도 `0014`가 관리자 DELETE 정책(`listing_images_objects_admin_delete`)을 추가해 권한을 맞췄다 — 관리자는 남의 경로라 `owner_delete` 정책에 걸리지 않았다.
