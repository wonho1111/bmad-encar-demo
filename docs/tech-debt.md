...
**합계 53건** = 🔴1 + 🟡21 + 🟢24 + 🔒1 + ✅6. *(✎ 2026-07-21 코드리뷰: 이 줄이 또 옛 숫자(47 = 🟡19+🟢20)로 남아 있었다 — 아래 §가 이미 두 번 기록한 바로 그 사고다. 표를 고칠 때 이 줄을 같이 고치지 않으면 세 번째가 된다.)* *(📅 3(+6) · ⚪ 3은 이 합계에 넣지 않는다 — 위 행들과 겹친다.)*
...
- **📅 예약: Epic 10** — 카드 값(신뢰속성·옵션 칩)을 채우는 에픽이 어차피 §4.1 락스텝을 한 번 돈다. **그 스토리의 인수조건 체크박스로 심을 것**: "`fuel`을 `ListingCard` 계약에 추가하고 meta를 `주행·연료·지역`으로 되돌린다"(B5·B8).
...
- **트리거:** Epic 10.2/10.6이 빈 슬롯을 채울 때 — 그때 상세 접근성을 한 번에 본다. (관리자 화면은 Epic 15)
...
- **트리거:** 문의 CTA를 다시 손대는 스토리(Epic 10 판매자 정보 슬롯이 같은 카드에 들어온다).
