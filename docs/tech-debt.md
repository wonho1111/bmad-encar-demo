...
| 🟢 품질/테스트 | **20** | AI 정규화 회귀테스트(#13) · FR17 경로(#14) · Riverpod 컨트롤러(#17) · 누수테스트 의존(#32) · DSN 포트(#33) · anon 테스트 중복(#34) · 시드 self-heal(#35) · FocusTrap 2종(#36·#37) · ErrorState tone(#38) · error useState(#39) · 런북 §8 누락(#41) · **randomUUID secure context(#64)** · *(📅 예약: #15 LIMIT→13-1 · #16 거리컷오프→13-6 · #40 Pretendard→11-0)* *(✎ 2026-07-19 Story 9.4 Task 0: **#57·#58·#63 해소** — 행에서 제거)* *(✎ 2026-07-20 Story 9.5: **#59 해소** — 마지막 소비처(상세 갤러리)가 정렬을 걸고 검사로 고정됨. 행에서 제거)* *(✎ 2026-07-20 Story 9.5 코드리뷰 이월 **4건 추가**: 갤러리 접근성 3종(#79 — Epic 10 트리거) · 사진 용량 근거 오기재(#80) · 정렬 로직 2벌(#81) · `InquiryCta` 이중 마운트(#82))* *(✎ 2026-07-20 Story 9.6: **#48·#71 해소** — 행에서 제거. #48은 FR11 강제가 실행되는 검사 2벌로 바뀌었고(api sold 차단 + sql_guard 4건), #71은 AI 카드에 사진이 실제로 뜨면서 소멸. **신규 1건 추가**: `/ai` 390px 가로 넘침(#84 — 9.6 검증 중 발견, **9.6이 만든 것이 아님**을 대조군으로 확인. 트리거 = Epic 11.3/13.4/11.5))* |
...
- **닫는 근거 — 약속 2개가 각각 실행되는 검사가 됐다:**
  - 방어 ①"api가 on_sale로 좁힌다" → `api/tests/test_listing_cards.py::test_sold_listing_photo_never_reaches_card`. 고정쿼리의 `l.status='on_sale'`을 **일부러 지워 red를 확인**한 뒤 되돌려 green(B4). 같은 파일의 `test_query_joins_listings_and_filters_on_sale`이 WHERE절 존재를, 이 테스트가 **실제로 걸러지는지**를 각각 본다(존재 확인 ≠ 작동 확인).
...
- ⚠️ **이 검사들이 못 보는 것**(9.6에서 실측해 확인한 경계): `test_listing_cards.py`는 `run_select`를 가짜로 바꾸므로 **SQL이 Postgres에 닿지 않는다.** "조건을 안 쓰면 잡는다"는 보장하지만 "쓴 조건이 실DB에서 의도대로 도는지"는 보장하지 않는다 — 그 축은 9.6의 실브라우저·실DB 검증이 담당했다(스토리 Completion Notes).
...
- ✎ **2026-07-20 Story 9.6 — 세 번째 독립 실측이 이 정정을 다시 확인했다.** AI 카드가 고르는 대표사진 5장을 `curl`로 직접 재니 **152 / 217 / 260 / 274 / 336 KB** — 5장 중 **4장이 "196~205KB" 범위 밖**이고 최대값은 상한의 **1.6배**다. 이제 이 숫자가 틀렸다는 근거는 서로 다른 스토리의 실측 2건(#83의 217·421·423, 9.6의 위 5건)이다.
...
- ✅ **2026-07-20 Story 9.6 — 세 번째 소비처가 생겼는데 비교자는 안 늘었다(2벌 유지).**
  ...
  - **강제 장치:** `api/tests/test_listing_cards.py::test_query_orders_by_sort_order_then_id` — 쿼리에 `sort_order`·`id` 정렬이 있는지, 그리고 `is_cover`를 읽지 **않는지**를 단언한다(파생값을 읽으면 목록 카드와 대표가 갈린다).
