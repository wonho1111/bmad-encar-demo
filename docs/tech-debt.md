...
### 206. 큐가 막히면 **사용자가 다시 보낼 방법이 없다** — 재시도 트리거가 "다시 끊겼다 붙기" 하나뿐 (2026-07-29 Story 12.4 3차 후속 리뷰 adversarial+edge-case 교차 지적, 🟡 기능, defer)
...
### 215. 뒤로가기로 방 목록에 돌아오면 안읽음 배지가 **읽기 전 값으로 되살아난다** — Next Router Cache가 갱신 전 렌더를 복원한다 (2026-07-29 Story 12.5 3차 후속 리뷰 edge-case-hunter 지적 → 실브라우저 실측, 🟡 기능, defer)
