...
### 101. 파일명 규약 `NNNN[a-z]?_`의 **접미사 파일을 Supabase CLI가 조용히 건너뛴다** — 규약과 도구가 정면 충돌 (로컬 개발 DB 도입 중 발견, 2026-07-21 등재, 🔴 필수)
...
- **⚠️ 아직 안 닫힌다 — 다음 접미사 파일이 같은 방식으로 또 사라진다.** 게이트 정규식이 여전히 `[a-z]?`를 허용하므로 누군가 `0017b_...`를 만들면 **게이트는 초록인데 로컬 fresh DB에선 조용히 빠진다.** 문서로 막으면 또 안 지켜진다(B9).
- **해소안 (사용자 결정 필요 — 정본 계약 문서를 고치는 일이라 dev 자율 범위 밖):** 접미사 정본 (가)를 **폐지**하고 신규 마이그는 항상 `max+1`만 쓴다. 손댈 곳 4군데 — ① `check_migrations.py`의 `FILENAME_RE`에서 `([a-z]?)` 제거 + 규칙 ④(선행 정본 검사) 정리 ② `conventions.md` §9.2 (가) 절 삭제·(나)만 남김 ③ `deployment-runbook.md` §7-4·§8-7의 접미사 서술 ④ `db-schema-guide.md`의 `0003c` 표기 2곳. **이러면 규약이 CLI와 일치하고, 게이트가 강제하므로 다시 못 깨진다.**
- **왜 지금 안 했나:** `conventions.md`는 크로스-파트 계약 정본이고 §9.2는 (가)/(나) 판별 규칙 전체가 얽혀 있다. 파일 하나 개명은 되돌리기 1초지만 계약 개정은 아니다(A1 — 말없이 하나 고르지 않는다).

### 102. 개명된 `0003c`의 옛 이름이 **규칙 문서 4곳에 남아 있다** (#101의 잔여, 2026-07-21 등재, 🟢 품질/문서)
...
- **📅 #101의 해소안을 실행할 때 같이 고친다** — 어차피 같은 4개 문서를 연다. 따로 하면 두 번 연다.
