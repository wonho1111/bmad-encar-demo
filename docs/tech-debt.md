...
| 🟡 조건부 | **18** | 재오픈 차단(#9) · 타입(#12) · **GRANT(#18 — Epic 9)** · 관리자 운영 실사용 미확인(#19) · 거래일 `sold_at`(#20) · **게이트 구조적 대가 3종(#22~24 — Epic 13)** · psycopg 버전핀(#25) · 폐기토큰 401(#26) · **시드 멱등 delete(#27 — Epic 9·10)** · **앱 사진 업로더 계획 공백(#52 — Epic 16)** · **수정 중 sold 전환 관통(#54)** · **동시 편집 10장 초과 고아(#56)** · *(📅 예약: #6 멱등키→12-1 · #7 폴링배너→12-4 · #11 옵션쉼표→10-3 · **#54→9-7 · #56→Epic 9 회고**)* *(✎ 2026-07-19: `#51`은 아래 ✅ 행에서 닫힌 항목인데 여기 "원인 미상"으로도 남아 있었다 — 모순이라 제거)* *(✎ 2026-07-19 Story 9.0: `#55`·`#62`는 서명 URL이 사라져 **항목 자체가 소멸** — 행에서 제거, 9.5 예약도 해제)* *(✎ 2026-07-20 Story 9.5 코드리뷰: **썸네일이 원본을 그대로 받는 AC2 미충족(#83)** 추가 — 사용자가 데모 규모에서 수용, **트리거 = #68 전량 시딩**)* |
| 🟢 품질/테스트 | **20** ⚠️미신뢰 | AI 정규화 회귀테스트(#13) · FR17 경로(#14) · Riverpod 컨트롤러(#17) · 누수테스트 의존(#32) · DSN 포트(#33) · anon 테스트 중복(#34) · 시드 self-heal(#35) · FocusTrap 2종(#36·#37) · ErrorState tone(#38) · error useState(#39) · 런북 §8 누락(#41) · **randomUUID secure context(#64)** · *(📅 예약: #15 LIMIT→13-1 · #16 거리컷오프→13-6 · #40 Pretendard→11-0)* *(✎ 2026-07-19 Story 9.4 Task 0: **#57·#58·#63 해소** — 행에서 제거)* *(✎ 2026-07-20 Story 9.5: **#59 해소** — 마지막 소비처(상세 갤러리)가 정렬을 걸고 검사로 고정됨. 행에서 제거)* *(✎ 2026-07-20 Story 9.5 코드리뷰 이월 **4건 추가**: 갤러리 접근성 3종(#79 — Epic 10 트리거) · 사진 용량 근거 오기재(#80) · 정렬 로직 2벌(#81) · `InquiryCta` 이중 마운트(#82))* *(✎ 2026-07-20 Story 9.6: **#48·#71 해소** — 행에서 제거. #71은 AI 카드에 사진이 실제로 뜨면서 소멸. #48은 **한 번 잘못 닫혔다가 코드리뷰에서 다시 열린 뒤 실DB 게이트(`api-db` 잡)로 최종 종결** — 항목 본문의 경위 참조. **신규 1건**: `/ai` 390px 가로 넘침(#84 — 9.6 검증 중 발견, **9.6이 만든 것이 아님**을 대조군으로 확인. 트리거 = Epic 11.3/13.4/11.5))* *(✎ 2026-07-20 Story 9.6 코드리뷰 이월 **3건 추가**: `storage_path` 모양 미검증(#85 — 트리거 Epic 16.2) · 9.6 검증 산출물 재실행 불가(#86 — 트리거 Epic 11.5) · project-context §12 CI 경고문 노후(#87))* *(✎ 2026-07-20: **E2E 도구 클릭 무동작(#88)** — 세 번째 발생, 원인 미규명. 제품 결함 아님은 사용자 실브라우저로 확정)* |
...
- **해소 방향:** Playwright MCP 버전·실행 옵션(headless 여부, `--isolated`, 브라우저 채널)을 바꿔가며 재현 조건을 좁힌다. 재현이 특정 조건에 묶이면 그 조건을 피하는 것으로 끝난다.
