...
| 🟡 조건부 | **16** | 재오픈 차단(#9) · 타입(#12) · **GRANT(#18 — Epic 9)** · 관리자 운영 실사용 미확인(#19) · 거래일 `sold_at`(#20) · **게이트 구조적 대가 3종(#22~24 — Epic 13)** · psycopg 버전핀(#25) · 폐기토큰 401(#26) · **시드 멱등 delete(#27 — Epic 9·10)** · **앱 사진 업로더 계획 공백(#52 — Epic 16)** · **수정 중 sold 전환 관통(#54)** · **동시 편집 10장 초과 고아(#56)** · *(📅 예약: #6 멱등키→12-1 · #7 폴링배너→12-4 · #11 옵션쉼표→10-3 · **#54→9-7 · #56→Epic 9 회고**)* *(✎ 2026-07-19: `#51`은 아래 ✅ 행에서 닫힌 항목인데 여기 "원인 미상"으로도 남아 있었다 — 모순이라 제거)* *(✎ 2026-07-19 Story 9.0: `#55`·`#62`는 서명 URL이 사라져 **항목 자체가 소멸** — 행에서 제거, 9.5 예약도 해제)* *(✎ 2026-07-20 Story 9.5 코드리뷰: **썸네일이 원본을 그대로 받는 AC2 미충족(#83)** 추가 — 사용자가 데모 규모에서 수용, **트리거 = #68 전량 시딩**)* *(✎ 2026-07-21 Story 9.7: **#27·#54 해소** — 행에서 제거. #27은 시드 재실행이 사진·채팅을 지운다는 것을 빈 Postgres 실측으로 확정하고 운영 결론을 런북 §9에 박아 종결(다음 판단 지점 Epic 10.5는 그대로). #54는 마이그 `0015`가 `listings_update_own`의 `using`에 `status <> 'sold'`를 넣어 red-green 증명과 함께 해소. **#83은 트리거가 도달했으나 닫지 않았다** — "매물당 5~10장" 전제가 안 왔고(시드가 1~3장으로 섞는다) 실측 상세 1건 1,028KB. 트리거를 *"매물당 사진 장수 증가"* 로 정정)* |
| 🟢 품질/테스트 | **19** ⚠️미신뢰 | AI 정규화 회귀테스트(#13) · FR17 경로(#14) · Riverpod 컨트롤러(#17) · 누수테스트 의존(#32) · DSN 포트(#33) · anon 테스트 중복(#34) · 시드 self-heal(#35) · FocusTrap 2종(#36·#37) · ErrorState tone(#38) · error useState(#39) · 런북 §8 누락(#41) · **randomUUID secure context(#64)** · *(📅 예약: #15 LIMIT→13-1 · #16 거리컷오프→13-6 · #40 Pretendard→11-0)* *(✎ 2026-07-19 Story 9.4 Task 0: **#57·#58·#63 해소** — 행에서 제거)* *(✎ 2026-07-20 Story 9.5: **#59 해소** — 마지막 소비처(상세 갤러리)가 정렬을 걸고 검사로 고정됨. 행에서 제거)* *(✎ 2026-07-20 Story 9.5 코드리뷰 이월 **4건 추가**: 갤러리 접근성 3종(#79 — Epic 10 트리거) · 사진 용량 근거 오기재(#80) · 정렬 로직 2벌(#81) · `InquiryCta` 이중 마운트(#82))* *(✎ 2026-07-20 Story 9.6: **#48·#71 해소** — 행에서 제거. #71은 AI 카드에 사진이 실제로 뜨면서 소멸. #48은 **한 번 잘못 닫혔다가 코드리뷰에서 다시 열린 뒤 실DB 게이트(`api-db` 잡)로 최종 종결** — 항목 본문의 경위 참조. **신규 1건**: `/ai` 390px 가로 넘침(#84 — 9.6 검증 중 발견, **9.6이 만든 것이 아님**을 대조군으로 확인. 트리거 = Epic 11.3/13.4/11.5))* *(✎ 2026-07-20 Story 9.6 코드리뷰 이월 **3건 추가**: `storage_path` 모양 미검증(#85 — 트리거 Epic 16.2) · 9.6 검증 산출물 재실행 불가(#86 — 트리거 Epic 11.5) · project-context §12 CI 경고문 노후(#87))* *(✎ 2026-07-20: **E2E 도구 클릭 무동작(#88)** — 세 번째 발생, 원인 미규명. 제품 결함 아님은 사용자 실브라우저로 확정)* *(✎ 2026-07-21 Story 9.7: **#80 해소** — 행에서 제거. 대표사진 **90장 전량** 실측(50~615KB, 중앙값 197·평균 229)으로 "196~205KB"가 **값이 아니라 형태**로 틀렸음을 확정하고 `ListingCardImage.tsx` 주석을 정정. **#88은 발생 횟수 변동 없음(세 번째 그대로)** — 네 번째로 등재했다가 **오진으로 철회**했다(파일은 정상 저장돼 있었고 위치를 잘못 봤다. 경위는 항목 본문 — 대장이 없는 결함을 키울 뻔한 자리라 남겼다). **#73은 트리거 도달했으나 계속 이월** — 폴백 발화는 실측 확인(깨진 이미지 0), "N장" 배지가 남아 전면 장애를 판정할 신호가 생겼으나 설계된 관측 수단은 여전히 없음)* |
...
- **9.7에 남은 일 (스토리 생성 시 인수조건 체크박스로 심을 것 — B5):**
  - [ ] 100건 전량 시딩
  - [ ] **시드 2회 연속 실행 후 `listing_images` 행 수 유지 확인**(#27 — "에러 0건 ≠ 정상", `ON DELETE CASCADE`가 조용히 지울 수 있다). 1회차·2회차 매물 id 동일 여부 기록
  - [ ] `listings_update_own`에 `status <> 'sold'` 추가하는 마이그레이션(#54)
  - [ ] SM-A·CM-A·G3 검증(이 스토리가 exit-gate)
...
- ✎ **2026-07-21 Story 9.7 — 트리거 도달(전량 시딩 완료). 실험했고, 판단은 "계속 이월"이되 전제 하나가 실측으로 바뀌었다.**
  ...
  - **해소가 쉬워졌다는 것이 이 실험의 소득이다**: 새 데이터를 만들 필요 없이 **이미 카드가 들고 있는 `image_count` > 0 인데 이미지가 실패했다**는 조건 하나로 "전면 장애"를 판정할 수 있다. 트리거는 **실사용 배포 시점**으로 갱신(전량 시딩 트리거는 여기서 소진).
...
- ✅ **2026-07-21 Story 9.7 — 트리거(#68 전량 시딩) 도달. 재측정했고, 예측한 3~4MB는 오지 않았다.**
  ...
  - **트리거 변경 없음.** 다시 열 지점은 여전히 #68 전량 시딩 · 16.2 앱 · 실사용 배포이고, 그때의 답도 여전히 `next/image`다.
