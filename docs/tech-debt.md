...
### 74. Story 9.0 AC9의 **브라우저 검증이 미완**인데 #66이 닫혔다 (코드리뷰 2026-07-19 이월, 🟡 조건부 — 사용자 액션 필요)
...
- **🔴 사용자가 직접 해야 하는 일:** 관리자 계정으로 로그인 → 사진 있는 매물 삭제 → Storage에서 해당 경로 객체가 사라졌는지 확인. 판매자 계정으로도 동일 확인. (Claude는 로그인 세션을 못 연다.)
- **트리거:** 지금. 미루면 "고쳤다고 적힌 것"과 "고쳐진 것"의 차이가 계속 누적된다.
