...
| 🟡 조건부 | **19** | 재오픈 차단(#9) · 타입(#12) · **GRANT(#18 — Epic 9)** · 관리자 운영 실사용 미확인(#19) · 거래일 `sold_at`(#20) · **게이트 구조적 대가 3종(#22~24 — Epic 13)** · psycopg 버전핀(#25) · 폐기토큰 401(#26) · **시드 멱등 delete(#27 — Epic 9·10)** · **storage.objects 인증 DELETE/LIST 403(#51 — Epic 9.3, 원인 미상)** · **앱 사진 업로더 계획 공백(#52 — Epic 16)** · **수정 중 sold 전환 관통(#54)** · **서명 URL 만료 미복구(#55)** · **동시 편집 10장 초과 고아(#56)** · *(📅 예약: #6 멱등키→12-1 · #7 폴링배너→12-4 · #11 옵션쉼표→10-3)* |
| 🟢 품질/테스트 | **18** | AI 정규화 회귀테스트(#13) · FR17 경로(#14) · Riverpod 컨트롤러(#17) · 누수테스트 의존(#32) · DSN 포트(#33) · anon 테스트 중복(#34) · 시드 self-heal(#35) · FocusTrap 2종(#36·#37) · ErrorState tone(#38) · error useState(#39) · 런북 §8 누락(#41) · **WebP 폴백 미검증(#57)** · **애니메이션/알파 손실 무고지(#58)** · **§10.1 정렬 서술 미래형(#59)** · *(📅 예약: #15 LIMIT→13-1 · #16 거리컷오프→13-6 · #40 Pretendard→11-0)* |
...
| ✅ 해소 | **6** (+3) → 9.3 리뷰로 **#51 닫힘 추가**(원인 규명, 아래 항목 참조) | DB 커넥션 풀(#5, 8.4) · 채팅 본문 길이(#8, 0010) · open-redirect(#10, 8.5) · **E2E 대본 계약 오기재(#28 — 대본 폐기로 종결)** · **CI 구멍(#29 — tests.yml)** · **architecture baseline 배너(#31)** *(+ 부록: 4-1 이월 3건이 4.2·4.6·4.7에서 해소된 것으로 실측 확인)* |
...
### 60. 🔴 **매물을 삭제하면 그 매물의 사진 파일이 전부 영구 고아가 된다** (9.3 리뷰 후속 실측, 2026-07-19)
...
### 61. 매물 저장 버튼의 동시 더블클릭을 `loading` 가드가 막지 못한다 (9.3 리뷰 후속 실측, 🟡 조건부)
