# 결정 아카이브 — 왜 그렇게 정했나 (경위 전용)

> ## 이 문서는 규칙이 아니다. 지켜야 하는 것을 찾으러 왔다면 여기가 아니다.
>
> 여기 있는 것은 **작성 시점의 판단과 그 근거**다 — party-mode 토론, 실측으로 뒤집힌 주장,
> 설계 경위. 이유 없는 규칙은 다음 사람이 또 뒤집으므로 근거를 보존한다(CLAUDE.md B8:
> "제약은 짧게 정본 하나, 경위는 길어도 된다 — 섞지 않는다").
>
> | 찾는 것 | 가야 할 곳 |
> |---|---|
> | **지금 뭐가 열려 있나?** | `_bmad-output/implementation-artifacts/deferred-work.md` (열린 일 장부) |
> | **지켜야 하는 계약은?** | `docs/conventions.md`(크로스-파트) · `_bmad-output/project-context.md`(구현 규칙) |
> | **왜 그렇게 정했나?** | **이 문서** · 각 에픽 회고 |
> | 닫힌 부채의 경위는? | `docs/tech-debt.md` (2026-07-29 전방 동결) |
>
> ### ⚠️ 읽을 때 주의 — 여기 적힌 판정이 최신이 아닐 수 있다
> 아래 서술은 그때의 판단이고, 이후 실측·코드리뷰로 뒤집힌 것이 있다. **이 문서와 장부가
> 어긋나면 장부가 맞다.**
>
> 실제로 그 어긋남이 사고를 냈다 — 아래 "결정 2"의 테이블 GRANT 판정이 장부와 **정반대**
> (여기 "(a) dev 자율" / 장부 "(b) 사용자 승인")로 1일간 공존했고, Epic 9이 그 축을 건드리기
> 직전이었다. 2026-07-15에 (b)로 통일했다.

---

## 이 문서의 유래 (2026-07-29 장부 통합)

이 내용은 원래 `_bmad-output/implementation-artifacts/deferred-work.md` 앞부분에 있었다.
그 파일은 2026-07-15에 "경위 보관용"으로 동결됐지만, **bmad-loop 엔진이 그 동결을 읽지
못해** 계속 새 항목(`DW-1`~`DW-9`)을 append 했다 — 엔진은 Claude 세션이 아니라 파이썬
프로세스라 PreToolUse 훅이 호출조차 되지 않았다.

2026-07-29에 방향을 뒤집어 그 파일을 **BMAD 표준대로 살아있는 장부**로 되돌렸고
(`bmad-loop` 상류가 `deferred-work.md`를 유일한 ledger로 규정한다), 섞여 있던 경위 산문을
이 문서로 분리했다. 실측 근거: 통합 직전 `bmad-loop sweep --dry-run`이 아래 산문을
**"옛 형식으로 적힌 열린 일 61건"으로 오독**했고, 실제 sweep은 파일 전체를 재작성했을 것이다.

> ⚠️ **다른 문서가 이 내용을 `deferred-work.md:52-53` 같은 줄번호로 인용한 곳이 있다.**
> 그 좌표는 이관 이전에 **이미 어긋나 있었다**(원 파일이 19회 수정되는 동안 밀렸다).
> 줄번호가 아니라 **섹션 제목으로** 찾아라.
...
## ✅ 결정됨: 앱은 §8 "게이트가 입력을 삼키지 않는다"를 따르지 않는다 (사용자 결정 2026-08-10)

**무엇을 정했나.** Flutter 앱에서 비로그인이 AI 검색을 전송하면 **질의를 버리고 `/login`으로 보낸다.**
로그인 후에도 원래 화면·질의로 돌아가지 않고 `/home`으로만 간다. 이것이 앱의 **의도된 동작**이다.
**웹은 손대지 않는다** — `web/src/lib/heroSearchHandoff.ts`는 양 갈래 모두 그대로 유지한다.

**어떻게 나온 결정인가.** Epic 16 묶음 코드리뷰(2026-08-10)에서 **서로 모르는 리뷰어 3명**이
`docs/conventions.md` §8의 *"행동 게이트는 사용자 입력을 삼키지 않는다 — 보존했다가 로그인 복귀 시
복원한다"* 를 근거로 앱을 지적했다. 계약이 웹 전용이라고 적혀 있지 않았고, `epic-16-context.md`가
*"'웹 한정'이라고 적혀 있지 않으면 전부 앱도 포함"* 을 일반 규칙으로 못박고 있어 지적은 형식상 옳았다.

사용자 판단: *"앱에서는 그 기능 신경 쓰지 마, 따로 뭐 건들지를 마. 앱에서는 그냥 검색 누르면
로그인 기능으로 넘어가고 끝. 대장에 어긋나는 거면 예외처리하고."*

**중간에 뒤집힌 안 — 기록해 둔다.** 오케스트레이터가 처음 제안한 것은 "웹에서도 이 기능을 제거해
양쪽을 통일"이었다. 코드를 열어보니 **그 모듈이 두 갈래를 처리**하고 있어 성립하지 않았다:

| 갈래 | 하는 일 | 지우면 |
|---|---|---|
| `autoRun: true` | **로그인** 사용자가 랜딩 히어로에 질의 제출 → `/ai`로 이동해 **1회 자동 실행** | **Epic 11 랜딩의 핵심 동선이 끊긴다**(AI 화면이 빈 채로 뜸) |
| `autoRun: false` | **비로그인** 게이트 → 로그인 후 랜딩 입력창 **복원만**(자동 실행 금지 = 재과금 방지) | 이 갈래만 제거 가능 |

즉 "기능 통째로 제거"는 불가능했고, 사용자는 그 설명을 받은 뒤 **웹은 아예 건드리지 않는 쪽**으로
결정했다. (이 에픽은 이미 웹을 세 번 범위 밖으로 건드렸다 — 실루엣·헤드라인 크기·그라데이션 축.
여기서 끊는 것은 그 흐름과도 맞다.)

**어디에 박았나.** 계약 정본 `docs/conventions.md` §8의 해당 조항에 **"⚠️ 웹 한정, 앱은 예외"** 를
직접 달았다. **코드 주석으로 처리하지 않은 이유**: 리뷰어는 코드가 아니라 **계약 문서**를 읽고
지적한다 — 정본을 안 고치면 다음 리뷰가 같은 지적을 그대로 반복한다(이번에 3명이 그랬다).
CLAUDE.md B9: *"주석·문서는 계약이 아니다. 지켜야 하는 규칙이면 어길 수 없는 자리에 박는다."*

**절충(감수하기로 한 것).** 비로그인 사용자가 앱에서 AI에 질문을 쓰다 로그인 게이트를 만나면
**처음부터 다시 타이핑**해야 한다. 시연 동선에 노출될 수 있음을 알고 내린 결정이다.

**부수 사실 하나.** `app/test/ai_chat_screen_test.dart`가 이미
`expect(find.text('아반떼 찾아줘'), findsNothing)` 으로 **"입력이 사라진다"를 단언**하고 있었다.
이 결정 이전에는 그것이 "검사가 계약 위반을 고정하고 있는" 상태였고, 이 결정으로 **의도된 동작을
지키는 검사**가 됐다. 같은 코드가 결정 하나로 성격이 바뀐 사례라 남긴다.
