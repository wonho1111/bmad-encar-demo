...
4. **정본 파일 in-place 수정 + 따라잡기 패치 규약** (`docs/conventions.md` §9에 상술):
   ...
   - 이미 그 정본을 적용받은 **살아있는 원격 DB**에 그 수정을 반영해야 하면, **따라잡기 패치**를 만들어 **원격에만** 적용한다(레포에 파일로 남기지 않으므로 파일명 규약의 적용 대상이 아니다). 신규 환경은 정본 파일 자체가 이미 최신이라 따라잡기 패치가 불필요하다.
   - ⚠️ **레포에 새 파일을 만들 때 알파벳 접미사(`NNNNb_`)는 금지다** — 항상 `max+1`을 쓴다. Supabase CLI가 접미사 파일을 조용히 건너뛰기 때문이며, 게이트가 종료코드 1로 막는다(`docs/conventions.md` §9.2 · 대장 #101).
...
7. ⚠️ **게이트는 파일이 *없어진 것*을 모른다 — 인벤토리 검사가 아니다.** 프로브 3건은 `listings`·`guide_documents`만 보므로 chat 계열이 통째로 사라져도 모른다. 다만 **접미사 파일을 지워도 초록이던 구멍은 닫혔다** — 접미사 자체가 금지되어(2026-07-21) 밀집 검사가 이제 모든 파일을 대상으로 하므로, 중간 번호가 사라지면 `[번호 공백]`으로 잡힌다. **맨 끝 번호를 지우는 것은 여전히 안 잡힌다**(max가 같이 줄어들어 밀집이 유지된다).
