...
| `price` (가격) | 정수 **원(KRW)** | `2,980만원` (⚠️ 아래) | "2,980만원" | 음수 불가 |
...
⚠️ **가격은 저장 단위와 표시 단위가 다르다**(2026-08-13 사용자 결정). 저장·입력·검색 쿼리는 **원**
그대로이고, **화면에 찍는 글자만 만원**이다(`29,800,000원` → `2,980만원`). UX 목업 전체와 엔카·
헤이딜러 같은 실서비스가 만원 표기라 자릿수가 길어 한눈에 안 읽히던 문제를 고친 것이다.

- 표시 규칙은 `web/src/lib/price.ts`의 `formatPrice()` **한 곳**에만 둔다 — 화면에서
  `price.toLocaleString() + '원'`을 직접 조립하지 않는다(같은 규칙이 10군데로 흩어지면 또 갈린다).
- 만원 단위로 **딱 떨어지지 않는 값은 만원으로 뭉개지 않고 원 표기를 그대로 쓴다.** 가격은 반올림이
  곧 오표시라, 짧게 보이는 것보다 틀리지 않는 것이 우선이다.
- **원 단위가 그대로 남는 자리**: `/sell` 등록 폼의 "가격 (원)" 입력, `/search`의 가격 범위 입력.
  사용자가 숫자를 **넣는** 자리는 저장 단위와 같아야 변환 실수가 안 생긴다.
...
- 강제 지점:
  ...
  - **SECURITY DEFINER 함수 축** — 정의자 함수 안에서는 RLS가 적용되지 않는다(정의자=소유자 권한으로 평가되고, 소유자에겐 RLS가 애초에 안 걸린다). 그래서 위 두 축과 달리 RLS가 대신 걸러주지 않고, **함수 본문의 인라인 조건이 유일한 강제 지점**이다.
    ...
    - `increment_listing_view`(`0020_listings_view_count.sql`, 상세 조회수 증가 — Story 11.1. 정지 가드는 Story 17.2 `0034_view_count_suspended_guard.sql`, DW-805) — **`anon`·`authenticated` 둘 다** `grant execute`(FR58, 비로그인 상세 열람도 조회수를 올려야 한다). 정의자 함수가 `public.listings.view_count`를 UPDATE하는 **쓰기** 경로라 RLS(`listings_update_own`)가 대신 걸러 주지 않는다 — 함수 본문의 `not exists(auth.uid()가 가리키는 profiles 행이 status='suspended')`가 유일한 강제 지점이며, 지우면 정지 회원이 임의 매물의 조회수를 계속 부풀릴 수 있다(0032 9절이 원래 "해당 없음 — anon도 호출"로 판정했던 자리, DW-805가 뒤집음). `auth.uid()`가 NULL(비로그인)이면 조건이 항상 참 → 통과 — 우연이 아니라 설계다. 강제 장치: `api/tests/integration/test_suspended_write_block_real_db.py`(정지·비로그인·활성 3주체 실호출, view_count 증분값 확인) + `api/tests/integration/test_write_policy_manifest_real_db.py`(DW-803 전수조사 매니페스트 대조).
...
### 6.2 정지 판매자 비노출 (Story 17.4, DW-804(a))

`profiles.status='suspended'`인 판매자의 매물은 **구매자·비로그인·AI 응답 어느 조회 경로에서도 노출되지 않는다.** 단 **판매자 본인과 관리자에게는 그대로 보인다.** ✎ 이 축은 17.1이 세운 "정지는 열람을 막지 않는다"는 원칙을 **매물 조회에 한해** 뒤집는다(2026-08-11 사용자 결정, Discord — 세 선택지(숨김/유지/배지) 중 "숨겨야돼". 근거는 `_bmad-output/implementation-artifacts/epic-17-context.md`·`spec-17-4-정지-판매자-매물-비노출.md`의 Intent가 정본).

- **판정 함수**: `private.is_seller_active(uuid)`(`0036_is_seller_active_private_schema.sql`, SECURITY DEFINER) — `is_admin()`(0001)·`is_admin_active()`(0032)와 같은 이유로 정의자 함수다. 매물의 **판매자**(`listings.seller_id`, 호출자 본인이 아닌 "남")가 활성인지 물어야 하는데, `profiles` SELECT 정책은 본인·관리자뿐이라 인라인 서브쿼리로 짜면 `profiles` RLS가 조용히 걸러 exists가 항상 거짓이 되고 **전체 매물이 사라진다**(2026-08-12 로컬 실측으로 확정, 마이그레이션 파일 헤더 참조). ✎ **왜 `public`이 아니라 `private` 스키마인가(2026-08-12 후속 코드리뷰 patch, P1)**: 최초 구현(`0035`)은 `public.is_seller_active`로 만들고 `revoke all ... from public`만으로 노출을 회수했다고 적었는데, 실측(로컬 55321)으로 그 REVOKE가 막지 못함이 드러났다 — `POST /rest/v1/rpc/is_seller_active`를 **anon 키만으로** 호출하면 임의 uuid의 정지 여부(`true`/`false`)를 그대로 돌려줬다(`profiles` 직접 조회는 RLS가 `[]`로 정상 차단하는 것과 대조적). `supabase/config.toml`의 노출 스키마(`public`·`graphql_public`)에 함수가 있으면 PostgREST가 GRANT를 그대로 RPC 경로로 연결하기 때문이다 — `is_admin()`/`is_admin_active()`처럼 인자 없이 `authenticated`에만 grant된 함수는 REVOKE로 anon 경로가 실제로 막히지만, 이 함수는 인자가 있고 `anon`에도 EXECUTE가 필요해(정책이 `anon` 세션으로 평가되므로) 같은 방식이 통하지 않았다. 그래서 `0036`이 노출 목록 밖의 `private` 스키마로 함수를 옮겨 RPC 경로 자체를 없앴다(REVOKE가 아니라 노출면 제거).
- 강제 지점:
  - **매물 축** — `listings_select_on_sale`(authenticated)·`listings_select_on_sale_anon`(anon)에 `and private.is_seller_active(seller_id)` 추가. 본인(`listings_select_own`)·관리자(`listings_select_admin`)는 건드리지 않는다. 강제 장치: `api/tests/integration/test_suspended_seller_hidden_real_db.py`.
  - **이미지 축** — `listing_images_select_on_sale`/`_anon`(0012)이 `listings`에 조인하므로(§6 "이미지 축"과 동일 메커니즘) **별도 정책 추가 없이 자동으로 상속된다**(2026-08-12 로컬 실측으로 확정 — 추론이 아니라 재본 결과). 강제 장치: 위와 동일 파일.
  - **SECURITY DEFINER 축** — `get_seller_public_summary`(0019)의 on_sale 집계 서브쿼리에 `and private.is_seller_active(p_seller_id)` 추가(함수 안 조건이 유일한 강제 지점, §6 SECURITY DEFINER 축과 동일 성질). 강제 장치: `api/tests/integration/test_seller_summary_real_db.py`.
  - **AI 3경로 축** — `listings_ai_readonly_select`(0006)를 `using(true)` → `using(private.is_seller_active(seller_id))`로 좁혀 경로 A(sql_guard 검증 LLM SQL)·경로 B(`doc_rag_node.py` 고정 SQL)·하이브리드(`hybrid_rag_node.py`)를 **코드 수정 없이** 한 자리에서 닫는다. FR11(sold)의 `using(true)` 설계 자체는 그대로다 — status 필터는 계속 코드(각 노드의 `WHERE status='on_sale'`)가 책임진다(2026-08-12 로컬 실측: sold 매물은 판매자가 활성이어도 `ai_readonly`에 계속 보임 — 이 축이 안 깨졌다는 회귀 고정). 강제 장치: `test_suspended_seller_hidden_real_db.py`(`set local role ai_readonly` 직접 확인) + `test_doc_rag_node_real_db.py`(문서 RAG 실제 호출).
- **판매자 profiles 행이 없는 매물**: `listings.seller_id`는 `references public.profiles(id) on delete cascade`(0002)라 판매자 없는 매물은 FK가 존재 자체를 막는다 — 이 분기는 도달 불가능이지만 `is_seller_active`는 방어적으로 "판정 불가 = 숨김"(exists 거짓 → false) 쪽으로 fail-closed한다.
- **찜(`/wishlist`) 회귀**: `isWishedListingBlocked`(`web/src/lib/wishlist.ts`)의 판정 로직 자체는 `embed === null`(RLS 차단)이면 sold와 동일하게 회색 타일로 분류한다 — 코드 경로는 새로 만들지 않고 기존 sold 처리를 그대로 재사용한다. ⚠️ 단 **"실제로 흡수되는 것"의 확인 수준은 raw SQL 시뮬레이션까지다** — `wishlists LEFT JOIN listings`를 직접 재현해 PostgREST의 null-embed 메커니즘이 정지 판매자 트리거에서도 에러 없이 NULL을 낸다는 것만 확인했고, 실제 PostgREST HTTP 응답이나 `/wishlist` 화면 렌더까지 이 경로로 e2e 검증하지는 않았다(스펙 Design Notes 참조). 남은 확인은 `web/src/lib/__tests__/wishlist.test.ts`(`isWishedListingBlocked(null) === true`, sold 축으로 이미 존재 — 정지 축도 같은 null 입력이라 동일 단언이 적용된다)와 웹 E2E의 몫이다.
- **채팅방 생성 진입점(`chat_rooms_insert_participant`)은 이 축이 안 닫는다** — `enforce_chat_room_seller()`(0016)가 SECURITY DEFINER라 listings RLS를 우회해 seller_id를 조회하므로, listing_id를 이미 아는 구매자는 API를 직접 호출해 여전히 방을 만들 수 있다(2026-08-12 실측). 브라우저 정상 흐름(상세페이지 → 문의하기)의 진입점은 상세페이지가 404가 되어 사라진다. 판정은 [[DW-799]]가 갱신해 들고 있다.
...
- **정지(`profiles.status='suspended'`)도 이 축의 "행동" 게이트다(DW-669, Story 17.1):** 막는 것은 매물·사진·파일 등록/수정/삭제와 관리자 쓰기 액션(회원 삭제·매물 삭제·되돌리기 RPC 등)뿐이고, 로그인·열람(매물 조회, 본인 profiles 조회 등)은 막지 않는다. ✎ **2026-08-11 사용자 결정으로 매물 조회도 좁혔다(Story 17.4, DW-804(a))** — 단 **본인·관리자 조회는 그대로**, 다른 축(로그인·profiles 조회 등)은 원문대로다. 즉 좁아진 것은 "정지된 계정이 보는 것"이 아니라 "정지된 계정의 매물을 남이 보는 것"이다 — 원문을 지우지 않는 이유는 17.1의 코드·검사·마이그레이션 주석이 이 원문을 근거로 쓰였기 때문이다(§6.2가 이 축의 정본). 강제 지점은 앱 코드가 아니라 `supabase/migrations/0032_suspended_write_block.sql`의 RLS 정책 + `public.is_admin_active()`(SECURITY DEFINER — RLS를 우회하는 `admin_restore_sold_listing` RPC도 이 함수로 함께 막는다, DW-721)다 — 화면마다 심지 않고 데이터 계층 하나로 모든 클라이언트를 예외 없이 막는다. **채팅 발신도 이 축에 포함된다** — 정지된 참가자의 `chat_messages` INSERT는 막고(`chat_messages_insert_participant`), 방송(0023)·안읽음 계산(0024~0026)은 "INSERT 성공 이후"에만 동작하므로 이 차단이 정상 참가자의 흐름을 깨지 않는다(실DB 확인: `api/tests/integration/test_suspended_write_block_real_db.py`). **조회수 RPC도 이 축에 포함된다(Story 17.2, DW-805):** `increment_listing_view`(SECURITY DEFINER, `0034_view_count_suspended_guard.sql`)는 함수 본문의 `not exists(auth.uid()가 가리키는 profiles 행이 status='suspended')`로 정지 회원의 조회수 증가만 막는다 — `auth.uid()`가 NULL인 비로그인(anon) 호출은 그 조건이 항상 참이라 지금과 동일하게 통과한다(FR58 회귀 아님, 설계 의도). 이전 판정("해당 없음 — anon도 호출하므로 게이트 불가")은 "anon 예외"와 "authenticated 정지 회원 예외"를 같은 것으로 취급한 오류였다(DW-805가 뒤집음). **막지 않는 쓰기(의도적 현재 범위, 2026-08-11 전수조사 — `pg_policies`·`pg_proc` 실측, 0032가 건드린 목록에 있는 것만 "막는다"다)**: `chat_rooms_insert_participant`(채팅방 생성 — 발신만 결정 대상이었다, [[DW-799]]로 이월) · `wishlists_insert_own`/`wishlists_delete_own`(찜 추가/삭제) · `chat_room_reads_insert_participant`/`chat_room_reads_update_own`(안읽음 커서 갱신). 근거: `_bmad-output/implementation-artifacts/deferred-work.md`의 [[DW-799]]·[[DW-801]]·[[DW-802]].
- **정지의 강제는 DB에만 있고, 화면은 거부 이후 사유를 설명한다(Story 17.3, DW-806·DW-804(b)).** 화면 검사를 강제력으로 쓰지 않는다 — 쓰기를 미리 막는 사전 검사가 아니라, DB가 이미 0행/`42501`로 거부한 **뒤에** 본인 `status`를 조회해 정지가 사유였는지만 구분한다(`web/src/lib/auth/status.ts`의 `getOwnStatus`/`writeRejectionMessage`가 유일한 조회 경로). **단, `/admin` 콘솔 게이트(`web/src/lib/auth/guard.ts`의 `requireRole`)는 이 원칙의 명시적 예외다 — 쓰기와 읽기의 강제 자리가 다르기 때문이다**(2026-08-11 후속 코드리뷰 실측 정정. 그 전 문장은 *"이 게이트는 미리 알려줄 뿐 못 막아도 DB가 막는다"* 였는데 **읽기 축에서 거짓**이었다): 관리자 **쓰기**(회원 삭제·매물 삭제·복원 RPC)는 `0032`가 `is_admin_active()`로 교체해 DB가 막지만, 관리자 **읽기**(콘솔의 회원·매물·채팅 목록/상세)는 `0032`가 admin SELECT 정책(`listings_select_admin`·`profiles_select_admin`·`chat_*_select_admin`·`listing_images_select_admin`)을 **일부러 `is_admin()` 그대로 뒀다**(`0032` 파일 주석 13~18·45~46·265~266이 그 결정을 명시 — `is_admin()`을 전역으로 좁히면 읽기 정책까지 함께 좁아진다). 그래서 정지된 관리자는 DB 수준에서 여전히 관리자 읽기 권한을 갖고 있고, **`requireRole`의 `status` 검사가 콘솔 열람의 유일한 차단**이다. 이것이 위 "정지는 열람을 막지 않는다"의 유일한 예외이며, 이 조건은 *"어차피 DB가 막는다"* 를 근거로 지우면 안 된다(지우면 정지된 관리자가 회원 개인정보·채팅 로그를 다시 열람한다). 강제 장치(단위, **CI에서 실제로 돈다** — `web` 잡의 vitest): `web/src/lib/auth/guard.test.ts`·`web/src/lib/auth/status.test.ts`·`web/src/lib/auth/__tests__/suspendedGateWiringContract.test.ts`(두 게이트의 락스텝 + 4개 쓰기 경로의 "거부 **뒤** 조회" 순서를 정적 스캔으로 고정). 강제 장치(E2E, ⚠️ **CI 미배선 — 로컬 `npm run test:e2e` 전용**, [[DW-468]] — `wont-do 2026-08-10`, E2E는 CI에 붙이지 않기로 확정): `web/e2e/suspended-access.spec.ts`.
