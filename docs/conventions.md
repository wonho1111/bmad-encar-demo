...
- **정본**: `NNNN_이름.sql` (4자리 번호, 유일, 밀집 — 0001부터 공백 없이). fresh DB(신규 환경)의 **단일 출처**다.
- **따라잡기 패치**: `NNNN[b-z]_이름.sql` (같은 바닥번호 + 알파벳 접미사). 정본 파일을 코드리뷰 지적 등으로 **in-place 수정**했을 때, *이미 살아있는* 원격 DB만 그 수정을 따라오게 하는 **멱등 패치**다. 신규 환경은 정본이 이미 최신이므로 따라잡기 패치가 불필요 — 파일로 남기지 않는다.

이 규약은 이미 실무에서 쓰이고 있었다(`0002b`·`0003b`·`0003c_revoke_trigger_execute`가 그 산물)— 여기 적기 전까진 어디에도 문서화돼 있지 않았다.
...
- **(a) → 개발자 자율 처리(사후 보고).** 아래 3조건을 **전부** 만족할 때:
  ...
  3. **기존 객체 정의가 불변**이다(컬럼 타입·제약·정책 술어·GRANT 대상 변경은 이 틀 밖).
- **(b) → 멈추고 사용자 승인.** 3조건 중 하나라도 어기면. 특히 **원격에 따라잡기 패치(`NNNNb`)가 실제로 필요해지는 순간 = 살아있는 공유 DB를 건드리는 순간 = 무조건 승인**이 필요하다.
