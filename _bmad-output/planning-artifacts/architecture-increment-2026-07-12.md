...
- **F14 역할통합(FR52~54,58)** — 별도 워크스트림. profiles.role 존치(is_admin 의존)+CHECK 완화, 소유권 기반 접근제어로 통일, **비로그인 열람(목록·상세) 허용 — 게이트는 행동에만(AI 검색 포함)**. *(2026-07-14 개정: AI 검색은 호출당 Gemini 실비가 나가는 **행동**이라 로그인 필수 — I2' 원칙 참조. anon에 여는 건 열람뿐이다.)*
...
- **ADR-IMG-02 이미지 스토리지 = 공개 버킷 + 고정 URL (2026-07-19, 사용자 결정 — ADR-IMG-01을 대체함)**: 버킷 `public=true`(`0014`), URL은 `getPublicUrl(bucket, path)`로 경로에서 조립(만료·발급 왕복 없음). `storage.objects` 읽기 정책은 `listing_images` 조인 → **경로 기반**(소유자 또는 관리자)으로 교체. **업로드 권한·MIME 3종·5MB·10장·경로 무결성 트리거는 전부 유지**("공개"는 읽기만이다). **FR11 재정의**: 사진 파일 URL은 FR11 강제 대상이 **아니다** — 매물의 목록·상세·AI 비노출은 listings RLS·sql_guard가 그대로 강제하고, sold 후에도 URL을 아는 사람은 파일을 열 수 있음을 **명시 수용**한다(`docs/conventions.md` §6.1, `docs/tech-debt.md` #65). **왜 뒤집었나**: ADR-IMG-01은 비공개+서명의 복잡도를 다 치르고도 서명 URL이 발급 시점에만 RLS를 보고 TTL(1시간) 동안 재검사하지 않아 sold 후에도 뚫려 있었다(#50-2 — "수용"으로 기록돼 있었다). 비용은 전부 내고 효과는 못 얻으면서 부채 5건(#45·#46·#50-2·#55·#62)을 만들었다. 전환으로 그 5건이 해소·소멸했고 삭제 순서 계약(§10.1)의 위험성도 사라졌다. 실측 근거는 Story 9.0 Debug Log.
...
- **~~I2 비로그인 AI검색~~ → 철회(2026-07-14, 코드리뷰 + party-mode)**: ~~api 델타에 `auth.py` 수정 추가 — `/ai/search` JWT 게이트를 비로그인 허용으로 완화(FR58). ai_readonly·sql_guard 안전장치는 유지.~~
  **→ `/ai/search`는 JWT 필수를 유지한다.** FR58에서 anon에 여는 것은 **열람(매물 목록·상세)뿐**이고, AI 검색은 **행동**이라 게이트에 남는다. `auth.py` 수정 없음(`get_current_user` 그대로).
  **왜 틀렸나 — 이 항목이 남긴 교훈**: 원 I2의 "ai_readonly·sql_guard 안전장치는 유지"는 **사실이지만 답이 아니었다.** 그 둘은 **DB 권한**을 지키지 **API 키 지출**을 지키지 않는다. 즉 "JWT를 완화해도 권한 누수가 없다"는 참이나, 그게 "열어도 안전하다"를 뜻하지 않는다. 검색 1회 = Gemini 호출 3회 내외 = **실제 과금**이고, JWT가 호출자를 식별하는 **유일한 수단** = 유일한 과금 울타리였다. **I2는 "안전"을 DB 권한 축에서만 물었고, 비용 축은 질문조차 되지 않았다** — PRD FR58의 비용 각주가 여기로 전파되지 않았기 때문이다(계보 끊김). 아래 원칙이 그 재발을 막는다.
- **★ I2' 원칙 — 비용이 발생하는 엔드포인트는 신원을 요구한다** (2026-07-14 신설, **전 에픽 governing**):
  - **분류 기준**: "열람이냐 행동이냐"는 화면이 아니라 **대가**로 가른다. 서버 자원을 **읽기만** 하면 열람(anon 허용 가능), 상태를 바꾸거나 **외부 유료 API를 호출해 청구서를 만들면** 행동(신원 필수).
  - **왜**: 인증은 접근제어 장치이자 **동시에 유일한 과금 울타리**다. 신원이 없으면 남용자를 특정할 수도, 한도를 걸 수도, 차단할 수도 없다. 익명에 한도를 걸려 해도 식별자가 IP뿐인데, Cloud Run 오토스케일에서 인메모리 카운터는 인스턴스마다 갈라지고(공유하려면 Redis+VPC — 얻는 건 프록시로 뚫리는 IP 상한), NAT·CGNAT은 무고한 사용자를 오폭한다. **신원을 지켜두면 나중 제한이 싸진다** — JWT 사용자 ID로 Postgres 카운트 한 줄(Redis 불필요).
  - **적용 대상(현재)**: `/ai/search`(Gemini). **⚠️ Epic 13(F13 RAG 고도화)에서 다시 확인할 것.**
    - **정확한 비용 구조**: 임베딩 과금은 **이미 오늘도 경로 B(문서 RAG, `doc_rag_node.py`의 `embed_query`)에서 발생 중**이다. Epic 13이 새 비용을 만드는 게 아니라, **HYBRID(13.3)가 SQL 생성(LLM) + 임베딩을 둘 다 태워** 그 비용을 훨씬 많은 질의로 **퍼뜨린다** — 현재는 A(임베딩 없음) / B(임베딩만) 중 택일이고, 실사용 질의는 대개 혼합형이라 상당수가 HYBRID로 간다. 검색 1회 ≈ 맥락정리+경로판단+(SQL생성 또는 임베딩) = 2~3회 → HYBRID 후 최대 4회.
    - **재발 예상 지점**: 13.3에서 *"sql_guard가 `status='on_sale'`을 강제하고 embedding 화이트리스트도 있으니 안전"* 이라는 답이 나올 것이다(**I4가 실제로 그렇게만 적혀 있다**). 그건 **DB 권한 축 답이고 비용 축은 또 질문되지 않은 것** — 이번 I2와 정확히 같은 함정이다.
    - **다만 8.5의 (A) 결정으로 상한은 이미 가입자 수에 묶여 있어** 정책 논의를 처음부터 다시 열 필요는 없다. 이 원칙이 실제로 걸리는 건 ① **신규 유료 API 연동** ② **계정당 쿼터가 필요해질 때**(→ JWT 사용자 ID로 Postgres 카운트 한 줄, Redis 불필요).
