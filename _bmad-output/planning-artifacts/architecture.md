...
create table listings (
  ...
  year          int  not null check (year >= 1990 and year <= (extract(year from now())::int + 1)),  -- 상한=올해+1(신차년식)
...
>
> **차형 용어 매핑(OI5):** `body_type` 값에 "세단·해치백·쿠페" 같은 차체 형태 용어가 없다(Encar식 크기 기반 분류). 사용자가 그런 용어로 질의하면 Text-to-SQL이 적절한 차종 값으로 매핑하거나(예: "세단"→준중형차·중형차·대형차) 무시하는 규칙을 질의셋(OI5)에서 정의한다.
