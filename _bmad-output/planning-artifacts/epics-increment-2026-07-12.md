...
- FR48: 가이드 문서 청킹·임베딩(조건부) — 현 규모(활성 10문서, 전체 12개 중 2개는 `_excluded/`, 13.9 DW-620 정정) 미도입. ≥20문서/≥800토큰 시 ## 헤딩 청킹.
...
>
> **📌 Epic 9 실적 반영 (2026-07-21, Story 9.7):** 이 표는 계획이고 **정본은 `supabase/migrations/`의 파일 목록**이다. Epic 9가 계획보다 마이그를 **3장 더** 썼으므로 표의 `0013` 이후가 전부 밀렸다. 실제로 쓰인 번호:
>
> | 실제 파일 번호 | 내용 | 소유 스토리 |
> |---|---|---|
> | 0013 | listing_images 경로 무결성 트리거 | **9.1** (계획 밖 추가) |
...
**➕ 2026-07-29 범위 추가(사용자 결정, 대장 `DW-546`):** 아직 리스킨되지 않은 **소비자 화면 9~10개**(로그인·회원가입·채팅·검색필터·판매 폼 등)도 여기서 함께 통일한다 — FR37이 "전 화면"인데 실행 스토리가 없어 새어 있었고, 관리자 축과 작업이 동일해 세팅을 두 번 하지 않는다. 상세 목록·실측치는 Story 15.1 하단 참조.
...
_✎ 2026-08-08 — 이 한 줄 요약은 착수 시점(스토리 16.1~16.6)의 범위다. 이후 **웹에 있는데 앱 짝이 없어 빠져 있던 화면 2개**를 사용자 결정으로 추가했다: 16.7 앱 사진 업로더(2026-08-07, DW-352), 16.8 앱 홈 랜딩 미러(2026-08-08, DW-736). 둘 다 "어느 스토리도 자기 것으로 안 가진 자리"였다 — 요약만 읽고 범위를 판단하지 말고 아래 Story 목록을 본다._
...
**Given** wishlists 마이그레이션 — ⚠️ **번호는 착수 시 `supabase/migrations/` 파일 목록에서 다음 빈 번호로 정한다.** 여기 적혀 있던 `0014`는 **이미 소진됐다**(0014=공개 버킷 전환, 0015=sold 수정 차단). *(✎ 2026-07-21 코드리뷰: 정정이 `:284` 각주에만 있고 본문은 `0014` 그대로여서, 본문만 읽은 dev가 존재하는 번호로 파일을 만들 자리였다 — B8 "각주의 제약은 본문으로 올린다".)*
**When** 적용하면
...
**추가 인수조건 (대장 이월분 — Story 11-2 코드리뷰 2026-07-28이 심음, B5):**
- [ ] **`#152` 비로그인 홈(`/`)에 상단 내비를 렌더한다.** **Given** 비로그인 사용자가 홈(`/`)을 열면, **when** 상단을 보면, **then** 로고·내 차 사기·AI로 찾기·내 차 팔기·로그인·내 차 등록이 `/search`와 **동일하게** 보인다.
  - ⚠️ **지금은 홈에서만 안 보인다** — `web/src/app/page.tsx`의 **로그인 분기에만** `<AppHeader>` 호출이 있고 비로그인 분기는 `<main>`만 반환한다(11-2 리뷰가 호출부 10곳 전수 확인). Story 11.2의 AC1("비로그인 사용자가 아무 공개 페이지를 열면 …")이 **홈에서만 미충족**인 상태다.
  - 11.2가 안 고친 이유: 그 스토리 intent의 Never가 `app/page.tsx` 수정을 명시적으로 금지했다(비로그인 홈은 이 스토리가 통째로 다시 그릴 화면이라 헤더만 얹으면 곧 덮어쓰므로).
...
**추가 인수조건 (대장 이월분 — Story 11-1 코드리뷰 2026-07-27~28이 심음, B5):**
- [ ] **`#134` `anon`에게 `view_count` SELECT를 부여한다.** 새 마이그레이션에 `grant select (view_count) on public.listings to anon;` 한 줄. **이게 없으면 이 스토리가 통째로 안 돈다** — `0011`이 anon의 테이블 SELECT를 회수하고 컬럼 화이트리스트로 되돌리는 구조라 `view_count`는 anon에게 안 보이고, Postgres는 `ORDER BY` 대상 컬럼에도 SELECT 권한을 요구하므로 `order by view_count desc`가 `ERROR: permission denied for table listings`로 죽는다(11-1 리뷰가 스크래치 PG18에 마이그 20개를 얹어 실측).
  - ⚠️ **로그인 상태에선 멀쩡히 돈다** — `authenticated`는 테이블 SELECT를 그대로 갖고 있어서다. **로그아웃해야만 깨지는** 비대칭이라 개발 중 가장 놓치기 쉽다. **비로그인으로 랜딩을 여는 검증을 반드시 포함할 것.**
  - 11-1이 안 고친 이유: 그 스토리 intent의 Never가 "실제 노출은 11.4의 몫"으로 GRANT 추가를 명시적으로 금지했다.
- [ ] **`#144` 인기 정렬용 인덱스를 같은 마이그레이션에 함께 넣는다.** `create index if not exists listings_view_count_idx on public.listings (view_count desc) where status = 'on_sale';` — 정렬 대상 컬럼에 인덱스가 없는 채로 인기 그리드가 나간다.
...
**추가 인수조건 (대장 이월분 — 코드리뷰 2026-07-21이 심음, B5):**
- [ ] **`#73` 전면 장애 폴백을 세 소비처에서 각각 재현한다.** 이미지 요청(`**/storage/v1/object/public/**`)만 abort하고 데이터 조회는 살린 채 **목록(`/search`)·상세·`/ai`** 를 각각 관찰한다. Story 9.7은 **목록만** 쟀는데 #73 본문은 *"해소는 목록·상세를 함께 다룬다"* 고 못박았고, 상세 갤러리(`ListingGallery.tsx`)는 별도 구현이라 덮이지 않는다.
  - ⚠️ **`loading="lazy"` 때문에 화면 밖 카드는 요청 자체를 안 보낸다** — 9.7에서 90장 중 38장만 폴백한 이유가 이것이다. 스크롤로 전량을 뷰포트에 넣거나 lazy를 끄고 재야 "전량 검증"이라 말할 수 있다.
- [ ] **`#86` 검증 산출물을 재실행 가능한 형태로 남긴다.** 9.6·9.7의 스크린샷·네트워크 로그가 `.gitignore` 대상이라 레포에 없고, 수치를 나중에 대조할 수단이 없다.

**추가 인수조건 (대장 이월분 — Story 11-2 코드리뷰 2026-07-28이 심음, B5):**
- [ ] **`#160` 상단 내비의 상호작용을 E2E로 고정한다.** 이 스토리가 web에 세우는 E2E 층에 다음 4가지를 포함한다 — (a) 390px에서 햄버거 열기 → 패널 **바깥 클릭**으로 닫힘, (b) Esc로 닫히고 **포커스가 트리거로 복귀**, (c) 프로필▾을 연 상태에서 햄버거를 열면 **프로필이 닫힘**(상호배타), (d) 390px에서 연 뒤 1280px로 리사이즈하면 패널이 DOM에서 사라지고 `aria-expanded="false"`가 된다.
  - ⚠️ **네 가지 전부 Story 11.2가 실측으로 한 번 잡아 고친 결함이다.** 그런데 재발 방지선이 없다 — 11-2 3차 리뷰가 `SiteNav.tsx`의 상호배타 한 줄을 지우고 `lint`·`tsc`·`vitest`·`build`를 돌려 **전부 green**임을 확인했다. `SiteNav.test.ts`는 SSR 렌더 계약까지만 고정하고 `useEffect`는 SSR에서 실행되지 않으므로 **이 층은 구조적으로 E2E 몫**이다.
  - 같은 E2E 층을 기다리는 항목: **`#127`**(폰트 CDN 재유입·용량 검사) · **`#86`**(위) — 한 자리에서 함께 답한다.
...
**Given** 가이드 문서 코퍼스(활성 10문서, 전체 12개 중 2개는 `_excluded/` — 13.9 DW-620 정정)
**When** 질의를 처리하면
...
**And** 청킹은 도입하지 않는다(현 활성 10문서=page-level, FR48 조건부 — ≥20문서/≥800토큰 시에만)
...
### Story 13.9: 라우팅 안정화 (최상급·교체요청·맥락 재작성)

> **선행 조건**: 13.8(에픽 종료 검증) **이후**에 착수한다. 이 스토리는 라우팅 동작을 바꾸므로
> 2026-08-02에 찍은 기준선(`api/docs/g2-baseline.json`)이 무엇을 기준으로 찍혔는지 흐려진다 —
> 13.8이 그 기준선으로 판정을 마친 뒤에 손대고, 완료 시 기준선을 재캡처한다.
> 점수는 **올라가는 방향**이라 13.8의 회귀 게이트를 막지 않는다(DW-611·612 근거).

As a AI 검색 사용자,
I want 조건을 분명히 말했는데 되묻기로 되돌아오지 않길,
So that 답할 수 있는 질문에 답을 받고, 대화 중 조건이 사라지지 않는다.

**배경(2026-08-02 재설계 기준선에서 실측된 오답 3턴)**: 라우터는 갈래를 "명시조건 유무 + 용도·느낌 유무"로만
가르는데, **최상급 표현("제일 싼"·"가장 비싼")을 조건으로 세지 않는다.** 실측: `제일 싼 차 뭐야?`·
`제일 싼 차 보여줘`·`제일 싼 차 알려줘`·`가장 비싼 차 뭐야?`가 전부 CLARIFY이고(동사와 무관),
`제일 싼 SUV 보여줘`는 SQL이다 — 즉 최상급 외 다른 구체 조건이 있어야만 검색으로 간다.
별개로 `SUV로 바꿔줘`·`아반떼 같은 준중형 보여줘`는 SQL인데 `쏘렌토 같은 SUV로 바꿔줘`만 CLARIFY라
**같은 형태가 조합에 따라 갈리는 불안정**이 있다. 또 `contextualize_query`가 REFINE 턴을 재작성 없이
그대로 반환해 앞선 조건이 독립 질의로 접히지 않는 사례를 확인했다(M1 3턴째).

**Acceptance Criteria:**

**Given** 라우터 노드와 맥락 재작성 노드
**When** 재설계된 질의셋(47항목/57턴, 2026-08-02)을 전량 재캡처하면
**Then** **최상급·정렬 표현이 구조조건으로 취급**돼 `S6("제일 싼 차 뭐야?")`·`M1.t2("제일 싼 거 하나만 알려줘")`가 SQL로 라우팅된다(DW-611)
**And** **명시 차종이 든 교체 요청**이 되묻기로 새지 않는다 — `M6.t1("아니 쏘렌토 같은 SUV로 바꿔줘")`는 SQL이다(DW-612).
  ⚠️ **사분면 귀속 확정(DW-617 #1)**: "교체 요청은 SQL"은 **무조건이 아니다.** 13.2의 4분기 계약이 상위이며, 교체 요청이라도 **의미·용도 조건이 섞이면 HYBRID**다(예: `"가족이 타기 좋은 SUV로 바꿔줘"` → HYBRID). 즉 이 인수조건이 요구하는 것은 "교체 요청을 SQL로 보내라"가 아니라 **"교체 요청이라는 이유로 CLARIFY로 떨어지지 않게 하라"**이다 — 조건이 명시돼 있으면 SQL, 거기에 느낌이 섞이면 HYBRID. 이 문장이 없으면 구현자가 13.2 계약과 이 AC 중 하나를 반드시 어긴다
**And** `M6`의 RESET 오염 게이트가 **실제로 발화 가능한 상태**임을 확인한다 — 매물 0건이라 자동 통과하던 상태에서 벗어났는지를 "일부러 오염 데이터를 넣으면 red가 되는지"까지 돌려 확인한다(존재 확인 ≠ 작동 확인, B4)
**And** `contextualize_query`가 REFINE 턴을 **혼자서도 말이 되는 독립 질의로 재작성**한다 — `제일 싼 거 하나만 알려줘` + (3천만원·SUV·서울 맥락) → 조건이 접힌 질의. 재작성 실패 시 원 질의로 되돌리는 기존 방어선은 유지한다
**And** 라우팅 정답이 **54/57 → 57/57**이 되고, 결과집합 정확도(0.894)·가이드 인용(12/13)·되묻기 발동(9/9)·오염 0·dead-end 0이 **모두 하락하지 않는다**
**And** 라우팅 규칙 중 **LLM을 거치지 않고 코드가 판정하는 부분**(예: `_fallback_route`의 결정론 보정, 최상급·교체요청 표현의 전처리·정규화)은 결정론적 단위테스트로 고정한다.
  ⚠️ **"단위테스트가 프롬프트 흔들림을 먼저 잡는다"는 요건은 삭제한다(DW-614 #6)** — 이 저장소의 api 단위테스트는 LLM을 fake로 갈아끼우는 것이 표준이라, fake 응답은 프롬프트를 바꿔도 반응하지 않는다. **정의상 발동할 수 없는 검사**를 요구하는 문장이었다. 프롬프트 흔들림을 잡는 것은 단위테스트가 아니라 **라이브 재캡처(G2)**이며, 그 사실을 스펙에 명시한다
**And** **G2는 두 단계로 판정한다(DW-614 #1 — 이 순서가 뒤집히면 게이트가 무의미해진다)**:
  1. **판정**: 13.9 **이전** 기준선(2026-08-02 캡처, `api/docs/g2-baseline.json`)을 그대로 두고 변경 후 코드로 재캡처해 **그것과 비교**한다. 네 축(결과집합·라우팅·가이드인용·되묻기) 모두 하락이 없어야 한다
  2. **갱신**: 1이 통과한 **뒤에만** 기준선을 새 캡처로 교체한다
  ⚠️ 초안은 "완료 시 재캡처해 갱신하고 그 새 기준선으로 게이트를 재실행"이라고 적었는데, 그러면 **변경 후 코드로 만든 기준선과 변경 후 코드를 비교**하게 되어 회귀가 얼마든 나도 항상 통과한다 — 에픽의 유일한 회귀 게이트가 하필 그걸 가장 깨기 쉬운 스토리에서 무력화된다
**And** 1단계에서 **불합격하면** 기준선을 갱신하지 않고 중단한다(DW-613 #6) — 원인을 스펙에 기록하고 재작업하거나, 재작업이 불가하면 에픽 종료를 보류한다. "하락했지만 의도된 변경"이라는 판단은 **사용자 확인 없이 하지 않는다**
**And** RESET 오염 검사에 **일부러 넣은 오염 데이터의 정리(teardown)**가 포함된다(DW-614 #7) — 같은 에픽이 공용 로컬 DB에서 G2 기준선을 재캡처하므로, 잔존 행이 그 캡처에 섞이면 위 판정 자체가 오염된다

**추가 인수조건 — 13.7·13.8 후속 리뷰가 이 스토리로 넘긴 것(2026-08-03):**

**Given** G2 회귀 게이트와 CM-B(FR11) 강제지점 검사
**When** 이 스토리의 변경을 검증하면
**Then** **게이트가 네 축 전부를 기준선과 자동 대조**한다(DW-626) — 지금은 `result_mean` 한 축만 코드가 비교하고 `routing_correct`·`doc_hit_n`·`clarify_ok_n`은 사람이 눈으로 대조해야 한다. 즉 13.4(되묻기)·13.6(가이드 활용) 기능이 전멸해도 게이트는 초록이다
**And** **라이브 검증이 스킵되면 게이트가 실패**한다(DW-630·622) — 지금은 SM-F/SM-G 라이브 스모크가 전부 스킵돼도 종료코드 0이라 "확인 안 함"과 "확인해서 통과"가 구별되지 않는다
**And** **`doc_rag_node`의 FR11 판매완료 필터가 실DB로 검증**된다(DW-627) — 이 공백은 뮤테이션으로 실증됐다: 그 필터를 무력화해도 CM-B 전량이 초록이다. 4개 강제지점 중 실제 Postgres에 붙는 것은 이미지 축 1곳뿐이고 나머지 3곳은 "생성된 SQL 문자열에 `status='on_sale'`이 들어 있나"만 본다
**And** FR11 강제지점 로스터를 임의 목록이 아니라 `docs/conventions.md` §6에서 가져오고, §6에 누락된 `hybrid_rag_node`를 등록한다(DW-619·625)
**And** `ai-demo-queries.md`의 회색지대 서술(①②④ 표 포함)이 실제 라우팅과 일치한다(DW-632) — 서두는 "회색지대는 CLARIFY"라고 못박는데 같은 문서 표 3행 중 2행은 CLARIFY가 오답이고, 이 문서가 라우터의 단일출처다
**And** 계획 문서의 "가이드 문서 12개"를 활성 코퍼스 실제 개수(10개)와 맞춘다(DW-620)
**And** 라이브 단언 로직을 순수 함수로 분리해 결정론 검사로 고정한다(DW-641) — 지금은 테스트 본문에 인라인이라 "노드 계측이 죽으면 red가 되는가"를 확인하려면 사람이 손으로 돌연변이를 만들어야 하고, 실제로 3차·4차 리뷰가 각각 손으로 했다

> **이 스토리가 새로 넣거나 고치는 모든 검사는 양방향으로 깨서 red를 확인한다.**
> 13.8 4차 리뷰가 찾은 high 2건이 정확히 "한 방향으로만 깨보고 red를 확인했다고 기록한" 검사였다 —
> 대조 검사는 **양쪽이 같이 줄어드는 방향**도, 파괴적 검사는 **지키려던 대상이 안 다치는지**도 함께 본다.

**Design Notes(구조 대안 — 결정을 기록할 것):**
현재 구조는 **"먼저 갈래를 정하고 그 다음 조건을 뽑는다"**인데, 갈래 판정을 LLM 감에 맡기므로
프롬프트가 다루지 않은 질의 형태에서 표면 표현에 흔들린다(위 실측이 그 증상이다).
대안은 **"조건을 먼저 뽑고, 뽑힌 결과로 코드가 갈래를 정한다"**로, 분기 판단이 LLM이 아니라
코드로 내려와 결정론이 된다. 이 패턴은 이미 `hybrid_rag_node`에 반쯤 존재한다(구조조건 `NONE`이면
벡터검색으로 폴백). 이 스토리에서 **프롬프트 보강으로 끝낼지, 추출-후-분기로 구조를 바꿀지 판단해
근거와 함께 기록**한다 — 프롬프트 보강만 하면 다음 미지 형태에서 같은 종류의 흔들림이 재발할 수
있다는 점을 감안한다.

⚠️ **대안을 택할 경우 초안의 "추출 실패 = 되묻기" 한 줄 규칙은 그대로 쓰면 안 된다**:
- **REJECT가 붕괴한다(DW-614 #2)** — 날씨·잡담·금융 같은 무관 질의는 **정의상 구조조건이 0개**라
  그 규칙에서 CLARIFY로 떨어진다. 13.2의 4분기 인수조건과 `test_live_smoke_pathC`(route==REJECT)를
  동시에 깬다. 따라서 **REJECT 판정은 추출 이전의 별도 1단계로 남겨야** 한다(도메인 밖 여부는
  조건 개수로 판별할 수 있는 것이 아니다).
- **SQL/HYBRID 경계가 빈다(DW-614 #3)** — "추출 실패=되묻기"는 한 갈래만 정의하고, 추출 성공
  이후 두 갈래를 가르는 기준이 없다. 의미·용도 표현의 잔존 여부로 가른다면 그 판정을 무엇이
  하는지(코드인지 LLM인지)까지 정해야 4분기가 닫힌다.

⚠️ **최상급 × HYBRID의 정렬 표현 문제(DW-614 #4, 실물 확인됨)**: 최상급을 구조조건으로 취급하기로
해도, 의미조건이 섞이면 경로는 HYBRID이고 그 경로의 `ORDER BY`는 **벡터 거리절이 이미 점유**한다.
`api/app/db/sql_guard.py`(L266~275, DW-555 근거 주석)가 2차 정렬키(`... ::vector, price`)를 **명시적으로
차단**하므로, `"제일 싼 패밀리카"` 류에서 최상급이 조용히 버려질 수 있다. 이 스토리는 그 동작을
**정하고 기록**해야 한다 — 후보: (a) 최상급+의미조건이면 SQL로 보내 정렬을 살린다, (b) HYBRID를
유지하고 최상급을 버리되 그 사실을 답변에 알린다, (c) 가드의 2차 정렬키 차단을 재검토한다.
(c)는 보안 결정이라 이 스토리 단독으로 바꾸지 않는다.

단, 구조 변경 시 4갈래 라우팅(13.2)의 인수조건과 `RouterDecision`·`_fallback_route`·`_route_decision`
3곳 락스텝 계약을 깨지 않아야 한다.

**착수·종료 조건(DW-613 #5·DW-614 #5):**
- **착수**: 13.8이 "스토리 done"이 된 시점이 아니라 **13.8의 G2 게이트가 통과한 뒤**다(둘 다 충족됨 — 2026-08-02).
- **종료**: 13.8이 에픽의 exit-gate였는데 13.9가 그 뒤에 배치됐으므로, **에픽 13의 종료 판정은 이제
  "13.9의 위 G2 2단계 판정 통과 + 13.9 인수조건 충족"**이다. 13.8의 판정은 그 시점 코드에 대한
  것으로 유효하되, 13.9가 라우팅을 바꾸므로 **재판정 없이 에픽을 닫지 않는다.**

**이 스토리가 다루지 않는 인접 케이스(DW-613 — 실측 없이 발견돼 "후보"로만 남김):**
주관적 최상급(`"제일 좋은 차"` — 정렬 가능한 컬럼이 없음) · 차종 미명시 교체(`"다른 거 보여줘"`) ·
RESET 게이트의 오탐 방향(정상 세션을 오염으로 잘못 잡는 경우) · `contextualize_query`가 성공하지만
**잘못** 접어넣는 경우의 폴백 · 직전 턴이 CLARIFY/REJECT라 이어받을 조건이 0개인 상태 ·
같은 축을 다시 명시할 때(`"아니 5천만원대로"`) 누적인지 대체인지(DW-617 #2).
→ 스펙 작성 시 **인수조건으로 승격할지 명시적으로 판단**하고, 승격하지 않는 것은 그 이유를 적는다.
말없이 빠뜨리지 않는다.
...
**And** ⭐ **에픽 14의 최종 인수 조건(2026-08-06 사용자 명시)** — **세 종류의 계정이 전부 `/sell`에 도달한다**: ①기존 `role='buyer'` 계정 ②기존 `role='seller'` 계정 ③이 스토리 이후 새로 가입한 계정. 실제 브라우저로 세 계정 각각 로그인해 `/sell`이 열리는 것을 확인한다(홈으로 튕기지 않음). 이 스토리가 에픽 14에서 **마지막에 실행**되므로 여기가 에픽 전체의 최종 확인 자리다
> ⚠️ 이 조건이 왜 붙었나: 14.2를 14.3보다 먼저 돌렸을 때 *"기본 role은 `'user'`인데 판매 게이트는 `'seller'`만 통과"* 라는 **스펙 내부 모순**이 드러나 dev 세션이 CRITICAL 에스컬레이션으로 멈췄다(run 0381). 사용자가 *"중간에 깨지는 건 상관없고 최종적으로 셋 다 판매 페이지로 갈 수만 있으면 된다"* 로 정리해, **실행 순서를 14.3 → 14.2로 뒤집고**(장부 `sprint-status.yaml`에 근거 기록) 그 최종 상태를 여기 검증 조건으로 심는다. 회고나 별도 문서에 두면 이행되지 않으므로 인수조건으로 내린다(CLAUDE.md B5).
...
**➕ 추가 범위 — 소비자 잔여 화면 리스킨 (사용자 결정 2026-07-29, 대장 `DW-546`)**

FR37은 *"**전 화면**에 일관된 디자인 시스템"* 인데, Epic 8은 **토큰·프리미티브를 만드는** 에픽이었고
화면 적용은 각 에픽이 자기 화면을 건드릴 때 따라왔다(Epic 9=카드·상세, Epic 11=랜딩·내비,
Epic 15=관리자). 그래서 **아래 화면들은 어느 에픽의 화면 목록에도 들어가지 않아** 옛 원시 색
(`zinc-*`)으로 남았다 — 미룬 게 아니라 아무도 자기 일로 안 가져간 것이다(사용자가 로그인 화면에서 발견).
관리자 리스킨과 **같은 작업**(토큰 치환 + D5 반응형 확인)이므로 세팅을 두 번 하지 않도록 여기 붙인다.

**Given** 아래 소비자 화면과 디자인 토큰(8.1)
**When** 리스킨하면
**Then** 관리자 6화면과 **동일한 기준**(@theme 토큰·카드/뱃지/버튼/타이포 위계)으로 통일된다(FR37)
**And** 대상 화면 — 2026-07-29 실측(`zinc-*` 원시 클래스 수 / 토큰 사용 수):
  `search/SearchFilters`(24/0) · `sell/PhotoUploader`(22/0) · `chat/[roomId]/ChatRoomMessages`(20/0) ·
  `chat/[roomId]/page`(16/0) · `sell/[id]/edit/page`(9/0) · `chat/page`(9/0) · `(auth)/signup/page`(9/0) ·
  `(auth)/login/page`(9/0) · `sell/page`(8/0) · `app/page`(4/0, 잔존분)
  *(대조군 — 이미 리스킨된 화면: `listings/[id]`·`wishlist`·`account`·`OptionPicker` = zinc 0)*
**And** **신규 기능은 추가하지 않는다**(UI-only, 관리자 축과 동일 제약)
**And** `(auth)/login`·`(auth)/signup`에는 이미 상단바가 붙어 있다(`DW-547`, 2026-07-29 — "갇힘"만 먼저
푼 것) — 리스킨은 **본문 스타일만** 다루고 그 레이아웃을 되돌리지 않는다
**And** 리스킨 후 `zinc-*` 잔존 수를 다시 세어 0인지 확인한다(선언이 아니라 측정으로 닫는다)
...
> ⚠️ **이 에픽의 "UI-only" 범위를 한 칸 넘는 스토리다** — 마이그레이션 1개(복구 전용 RPC)가 필요하다.
> 관리자 화면 기능이라 여기 두지만, 착수 시 이 예외를 스펙에 명시할 것.

As a 관리자,
I want 실수로 '판매완료'가 된 매물을 화면에서 되돌리길,
So that 운영자가 DB에 직접 SQL을 치지 않아도 복구된다.

**배경(DW-391, severity high)**: `0015_listings_update_not_sold.sql`이 UPDATE 정책의 `using`에
`status <> 'sold'`를 넣으면서 **판매자도 관리자도 sold 행을 되돌릴 수 없다.** `0005`는 관리자
**DELETE** 정책만 만들었고 UPDATE는 *"현재 관리 요구사항에 없어 추가하지 않는다"*로 비워뒀다.
마지막 수단인 `service_role`은 프로젝트 규칙상 금지다. **가정이 아니라 이미 한 번 밟았다** —
Story 9.7이 검증 중 매물을 sold로 바꿨다가 되돌리지 못해 DB에 직접 SQL을 쳐서 복구했다.
2026-07-21에 "런북 SQL로 갈음"으로 결정했던 사안을 2026-08-02 사용자 판단으로 뒤집는다.

**Acceptance Criteria:**

**Given** 관리자(`profiles.role='admin'`)로 로그인한 상태
**When** 관리자 매물 화면에서 sold 매물의 '판매완료 되돌리기'를 실행하면
**Then** 그 매물의 `status`가 `on_sale`로 돌아오고 목록에 다시 노출된다
**And** 복구 경로는 **`status`만 되돌리는 좁은 RPC**다 — 관리자 UPDATE 정책을 통째로 여는 방식(`listings_update_admin`)은 **쓰지 않는다**. 그 방식은 `0015`가 막으려던 "sold 매물 임의 수정"을 반쯤 되열기 때문이다(DW-391이 기록한 선택지 ①의 대가)
**And** RPC는 `security definer`이며 **내부에서 `is_admin()`을 확인**한다 — 비관리자가 직접 호출하면 거부된다
**And** RPC는 `status='sold'`인 행에만 동작한다(이미 on_sale인 행에 호출해도 아무 일이 없다 — 멱등)
**And** 비관리자(일반 회원·판매 본인·비로그인)가 이 RPC를 호출하면 실패하는 것을 **실제 데이터로 확인**한다 — 정책이 "있다"가 아니라 실제로 "거른다"를 본다(권한과 필터 정책은 별개)
**And** 되돌린 뒤 판매자가 그 매물을 다시 정상 수정·판매완료할 수 있다(복구가 반쪽이 아님)
**And** 관리자 화면의 기존 패턴을 따른다 — 회원관리 정지/해제 토글(`MemberActions.tsx`)이 쓰는 "실행 → 0행이면 한국어 오류 → `router.refresh()`" 흐름과 같은 모양으로 만든다
**And** 복구 사실이 추적 가능해야 한다 — 최소한 런북(`docs/` 운영 문서)의 "직접 SQL로 복구" 항목을 이 화면 절차로 대체하고, 옛 SQL 절차는 **비상용 백업으로만** 남긴다
...
_✎ 2026-08-07 추가된 인수조건 2건 — 둘 다 원래 `16-0`(앱 역할 통합, 2026-08-06 완료)을 트리거로
지목했다가 **그 스토리가 지나가도록 이행되지 않은** 항목이다. 대장에만 두면 또 밀리므로 여기 심는다(B5)._

**And** `currentRoleProvider`가 role 없는 세션에서 무엇을 돌려주는지 **`app/test/`가 단언한다**(DW-686) — 지금은 `grep -rn 'currentRoleProvider' app/test/`가 **0건**인데 `main.dart`의 admin 차단이 그 값을 쓴다. 즉 쓰이는데 계약이 안 잡혀 있다. 채택 전에 red를 확인할 것
**And** role 어휘의 정본이 **`docs/conventions.md`에 한 절로 존재한다**(DW-681) — 지금 정본은 `0029`가 붙인 **DB 컬럼 주석에만** 있다(*"admin만 특별 취급, 그 외는 전부 'user'이며 구매/판매 구분이 없다. 매물 접근 권한은 이 값이 아니라 소유권(seller_id)+RLS로 판정한다"*). 이 스토리가 하단 4탭·admin 차단을 손대며 그 어휘를 실제로 쓰므로, 같은 자리에서 문서로 올린다
...
### Story 16.9: 앱 UI 정합성 교정 (스파인 기준)

_✎ 2026-08-09 신설 — 사용자가 실기기 화면을 보고 *"목업이랑 딱 봐도 다르다, 전체적으로 완성도가 떨어지는 느낌"*이라고 지적한 데서 출발했다.
목업·웹 구현·앱 코드·**시각 스파인(`DESIGN.md`)** 넷을 대조해 원인을 특정했고, 그 결과가 DW-755·DW-759·DW-760·DW-735다.
**정본은 목업이 아니라 스파인이다** — `DESIGN.md`가 *"이 스파인이 목업과 충돌하면 스파인이 우선한다. 목업(`mockups/`)은 시각 참고이지 구속 스펙이 아니다"*라고 명시한다.
네 항목이 전부 **홈 화면 최상단 한 덩어리**라 따로 고치면 중간 상태가 어색해지므로 한 스토리로 묶는다.
공통 원인은 **검사가 보는 축과 사람이 보는 축이 다른 것**이다 — 기존 검사는 "필드가 있나·순서가 맞나"만 보고 폭·색·크기·요소 유무를 안 봐서 전 검사 green으로 통과했다._

As a 데모를 보는 사람,
I want 앱 화면이 확정된 시각 스파인대로 보이길,
So that 웹·앱이 같은 제품으로 보이고 시연에서 완성도가 떨어져 보이지 않는다.

**Acceptance Criteria:**

**Given** `DESIGN.md`(시각 스파인) · `EXPERIENCE.md`(앱 내비 D12) · `mockups/consistency-1.html` PART 3(앱 프레임)
**When** 앱 홈을 열면
**Then** **상태바 → 앱바 → AI 히어로가 이음새 없이 하나의 딥 petrol 면**으로 흐른다(DW-755·DW-759) — 지금은 앱바가 흰색이고 히어로가 사방 여백 있는 카드로 떠 있다
**And** 히어로 배경에 **글로우/메시 + 대형 차 실루엣 라인아트**(우상단, 옅게, 칩·검색창을 침범하지 않음)가 들어간다 — 스파인이 *"'허전함' 해소 + 자동차 정체성"* 목적으로 **명시적으로 요구**한 요소이며 지금은 둘 다 없다
**And** 앱바에 **로고 lockup**(petrol 라운드-스퀘어 배지 + Pretendard 800 "차" + "차장님" 워드마크)이 들어가고 우측은 **아바타 하나**다(DW-759) — 알림 벨은 넣지 않는다(스파인·EXPERIENCE에 없고 푸시는 에픽 Non-goal이라 누를 곳이 없는 버튼이 된다). 실제 로고 아트워크는 스파인이 *"추후 제작"*으로 남겨둔 별개 사안이라 **이 스토리 범위가 아니다**
**And** 히어로 바로 아래의 `어떤 차를 찾고 있나요?` 탐색 진입 박스가 **제거**되어 히어로 다음이 차종 칩이다(DW-755, 사용자 결정 2026-08-09) — 히어로 교정과 **같은 커밋**이어야 한다(먼저 지우면 중간 상태가 커밋으로 남는다)
**And** 차종 칩의 `전체`가 **선택 상태(petrol 채움)** 로 보인다(앱 목업 기준. 웹 구현에는 선택 상태가 없으나 이 스토리는 **앱만** 손댄다 — 웹 불일치는 별도 판단)

**And** 매물 카드가 웹 정본(`ListingCard.tsx`)·`card-final-1.html`·스파인 §매물 카드와 같아진다(DW-760) — ① **가격이 카드에서 시각적으로 가장 큰 요소**(26/800 상당) + **가격 전용 강조색**(`price-emphasis` #E08A1B 계열), 지금은 15px 잉크 검정이라 차량명과 무게가 같다 ② **희소옵션 칩**(petrol 아웃라인 3~4개, 넘치면 "외 N")이 생기고 **옵션이 없어도 슬롯을 예약**해 카드 높이가 흔들리지 않는다 ③ **신뢰속성 행이 사진 위 오버레이가 아니라 사진 바로 아래 전용 행**(초록 칩 + "판매자 제공 정보" 면책 muted 11px)이다 ④ 정보 순서 = 사진 → 신뢰속성 → 차량명 → meta(주행·연료·지역·판매자) → 가격 → 옵션 칩
**And** **카드 크기·열 수는 바꾸지 않는다**(Never) — 스파인 D5가 4→2→1 열 축소를 규정하고 브레이크포인트가 1100/640px라 390px는 1열이 맞다. `ai-flow-1.html`의 2열 모바일 프레임은 목업 변형이라 따르지 않는다

**And** 매물 상세에 **하단 고정 바(판매가 + 문의하기)** 가 생긴다(DW-735, `detail-1.html` 모바일 프레임 + 스파인 "떠 있는 요소: sticky 문의 바") — 지금은 없어서 스크롤 중 CTA를 놓친다

**And** ⚠️ **검사가 이 축들을 실제로 본다** — 위 항목마다 위젯 테스트로 단언한다: 앱바·히어로가 같은 배경색으로 연속인가 · 히어로가 화면 폭을 꽉 채우는가(좌우 여백 0) · 히어로 다음 위젯이 차종 칩인가(탐색 박스 0개) · 카드에서 가격 글자 크기가 차량명보다 큰가 · 옵션 칩 슬롯이 옵션 0개일 때도 존재하는가 · 신뢰속성이 사진 위젯 **뒤**(아래)에 오는가. **채택 전 일부러 깨서 red를 확인**한다(CLAUDE.md B4) — 기존 검사가 순서·개수만 봐서 이 갭들을 전부 놓친 것이 이 스토리의 존재 이유다

**Non-goals:** 웹 화면 수정(웹의 차종 칩 선택 상태 불일치 포함 — 별도 판단) · 실제 로고 아트워크 제작 · 카드 열 수/크기 변경 · 알림 벨·푸시 · AI 결과 그리드를 2열로 바꾸는 것

### Story 16.10: 히어로·검색창 마감 (타이포·실루엣·곡률)

_✎ 2026-08-09 신설 — 16.9가 **구조**(한 면·폭·로고·칩·카드)를 맞춘 뒤 사용자가 실기기를 보고 *"퀄리티가 좀 떨어지네"*라고 한 데서 출발했다.
원인을 목업 CSS·앱 코드의 **수치로** 특정했다: 히어로 헤드라인이 목업의 절반 크기(19px vs 30~34px)이고 `AI 매물 검색` 라벨이 없다(DW-767).
여기에 사용자가 육안으로 지목한 3건(검색창 테두리·차 그림·곡률)을 더해 **같은 화면 한 덩어리**로 묶는다.
16.9가 이걸 못 잡은 이유는 **인수조건에 타이포 축이 없었기 때문**이다 — 근거로 삼은 대장 항목들이 전부 "레이아웃·색" 언어였다._

As a 데모를 보는 사람,
I want 히어로가 목업만큼 또렷하고 마감돼 보이길,
So that 시연에서 "데모앱 같다"는 인상을 주지 않는다.

**Acceptance Criteria:**

**Given** `DESIGN.md`(스파인) · `mockups/consistency-1.html`(앱 프레임 · `.silhouette`·`.eyebrow`·`.hero h2`) · `mockups/app-home-2.html`
**When** 앱 홈을 열면
**Then** **히어로 헤드라인이 지금(19px)보다 확연히 크고 2줄로 놓인다**(목업 앱 프레임 기준 30~34px 상당, "원하는 차를 / **말**로 찾으세요") — 히어로는 스파인이 말한 *"단 하나의 몰입 순간"*이라 이 화면에서 가장 큰 글자여야 한다
**And** 헤드라인 위에 **`AI 매물 검색` eyebrow 라벨**이 놓인다(목업 `.eyebrow` = 반투명 배경 + 얇은 테두리 + 작은 점 + 자간 넓힌 소문자 톤)
**And** **히어로 검색창의 테두리가 사라진다** — 원인은 `app_theme.dart`의 `InputDecorationTheme`이 `enabledBorder`·`focusedBorder`를 정의하는데 히어로 `TextField`는 `border: InputBorder.none`만 껐기 때문이다(`border`는 나머지 둘을 덮지 않는다, 2026-08-09 코드 실측). 웹 `HeroSearch.tsx`는 `rounded-full` + 그림자만 쓰고 테두리가 없으므로 **웹과 맞추는 방향**이다. ⚠️ 테마를 통째로 바꾸지 말고 **이 검색창에서만** 끈다 — 테마는 로그인·등록 폼이 함께 쓴다
**And** **차 실루엣이 머티리얼 아이콘이 아니라 목업의 라인아트로 바뀐다**(사용자 결정 2026-08-09 = ①안 "목업 SVG로 교체"). 지금은 `Icons.directions_car_filled`(꽉 찬 작은 아이콘)라 목업의 `.silhouette`(폭 58%·`opacity .10`·우하단으로 흘러나감)과 크기·질감이 전혀 다르다. **앱에 SVG 렌더 의존성이 없으므로**(`pubspec.yaml`에 `flutter_svg` 없음) ⓐ`flutter_svg` 추가 ⓑ경로를 `CustomPainter`/`Path`로 옮기기 중 **하나를 골라 근거를 남긴다** — 새 의존성을 들이는 판단이므로 조용히 넣지 않는다
**And** ⚠️ **웹 히어로에도 같은 실루엣이 들어간다**(사용자가 "통일성"을 이유로 ①안을 골랐고, 실측 결과 **웹 히어로에도 차 실루엣이 없다** — `HeroSearch.tsx`의 svg는 검색 아이콘뿐이다). 이 에픽의 Non-goals는 "웹 랜딩 **재작업**"인데 이건 재작업이 아니라 스파인이 요구한 배경 요소를 넣는 것이고 사용자가 명시 결정했다 — **같은 에셋·같은 배치 규칙**을 쓰되 웹 변경은 이 배경 요소 하나로 한정한다
**And** **히어로 아래 모서리 곡률이 줄어든다**(지금 22 → 14~16 계열). 값 자체는 `app-home-2`와 같지만 실기기에서 곡선이 과해 *"데모앱 같다"*는 인상을 준다(사용자 육안 평가). 함께 **카드 radius가 웹 토큰(16)과 같은지 확인**해 어긋나면 맞춘다
**And** **웹을 폰 폭(390px)으로 띄워 앱과 나란히 대조**하고 남은 편차를 대장에 등재한다 — ⚠️ 비교 대상은 **로컬 또는 develop 배포본**이다. 운영(`bmad-encar-demo.vercel.app`)은 `main` 반영을 사용자가 보류해 **Epic 11 이전 화면**이므로 비교하면 안 된다(2026-08-09 실측·사용자 확인)
**And** **검사가 이 축들을 본다** — ①헤드라인 글자 크기가 카드 차량명보다 **확연히 크다**를 **관계로** 단언한다(절대값을 박으면 토큰이 바뀔 때 또 깨진다) ②eyebrow 라벨이 헤드라인보다 위에 있다 ③히어로 검색창에 그려지는 테두리가 없다 ④실루엣 위젯이 머티리얼 아이콘이 아니다. 채택 전 일부러 깨서 **red를 확인**한다(CLAUDE.md B4)
**And** 실기기(SM-G991N)에서 **육안 확인 + 스크린샷 증거**를 남긴다(DW-764가 "위젯 테스트만으로 시각 축을 닫았다"고 지적한 그 자리 — 같은 실수를 반복하지 않는다)

**Non-goals:** 카드 열 수·크기 변경(스파인 D5가 390px=1열로 규정) · 운영(`main`) 배포 · 웹의 히어로 배경 외 변경 · 실제 로고 아트워크 제작 · 알림 벨

### Story 16.6: SM-D 통합 시연 검증 (앱)

_✎ 2026-08-08 — **이 스토리는 Epic 16의 맨 마지막에 돈다.** 문서 순번은 6이지만 실행 순서는
16.8 → 16.7 → 16.6이다(sprint-status.yaml이 실행 순서의 정본). 이유: 이 스토리가 "앱이 웹과
동일 디자인 언어임을 실폰에서 검증"하는 자리인데, 검증 대상 화면(홈 16.8·사진 업로더 16.7)이
아직 없는 상태로 돌면 검증이 성립하지 않고 나중에 다시 해야 한다. 위 Given의 범위 정정도
같은 이유다 — 범위를 안 넓히면 16.8이 만든 홈을 아무도 실폰에서 안 본다._
...
**Given** 개편된 Flutter 앱(16.1~16.5 ✎ 정정 2026-08-08: **16.1~16.5 · 16.7 · 16.8**)
...
**And** ✎ 2026-08-09 추가(선행 구현 — DW-740, 사용자 결정 ①) **앱 상세 진입도 `increment_listing_view` RPC를 부른다**. 지금은 이 RPC를 부르는 곳이 웹 상세 페이지 하나뿐이라, 앱만 쓰는 이 시연에서는 `view_count`가 0에 머물고 16.8이 만든 홈 "지금 인기"가 2차 정렬키(`id desc`) 고정 목록으로 퇴화한다 — 라벨과 다른 것을 보여주는 상태다(CLAUDE.md B9). 웹이 이미 두고 있는 **호출 지점 단일성 검사**(`web/src/app/(user)/listings/[id]/__tests__/viewCountCallSite.test.ts`)와 같은 방식으로 앱 쪽 호출 지점도 하나로 고정한다 — 호출이 두 곳으로 늘면 한 번 열람에 2가 오른다
**And** 위 구현이 **실제로 순위를 바꾸는 것까지** 실폰에서 확인한다 — 상세를 열기 전/후로 홈 "지금 인기"의 순서가 달라지는 것을 본다. "RPC를 부른다"는 코드가 있는 것과 "인기 순서가 실제로 움직이는 것"은 다르다(CLAUDE.md B4 "존재 확인은 작동 확인이 아니다")
**And** ✎ 2026-08-09 추가(선행 구현 — DW-738, 사용자 결정 ②) **앱도 비로그인 열람을 연다.** 지금 `app_router.dart`의 `redirect`는 미인증 사용자를 **모든 경로에서** `/login`으로 보내, 웹에는 있는 FR58("비로그인도 목록·상세·홈 열람 가능", anon SELECT 정책이 DB에 실재)이 앱에서만 성립하지 않는다. 열람 경로(`/home`·`/search`·매물 상세)를 화이트리스트로 통과시키고, 상태를 바꾸거나 과금이 나가는 행동(AI 검색·문의·매물 등록·찜)은 기존 `requireUser` 게이트로 막는다 — **화면 단위가 아니라 행동 단위 게이트**다. 사용자 결정 원칙: *요구사항에 "웹 한정"이라고 적혀 있지 않으면 전부 앱 포함.*
**And** 위 개방이 **실제로 열리는지**를 실폰에서 확인한다 — 로그아웃 상태로 홈·목록·상세까지 도달하고, 거기서 찜/문의를 누르면 그때 로그인으로 유도되는 것까지 본다. 라우터 화이트리스트에 경로를 적은 것과 그 화면이 미인증으로 실제로 그려지는 것은 다르다(화면 본문이 Supabase 세션을 전제하면 통과시켜도 빈 화면·예외가 난다)

### Story 16.7: 앱 사진 업로더

_✎ 2026-08-07 신설 — 사용자 결정(2026-07-18, DW-352)의 이행이다. Story 9.3(사진 업로더)을
**web만**으로 확정하면서 "앱 업로더는 Epic 16에 별도 스토리로 추가한다"고 정했는데, 그 스토리가
이 문서에 없었다. 대장이 *"문서에 없으면 sprint-planning이 다시 만들어도 또 빠진다"*고 경고해둔
그대로라 Epic 16 착수 전에 심는다. **현재 계획을 그대로 끝내면 앱 판매자는 사진을 영영 못 올린다**
(16.2는 "이미지 **카드** 재설계"로 읽기 측이다)._

As a 앱 판매자,
I want 앱에서도 매물 사진을 올리길,
So that 폰으로 찍은 사진을 바로 등록해 문의를 받는다.

**Acceptance Criteria:**

**Given** 웹 9.3이 확정한 계약(경로 규약 `{판매자}/{매물}/{파일명}` · 1600px WebP 저장본 · 대표=순서 0번 · 순차 INSERT)
**When** 앱 등록/수정 폼에서 사진을 올리면
**Then** OS 갤러리·카메라 네이티브 피커로 고르고, 웹과 **같은 경로 규약·같은 저장 규격**으로 업로드된다(D14 "사진 업로드=OS 네이티브 피커")
**And** 업로드 실패 시 **Storage에 올라간 파일이 고아로 남지 않는다** — 파일명을 (매물+순번)에서 결정론적으로 만들어 재시도가 같은 이름을 덮어쓴다(2026-08-05 13-10에서 고아 138개를 실제로 만든 뒤 이 방식으로 고쳤다)
**And** 삭제는 **① Storage 오브젝트 → ② `listing_images` 행** 순서를 지킨다(반대로 하면 영구 고아, DW-346)
**And** `sold` 매물에서는 사진 추가·삭제가 **DB 정책으로** 막힌다(DW-390 — 웹과 같은 화면 방어를 또 짜지 말고 `listing_images` 쓰기 정책에 `status <> 'sold'`를 넣는다)
**And** 새 의존성(`image_picker` 등)과 카메라·갤러리 **권한 요청**이 안드로이드에서 실제 기기로 확인된다

**Non-goals:** 이미지 편집(크롭·회전) · 동영상 · 웹 업로더 재작업

### Story 16.8: 앱 홈 랜딩 미러

_✎ 2026-08-08 신설 — 사용자 지적("홈화면이 웹과 너무 다르다, 색상 같은 게 아니라 전체적으로.
수정 계획 있는 거지?")에서 출발해 이 문서를 전수 확인한 결과 **수정 계획이 없음**을 확인하고,
사용자 결정으로 심는다(DW-736). 웹 홈은 **Epic 11(AI 히어로 랜딩 + 내비)** 이 만든 화면인데
Epic 16에 그에 대응하는 앱 스토리가 없었다 — 16.1이 홈 구조를 언급한 것은 *"프로필은 우상단
아바타, 내 차 사기는 홈 하단 스크롤/필터에서 도달"* 한 줄뿐이고 히어로·제안칩·차종칩·인기/최신
2단은 어느 인수조건에도 없다. **16.7(앱 사진 업로더)과 똑같은 모양이다** — 웹에서 한 일에 앱 짝이
없어 계획에서 통째로 빠진 자리. 새로 디자인할 것은 없다: 목업 `app-home-2.html`이 최종 구조를
이미 확정했고 웹 구현이 정본으로 살아 있다._

As a 앱 사용자,
I want 앱 홈이 웹 랜딩과 같은 구조로 보이길,
So that 웹에서 쓰던 방식 그대로 앱에서 차를 찾는다.

**Acceptance Criteria:**

**Given** 웹 랜딩 구현(Epic 11: `HeroSearch`·`CategoryChips`·`PopularRecentGrid`)과 UX 확정 목업 `app-home-2.html`
**When** 앱 홈을 열면
**Then** 딥 petrol 히어로 밴드가 목업대로 그려진다 — 큰 타이포("원하는 차를 **말**로 찾으세요", '말'만 amber 강조) + 알약형 입력 + amber 검색 버튼 + 제안 칩(가성비 좋은 첫차·전기 SUV·무사고 세단 ✎ 정정 2026-08-08: **가성비 좋은 첫차·4천만원대 전기 SUV·주행거리 짧은 무사고 세단·7인승 디젤 패밀리카** — 목업은 앱 전용 축약본, 웹 `HeroSearch.tsx`의 `SUGGESTIONS`가 운영 중인 정본이라 차종 칩과 같은 "웹 우선" 원칙을 적용)
**And** 차종 칩 줄(전체·경차·세단·SUV·전기·수입 ✎ 정정 2026-08-08: **전체·경차·SUV·전기·화물·승합** — "세단"·"수입"은 DB `body_type`/`fuel`에 1:1 대응 값이 없어 웹이 이미 화물·승합으로 대체해 뒀고, 앱도 새 라벨-필터 불일치 칩을 만들지 않고 그대로 따른다)이 히어로 아래에 있고, 탭하면 해당 필터로 탐색에 도달한다
**And** "지금 인기"·"최신" 2섹션이 각각 전체보기 진입과 함께 렌더되며, **정렬과 발췌 건수는 웹과 같다** — 인기=`view_count desc`, 최신=`created_at desc`, 각 4건(Story 11.4, FR34). 앱은 가로 2단이 아니라 세로 2섹션으로 쌓는다 ✎ 정정 2026-08-08: 구현된 두 번째 섹션 헤더는 "최신"이 아니라 **"방금 올라온 매물"**이다(정렬·발췌 기준은 이 AC대로 동일, 헤더 문구만 다르다)
**And** Epic 7 잔재 퀵액션 3개(문의 채팅 `Key('go_chat')`·매물 등록 `Key('go_sell')`·내 매물 관리 `Key('go_my_listings')`)가 **제거**된다 — 하단 4탭(채팅·내차팔기)과 같은 곳으로 가는 두 번째 문이고, 목업에 없다
**And** ⚠️ 위 제거는 히어로·칩·2섹션과 **같은 커밋**이어야 한다 — 먼저 지우면 판매자가 매물 등록에 갈 길이 하단 탭뿐인데 그 탭이 그 화면을 제대로 열지 못하면 갇힌다(DW-728에서 "떼기만 하면 안 된다"로 배운 순서, 웹 14.3→14.2와 같다)
**And** 검사가 **있음/없음이 아니라 순서와 개수까지** 고정한다 — 히어로가 칩보다 위, 칩 줄이 매물 섹션보다 위, 퀵액션 0개를 좌표·개수로 단언한다(`home_ai_entry_test.dart`가 이미 "AI 진입이 탐색 CTA보다 위"를 좌표로 단언하는 선례). 채택 전 일부러 순서를 뒤집어 **red를 확인**한다(CLAUDE.md B4)

**Non-goals:** 웹 랜딩 재작업 · 판매자 화면(매물 등록·내 매물 관리) 리스킨 — 그 화면들은 Epic 16 어느 스토리의 범위도 아니다(별도 등재 대상) · 색 토큰 작업(이미 DW-729로 닫혔다 — `app_theme_color_drift_test.dart`가 웹 `globals.css`를 직접 읽어 17종을 대조한다. 여기서 다루는 것은 **레이아웃·정보구조**다)
