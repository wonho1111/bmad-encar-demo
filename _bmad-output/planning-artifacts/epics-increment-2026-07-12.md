...
- FR58: 비로그인 접근 정책 — 랜딩=홈(단일 공개 페이지). 비로그인도 매물 열람·AI 검색 가능. 로그인 게이트는 "행동"(문의·등록·찜)에만.
...
- AC-CONTRACT-1: ListingCard 이미지 계약 — wire = **단일 `image_url`(대표 서명 URL, nullable)**(CR3, 변형세트 wire 아님) + `view_count` + `image_count`(I7). null이면 "사진 준비중" 5:3 플레이스홀더. AI 응답 카드 동일 계약. api는 서명 안 하고 storage_path만 반환(CR2 — on_sale id로 listing_images 대표 1장 별도 고정쿼리). conventions.md §4 개정 선행.
- AC-DEPLOY-1: 배포 순서 db(마이그)→api(Cloud Run)→web(Vercel), nullable 하위호환, 부분배포 정합성 + 단계 실패 시 중단·역순 롤백.
- AC-F14-1: role CHECK 완화(0013)가 기존 RLS의 role 비교를 안 깨는지 grep 검증(0002 등). signup 역할선택 제거 + `0001 handle_new_user` 트리거 신규 가입 기본 role 배정(I3). `/ai/search` JWT 게이트 비로그인 허용 완화(I2, ai_readonly·sql_guard 유지).

**확정된 값 (사용자, 2026-07-13):**
- `SIGNED_URL_TTL` = 3600s(1시간). sold 전환 후 기발급 URL 최대 1시간 유효(수용).
- 매물당 최대 10장 / 장당 5MB.
...
- FR58 → Epic 8 (비로그인 anon 읽기 토대·게이트는 행동에만, 정책소유) *(Epic 9~14 진입점이 준수)*
...
| FR58 비로그인 접근 | **Epic 8** (anon 읽기 토대·"게이트는 행동에만" 계약) | Epic 9·10·11·13·14 진입점이 이 계약 준수 |
...
> forward-only 번호순 마이그레이션은 번호순=적용순=개발순이 강제다. 아키텍처의 `0011~0018`은 논리 라벨이었고, **아래가 실제 파일 번호의 정본**(현 에픽/스토리 순서로 재부여). 기존 `0001~0010`은 원본. 각 마이그는 self-contained(8.6 CI 체크 통과).

| 파일 번호 | 내용 | 소유 스토리 | (아키텍처 논리 라벨) |
|---|---|---|---|
| 0011 | listing_images + 비공개 버킷·Storage RLS | **9.1** | 0011 |
| 0012 | 신뢰속성(accident_status text+CHECK·is_single_owner·is_non_smoker) | **10.1** | 0012 |
| 0013 | wishlists(찜) | **10.5** | 0015 |
| 0014 | view_count + increment_listing_view() RPC 하드닝 | **11.1** | 0014 |
| 0015 | chat 멱등키(client_message_id + UNIQUE(room_id,…)) | **12.1** | 0016 |
| 0016 | chat realtime broadcast 트리거 + realtime.messages RLS | **12.2** | 0017 |
| 0017 | chat_room_reads(안읽음) | **12.5** | 0018 |
| 0018 | role CHECK 완화(admin 존치) | **14.1** | 0013 |
...
**기반 스토리:** 0011 listing_images(비공개 버킷·Storage RLS·ai_readonly SELECT) + 서명 URL 헬퍼(아티팩트 범용, TTL 3600) · api storage_path만 반환(CR2)
...
**기반 스토리:** 0012 신뢰속성(accident_status text+CHECK·is_single_owner·is_non_smoker, nullable 제3상태) · 0015 wishlists
...
**기반 스토리:** 0014 view_count + increment_listing_view() RPC 하드닝(AC-SEC-2)
...
### Story 8.5: FR58 비로그인 접근 토대

As a 비로그인 방문자,
I want 로그인 없이 매물을 둘러보고 AI 검색을 써보길,
So that 가입 전에 서비스 가치를 체험하고 결정할 수 있다.

**Acceptance Criteria:**

**Given** 비로그인(anon) 사용자
**When** 매물 목록·상세·AI 검색에 접근하면
**Then** 열람·검색이 허용된다(FR58 — 매물 RLS anon SELECT 경로 확인, sold 비노출 FR11은 유지)
**And** `/ai/search`의 JWT 게이트가 비로그인 허용으로 완화되되 ai_readonly·sql_guard 안전장치는 그대로 유지된다(I2)
**And** 로그인 게이트는 **"행동"(문의·매물 등록·찜)에만** 적용됨이 계약으로 확립되고, 게이트 통과 후 **원위치 복귀**가 규정된다
**And** 역할 통합 본체(FR52~54, signup·트리거·role CHECK)는 이 스토리 범위 밖(Epic 14)이며 여기선 접근 토대만 다룬다(FR58 정책 소유 = Epic 8)
...
**And** 마이그 번호는 아래 **"마이그레이션 원장"** 표(문서 상단)가 정본이며, 현 에픽 순서대로 0011~0018을 확정한다(아키텍처의 0011~0018은 논리 라벨이었고 실제 번호는 이 원장이 우선)
...
**Given** 마이그레이션 **0011**(원장 정본, 내용 = listing_images)
**When** 적용하면
...
**Given** 마이그레이션 **0012**(원장 정본, 내용 = 신뢰속성)
**When** 적용하면
...
**Given** 마이그레이션 **0013**(원장 정본, 내용 = wishlists — 아키텍처 논리라벨 0015)
**When** 적용하면
...
**Given** 마이그레이션 **0014**(원장 정본, 내용 = view_count + RPC)
**When** 적용하면
...
**And** 비로그인도 히어로 검색을 사용할 수 있다(FR58 소비)
...
**Given** 마이그레이션 **0015**(원장 정본, 내용 = chat 멱등키 — 아키텍처 논리라벨 0016)
**When** 적용하면
...
**Given** 마이그레이션 **0016**(원장 정본, 내용 = chat realtime broadcast — 아키텍처 논리라벨 0017)
**When** 적용하면
...
**Given** 마이그레이션 **0017**(원장 정본, 내용 = chat_room_reads — 아키텍처 논리라벨 0018)
**When** 적용하면
...
**Given** 마이그레이션 **0018**(원장 정본, 내용 = role CHECK 완화 — 아키텍처 논리라벨 0013)
**When** 적용하면
