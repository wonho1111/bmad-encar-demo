...
**And** **명시 차종이 든 교체 요청**이 SQL로 라우팅돼 `M6.t1("아니 쏘렌토 같은 SUV로 바꿔줘")`가 되묻기로 새지 않는다(DW-612)
...
**And** 위 케이스들이 라이브 재캡처가 아니라 **결정론적 단위테스트로도 고정**된다(라우터 프롬프트가 다시 흔들리면 검사가 먼저 깨져야 한다 — B9)
**And** 완료 시 기준선(`api/docs/g2-baseline.json`)을 재캡처해 갱신하고, 13.8의 G2 게이트를 새 기준선으로 재실행해 통과를 확인한다
...
대안은 **"조건을 먼저 뽑고, 뽑힌 결과로 코드가 갈래를 정한다"**(추출 실패=되묻기)로, 분기 판단이
LLM이 아니라 코드로 내려와 결정론이 된다. 이 패턴은 이미 `hybrid_rag_node`에 반쯤 존재한다
(구조조건 `NONE`이면 벡터검색으로 폴백). 이 스토리에서 **프롬프트 보강으로 끝낼지, 추출-후-분기로
구조를 바꿀지 판단해 근거와 함께 기록**한다 — 프롬프트 보강만 하면 다음 미지 형태에서 같은 종류의
흔들림이 재발할 수 있다는 점을 감안한다. 단, 구조 변경 시 4갈래 라우팅(13.2)의 인수조건과
`RouterDecision`·`_fallback_route`·`_route_decision` 3곳 락스텝 계약을 깨지 않아야 한다.
