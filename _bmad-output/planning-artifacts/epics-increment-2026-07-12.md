...
**When** 앱 홈을 열면
**Then** 딥 petrol 히어로 밴드가 목업대로 그려진다 — 큰 타이포("원하는 차를 **말**로 찾으세요", '말'만 amber 강조) + 알약형 입력 + amber 검색 버튼 + 제안 칩(가성비 좋은 첫차·전기 SUV·무사고 세단)
**And** 차종 칩 줄(전체·경차·세단·SUV·전기·수입)이 히어로 아래에 있고, 탭하면 해당 필터로 탐색에 도달한다
**And** "지금 인기"·"최신" 2섹션이 각각 전체보기 진입과 함께 렌더되며, **정렬과 발췌 건수는 웹과 같다** — 인기=`view_count desc`, 최신=`created_at desc`, 각 4건(Story 11.4, FR34). 앱은 가로 2단이 아니라 세로 2섹션으로 쌓는다
