...
- Story 13.9: 라우팅 안정화 (최상급 표현·교체 요청·맥락 재작성)
...
- 13.9는 13.8 완료 후에만 착수한다 — 13.8이 판정 근거로 쓰는 회귀 기준선(`api/docs/g2-baseline.json`)이 13.9의 라우팅 변경으로 먼저 흐려지지 않게 순서를 지킨다. 13.9가 라우팅 판정 구조를 바꾸더라도 13.2의 4분기 인수조건과 3곳 락스텝 계약은 유지해야 하며, 완료 시 기준선을 재캡처해 13.8의 G2 게이트를 다시 실행해 통과를 확인한다.
