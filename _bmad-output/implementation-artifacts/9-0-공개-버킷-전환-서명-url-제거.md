...
<!-- ✎ 2026-07-19 코드리뷰 정정: `ready-for-dev`로 남아 있었다. 완료 노트·File List가 채워져 있고
     sprint-status.yaml은 `review`인데 이 헤더만 "아직 시작 안 함"이라 말하고 있었다 — 공유 Supabase에
     마이그레이션(0014)이 이미 적용된 뒤라, "0014가 살아 있나?"를 이 문서에 물으면 정반대로 답하는
     상태였다. 아래 Task 체크박스도 같은 이유로 미체크였고 함께 정리한다. -->

> ✅ **AC9 브라우저 검증 완료 (2026-07-19)** — 사용자가 테스트 계정을 제공해 실제로 확인했다:
> 판매자·관리자 삭제 모두 **사진 파일이 실제로 사라졌고**(HTTP 200 → 400, `storage.objects` 0),
> `/sell/[id]/edit` 미리보기도 3장 전부 정상 로드됐다. 경위는 `docs/tech-debt.md` #74.
>
> ⚠️ **그 검증이 버그를 하나 잡았다** — 첫 시도에서 파일이 안 지워졌다. 같은 날 리뷰가 삭제 순서를
> 뒤집으면서, 경로를 알려주던 `listing_images` 행이 cascade로 먼저 사라져 정리 함수가 **0건을 조회하고
> 조용히 성공**하고 있었다. 수정·재검증 완료(#76). **단위 테스트 83건은 이 버그가 있을 때도 전부 통과했다.**
...
- [x] **Task 1 — 마이그레이션 0014 (AC: 1, 2)**
  - [x] `supabase/migrations/0014_listing_images_public_bucket.sql` 작성 — 버킷 public/상한 UPDATE + 읽기 정책 교체 + 관리자 DELETE 정책
  - [x] Supabase MCP `apply_migration`으로 적용(런북 §3 — db가 항상 먼저)
  - [x] **원격 실측**: 공개 URL 익명 접근 / 행 없는 객체 소유자 삭제 / 타인 경로 업로드 차단 → Debug Log에 응답 원문 기록

- [x] **Task 2 — web·app 헬퍼 교체 (AC: 3)**
  - [x] `web/src/lib/storage/index.ts` → `getPublicUrl` (순수 함수, `getSupabaseEnv()` 사용)
  - [x] Vitest `index.test.ts` — URL 조립·경로 인코딩(공백·한글 파일명) 검증. 일부러 깨서 red 확인 후 되돌림
  - [x] `app/lib/core/supabase/storage_helper.dart` 미러
  - [x] 소비처(`edit/page.tsx`) 갱신 + 관련 주석 정리

- [x] **Task 3 — 관리자 삭제 버그 (AC: 4)** 🔴
  - [x] `ListingAdminActions.tsx`가 `deleteListingPhotoObjects`를 먼저 호출하고, 실패 시 삭제 중단
  - [x] 브라우저로 실제 관리자 삭제 → Storage에서 파일이 사라지는지 확인

- [x] **Task 4 — 계약·대장 (AC: 5, 6)**
  - [x] `docs/conventions.md` §4·§6·§10·§10.1
  - [x] `docs/tech-debt.md` 6건 닫기 + 1건 신규 + AC4 해소 기록

- [x] **Task 5 — 스펙 정정·9.4 재작성 (AC: 7, 8)**
  - [x] architecture ADR-IMG-02 추가 · epics 정정 주석 · done 스토리 3건에 supersede 한 줄
  - [x] 9.4 스토리 재작성

- [x] **Task 6 — 검증 (AC: 9)**
  - [x] 위 실측·브라우저 확인 수행, 결과 기록, dev 서버 정리
