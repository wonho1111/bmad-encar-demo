...
> ⚠️ **AC9의 브라우저 검증은 여전히 미완이다** — 로그인 세션이 필요한 화면(`/sell/[id]/edit` 미리보기,
> 판매자·관리자 삭제 후 파일이 실제로 사라지는지)은 확인하지 못했다. `docs/tech-debt.md`가 이 미완을
> 들고 있다(코드리뷰 2026-07-19 defer). **"코드가 있다"와 "동작한다"는 다르다**(B4).
