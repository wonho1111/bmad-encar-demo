...
- [ ] **Task 2. 마이그레이션 0015** (AC4) — ⏸️ **원격 적용만 사용자 승인 대기**
  ...
  - [ ] ⏸️ **원격 Supabase 적용 + 원격에서 같은 3건 재측정** — 권한 정책에 막혀 승인 대기
...
- [ ] **Task 4. G3/CM-A 검증** (AC2) — ⏸️ `/ai` 화면만 미검증
  - [x] 목록·상세 실브라우저 관찰 — 콘솔 에러 0 / 요청 실패 0 / 깨진 아이콘 0
  - [ ] ⏸️ **AI 화면(`/ai`) 실브라우저 관찰** — CORS 차단(아래 Debug Log §4). **API 응답은 직접 확인함**(카드 5/5에 `image_path`·`image_count` 정상)
...
- [ ] **Task 7. 검증 DoD** (AC8) — 테스트 전량 ✅ · 스크린샷 ✅ · `/ai` 화면만 ⏸️
...
#### §4. `/ai` 화면 검증이 막힌 경위 (미완 사유 — 우회하지 않고 남긴다)
...
- **대신 한 것**: API를 직접 호출해 **카드 5/5에 `image_path`·`image_count`가 실려 오는 것**과, sold 매물을 겨냥한 질의가 **0건**을 돌려주는 것(FR11)을 확인했다. **화면 렌더링은 미검증으로 남긴다.**
...
- **green은 빈 Postgres에서** 정책만 갈아끼워 대조했다.
...
- ⏸️ **원격 적용은 사용자 승인 대기.** 단일 공유 Supabase라 적용 = 즉시 운영 반영이다.
...
#### 9. ⏸️ 미완 2건 (우회하지 않고 남긴다)

1. **0015 원격 적용 + 원격 green 재측정** — 운영 DB 변경이라 사용자 승인 대기.
2. **`/ai` 화면 실브라우저 관찰** — CORS·포트 점유로 차단(Debug Log §4). API 응답 레벨은 확인 완료.
...
- 2026-07-21 — Story 9.7 구현. 시드 사진 170장 추가(총 180장/90매물) · 마이그 0015 작성 및 red-green 증명 ·
  #27 시드 재실행 파괴 범위 실측 · G3/CM-A 화면 검증 · 대장 #27·#54·#68·#80 정리, #73·#83·#88 기록.
  **미완 2건**(0015 원격 적용 · `/ai` 화면 관찰)은 사용자 승인/환경 차단으로 남김.
