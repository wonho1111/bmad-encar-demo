...
### 2026-08-03 — Review pass (4차 / 독립 후속 리뷰, DW-633)
...
- addressed_findings:
  - `[high]` `[patch]` (adversarial·edge-case·verification-gap **3중 독립 실증**) 3차 패스가 "드리프트를 잡는다"며 넣은 큐리셋 락스텝 검사가 **양방향 중 한 방향만** 잡았다. `shipped`를 큐리셋에서 `if it.get("query") in GRAY_ALLOWED`로 **먼저 걸러** 만들었기 때문에, `GRAY_ALLOWED`에서 행을 지우면 양쪽이 같이 줄어 초록이다 — 세 레이어가 각각 뮤테이션으로 3행→1행 축소가 **전량 초록**(`-k gray` 3 passed)임을 증명했다. 3차 패스가 red를 확인한 것은 "기존 행의 허용경로가 좁아지는" 방향뿐이었다. 큐리셋에서 `category: gray` 항목을 **독립적으로** 뽑아 ①존재 ②경로일치 ③반대방향(큐리셋에 새 gray 질의가 생겼는데 표 ③에 없음)을 전부 보게 고치고, 행 수 불변식은 `demo_queries.py`의 `GRAY_ALLOWED` **바로 옆 import 시점 assert**로 옮겼다(B9 — 소비처가 늘어도 못 어긴다). → **행 삭제·큐리셋 추가 둘 다 red 확인, 복원 green 확인.**
  - `[high]` `[patch]` (verification-gap 뮤테이션 실증, md5 대조) `test_main_refuses_to_overwrite_committed_baseline`이 **실제 커밋된 기준선 경로**로 `main()`을 부른다 — 즉 가드가 회귀하는 순간(=이 테스트가 잡으라고 존재하는 바로 그 상황) **테스트 실행 자체가 두 기준선을 0항목으로 비운다**. `pytest tests/` 한 번이면 된다. 원인은 검사가 `main()`의 argparse에만 있고 실제로 파일을 비우는 `capture()`/`_flush()`에는 없어서다 — `capture()`는 공개 함수라 직접 호출 경로가 실재한다(edge-case 렌즈가 직접 호출로 47항목→0항목 실증). 검사를 `capture()` 진입부(첫 `_flush` 이전)로 내리고, 거부 테스트에 **사후 바이트 대조**를 추가했다. → **main() 가드를 제거하니 2건 red이면서 기준선 md5는 그대로**(이전엔 red와 함께 파괴됐다). `capture()` 직접 호출 거부 테스트 1건 신규.
  - `[medium]` `[patch]` (edge-case) `ai-demo-queries.md` 서두가 "회색지대는 **CLARIFY로 보내 되묻는다**"라고 무조건으로 규정하는데, 같은 문서 표 ③의 3행 중 2행은 CLARIFY가 **오답**이다(H6/H7은 SQL 또는 HYBRID이고, 표 밑 문구가 "거절·되묻기로 새면 불합격"이라 명시). 서두 두 줄 다 이 diff의 `+` 줄이다 — 구어휘 "회색지대 → B"를 기계적으로 번역하며 생긴 모순이고, **이 문서는 라우터의 단일출처이며 다음 스토리 13.9가 라우팅 안정화**다. 서두를 "허용 경로는 행마다 다르다, 표 ③이 정본"으로 정정.
  - `[medium]` `[patch]` (adversarial) 3차 패스가 `test_sm3_gray_zone_returns_listings_either_route`를 `..._allowed_routes_are_not_dead_ends`로 개명하면서 **SM3 매핑 표의 바로 그 행을 편집하고도 테스트 이름 칸은 안 고쳤다** — 단일출처 문서가 존재하지 않는 테스트를 가리키고 있었다(개명의 명분이 "실패 로그 이름만 보고 오진하는 것을 막는다"였다). 새 이름으로 정정하고 락스텝 검사도 함께 등재.
  - `[medium]` `[patch]` (verification-gap 재실행 실증) Verification 커맨드 3번(`score_ab.py` G2 채점)에 **`DATABASE_URL`이 빠져 있어 기록된 그대로 실행하면 크래시**한다(`RuntimeError: 환경변수 DATABASE_URL 가(이) 설정되지 않았습니다`, 채점기가 정답 결과집합을 실DB에서 다시 뽑기 때문). **2차 패스가 잡은 `TEST_DATABASE_URL` 오류와 똑같은 유형이 네 번째 커맨드에 남아 세 패스가 전부 놓쳤다** — G2는 재현 비용이 가장 비싼 게이트인데 그 채점 단계를 기록된 커맨드로는 아무도 재현할 수 없었다. 정정 후 실제 실행해 커밋된 리포트와 수치가 완전히 일치함을 확인(54/57·0.8936·33/47·12/13·9/9·`gate_pass:true`·`regression_block:false`).
  - `[medium]` `[patch]` (verification-gap·adversarial) 3차 패스가 넣은 **과차단 대조군** `test_main_allows_non_baseline_out_path`가 `API_ROOT/docs/...`를 계산해 놓고 정작 `tmp_path`에 썼다(죽은 식) — `docs/` 안의 경로를 한 번도 시험하지 않으므로 **가드가 `docs/` 전체를 과차단해도 초록**이다(실증: 문서화된 정상 캡처 커맨드가 exit 2로 거부되는 상태에서 3 passed). 기준선과 같은 디렉터리의 임시 경로를 실제로 쓰고 `finally`로 정리하게 고침 → **과차단 회귀 주입 시 red 확인.**
  - `[low]` `[patch]` (adversarial·edge-case) 기준선 가드의 주석·에러메시지·독스트링·테스트 독스트링 네 곳이 **"복구 수단은 유료 47문항 재캡처뿐"이라 적었는데 사실이 아니다** — 보호 대상 2개 파일은 모두 git 추적 중이고, 3차 리뷰가 실제로 파괴했을 때도 `git restore`로 되돌렸다(이 스펙 자신이 그렇게 기록했다). 진짜 위험은 복구 불가가 아니라 파괴가 exit 0으로 조용히 지나가는 것이다. 네 곳 정정.
  - `[low]` `[patch]` (adversarial) 가드의 에러메시지·독스트링이 권하는 `docs/g2-capture-YYYY-MM-DD.json`이라는 이름이 **리포 어디에도 없다**(실제 아티팩트는 `g2-exit-gate-<날짜>.json`) — 안내를 따르면 어떤 스펙·커맨드도 참조하지 않는 파일이 생긴다. 실제 쓰는 이름으로 통일.
  - `[low]` `[patch]` (edge-case) 락스텝 테스트 독스트링이 "표 ③·`GRAY_ALLOWED`·큐리셋 **셋**을 대조한다"고 적었으나 `ai-demo-queries.md`를 읽는 코드는 리포에 없다(스펙 본문은 정직하게 "두 곳"이라 적어 뒀다) — 독스트링만 과장이었다. 두 곳임을 명시하고 표 ③은 사람 몫임을 ⚠️로 남김.
  - `[low]` `[patch]` (오케스트레이터, 전체 스위트 재실행) 검사 1건 추가로 전체 개수가 398 → **399 passed**가 되어 Verification 기대값을 갱신. 아울러 frontmatter `final_revision: fd28b438`이 **HEAD(1e9921c)의 조상이 아닌 dangling 커밋**이라 `baseline_revision..final_revision` 범위가 실제 shipped 내용을 재현하지 못했다(8줄 차이, `git gc`에 사라질 객체) — 이번 Finalize에서 실제 HEAD로 갱신.
- 리뷰 레이어: adversarial 11건 · edge-case-hunter 11건 · verification-gap 4건(전부 재실행·뮤테이션 실증) · intent-alignment 서술(해석 A~E + 두 긴장 분석). **세 레이어가 독립적으로 같은 결론에 도달한 항목 1건**(락스텝 검사가 한 방향만 잡는다) — 이번 패스의 high 2건은 모두 *3차 패스가 red를 확인했다고 기록한 검사* 자체의 결함이다.
- **이번 패스의 요지**: 지시받은 확인 대상이었던 **3차 패스의 high 지적 2건과 신규 장부 DW-619~632는 실제로 해소·정확하다**(아래 Auto Run Result 참조 — 14건 전부 코드 대조, 틀리거나 과장되거나 이미 해결된 항목 0건). 새로 나온 것은 **그 해소를 수행한 검사들 자신의 결함**이었다: 3차 패스가 "일부러 깨서 red 확인"을 실제로 수행한 것은 사실이나 **깨는 방식을 한 가지만 시험**해서, 락스텝은 반대 방향이 열려 있었고 과차단 대조군은 아예 red가 안 떴으며 기준선 가드는 red와 함께 지키려던 파일을 파괴했다.
- **bad_spec으로 올리지 않은 이유**: 3차 패스와 같다 — 결함은 `<intent-contract>` 밖(테스트·스크립트·Verification 커맨드)에 있고, 되돌리면 유료 47문항 재캡처를 다시 태우면서 얻는 게 없다. 앱 코드(`api/app/**`)는 이 패스에서도 한 줄도 바뀌지 않았다.
- defer(신규 등재만, 기존 항목은 지시대로 일절 수정·재개방하지 않음): DW-634(medium, SM3 ①② 게이트에 노드 식별 단언 없음 — `SQL→hybrid` 오배선이 그 게이트에선 초록) · DW-635(medium, `score_ab.py --out`에 보호 없음 + 독스트링이 커밋된 리포트를 예시로 제시 — Manual checks의 유일한 기준점) · DW-636(medium, 0건 캡처가 exit 0 + 증거 아티팩트가 보호 밖) · DW-637(medium, `--raw` 인자 **순서**가 회귀 방향을 정하는데 리포트에 안 남음 — 순서 뒤집으면 진짜 하락도 통과, 실증) · DW-638(low, `--subset` 경로의 맨 `KeyError`) · DW-639(low, epic 컨텍스트가 "47개" 사본을 새로 심음) · DW-640(medium, DW-620·625·627·632의 trigger가 예정에 없는 스토리에 걸려 발화 불가 — 특히 실증된 보안 공백 DW-627).
- reject 3건(전부 low): **DW-622↔DW-629 포인터 부재**(DW-629 본문이 이미 "DW-622를 처리하는 사람은 DW-629를 함께 읽어야 한다"고 적었고, 기존 항목 수정이 금지돼 DW-622 쪽에 역포인터를 달 수 없다 — 새 항목을 하나 더 만드는 것은 장부만 늘리고 같은 문제를 남긴다) · **1회성 비재현 테스트 실패**(adversarial이 `test_main_refuses...` 2건의 `DID NOT RAISE`를 한 번 관측했으나 캐시 정리·파일 순서 교차 포함 10여 회 재실행에서 재현되지 않았고, 이번 패스가 그 테스트를 다시 쓰면서 사후 바이트 대조까지 넣어 같은 증상이 나면 원인이 드러나게 됐다) · **`epic-13-context.md`의 Cross-Story 의존 범위 축소에 근거 주석 없음**(13.4~13.7이 실제로 13.1에 의존하지 않음을 확인 — 부정확했던 옛 문장이 사실에 맞게 좁혀진 것이라 결함이 아니다).
...
- `cd api && DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:55322/postgres .venv/bin/python scripts/score_ab.py --raw docs/g2-baseline.json docs/g2-exit-gate-2026-08-02.json --out docs/g2-exit-gate-report.json` -- expected: 리포트의 `gate_pass:true`(양쪽)·`regression_block:false`·`coverage.is_partial:false`(양쪽). ⚠️ **`DATABASE_URL`이 없으면 채점이 시작조차 못 한다**(`RuntimeError: 환경변수 DATABASE_URL 가(이) 설정되지 않았습니다`) — 채점기가 정답 결과집합을 실DB에서 다시 뽑기 때문이다(`score_ab.py`의 `run_golden_ids` → `app/db/readonly.py`). 4차 리뷰 전까지 이 변수가 빠져 있어 **기록된 커맨드로는 G2 채점을 아무도 재현할 수 없었다**(2차 리뷰가 잡은 `TEST_DATABASE_URL` 오류와 같은 유형이 네 번째 커맨드에 남아 있었다). ⚠️ **`--raw`의 인자 순서가 회귀 판정 방향을 정한다**(앞=baseline, 뒤=candidate) — 순서를 바꾸면 진짜 하락도 `regression_block:false`로 통과하고 리포트엔 그 사실이 안 남는다(DW-637).
...
- `cd api && .venv/bin/python -m pytest tests/ -q` -- expected: **399 passed, 85 skipped, 0 failed**(4차 리뷰가 `capture()` 직접 호출 거부 테스트 1건을 추가해 398에서 +1). 앱 코드 변경 0, 회귀 없음. ⚠️ 이 커맨드의 초록에는 **85건의 skip**(라이브·실DB 통합 전량)이 묻힌다 — 위 개별 커맨드들이 그 skip을 걷어내는 자리다.
...
_4차 리뷰 패스(2026-08-03, DW-633)가 손댄 파일: `api/scripts/run_phase_b.py`(기준선 검사를 `capture()` 층으로 내림 · 거짓이 된 "복구 불가" 서술 4곳 정정 · 존재하지 않는 파일명 규칙 정정) · `api/tests/test_run_phase_b.py`(거부 테스트에 사후 바이트 대조 추가 · `capture()` 직접 호출 거부 테스트 1건 신규 · 과차단 대조군이 실제로 `docs/`를 쓰도록 수정) · `api/tests/demo_queries.py`(행 수 불변식을 `GRAY_ALLOWED` 옆 import 시점 assert로) · `api/tests/test_demo_acceptance.py`(락스텝을 큐리셋 독립 추출 3방향 검사로 · 중복 assert 정리) · `api/docs/ai-demo-queries.md`(서두↔표 ③ 모순 · 존재하지 않는 테스트 이름) · 이 스펙(Verification 커맨드 3번의 `DATABASE_URL` 누락 · 전체 개수 399) · `deferred-work.md`(DW-634~640 신규 7건). **앱 코드(`api/app/**`)는 이 패스에서도 한 줄도 바뀌지 않았다** — 뮤테이션 5건은 검증용이었고 전부 원복·md5 대조 확인했다._

**Review findings breakdown (4 layers: adversarial·edge-case-hunter·verification-gap·intent-alignment, 4 passes):**
...
- 4차 패스(2026-08-03, DW-633): patch **high 2**(락스텝 검사가 한 방향만 잡음, 기준선 거부 테스트가 red와 함께 기준선을 파괴) → true. 참고 점수 3×4+1×4=16.
- **네 패스 연속 true인 것이 이 스토리의 가장 중요한 신호다.** 각 패스가 이전 패스의 "확인했다"를 한 겹씩 벗겨냈다 — 1차는 문서 모순을, 2차는 두 게이트가 실제로는 스킵이었음을, 3차는 통과한 그 검사들이 **무엇을 못 잡는지**(기준선 파괴·필터 무력화·세 축 미비교)를, 4차는 **3차가 그걸 막겠다고 넣은 검사들 자신의 결함**을 드러냈다. 공통점은 전부 **diff를 읽어서가 아니라 커맨드를 실제로 돌리거나 코드를 일부러 깨서** 나왔다는 것이다(B4). 그리고 4차가 찾아낸 패턴이 하나 더 있다 — **"일부러 깨서 red를 봤다"는 기록이 있어도 깨는 방식을 한 가지만 시험하면 반대 방향이 열려 있다.** 5차 패스를 또 도는 것보다, 13.9에서 (a) DW-626·627·630을 실제 검사로 바꾸고 (b) 새 검사를 넣을 때 **양방향 뮤테이션**을 관례로 삼는 편이 남은 위험을 더 줄인다.
...
- **4차 리뷰(2026-08-03, DW-633)에서 실제로 돌린 것:** 지시받은 확인 대상부터 — **DW-619~632 14건 전부를 코드·CI 워크플로와 대조**했고 **틀리거나 과장되거나 이미 해결된 항목은 0건**이었다(DW-626의 단일 축 비교는 `score_ab.py:777`에서, DW-627의 문자열 검사는 `test_doc_rag_node.py`의 `run_select` 모킹에서, DW-628은 `tests.yml`의 두 잡 범위에서, DW-630은 `RUN_LIVE_SMOKE` 없이 `6 skipped`·exit 0으로 각각 재확인). 3차 패스의 high 2건도 코드에서 실제로 해소돼 있다(가드 존재·AC2/AC3 문장 정정 확인). 커맨드 재실행: CM-B 유닛 5파일 `DATABASE_URL=… -rs` → **149 passed, 0 skipped** · FR11 실DB `TEST_DATABASE_URL=…` → **1 passed** · SECURITY DEFINER → **5 passed** · 회색지대 `-k gray` → **7 passed** · 전체 `pytest tests/ -q` → **399 passed, 85 skipped, 0 failed**(신규 검사 1건만큼 398에서 증가). G2는 유료 재캡처 없이 **커밋된 두 캡처로 채점만 재실행**해 커밋된 리포트와 수치가 완전히 일치함을 확인(54/57·0.8936026936026936·33/47·12/13·9/9·`gate_pass:true`·`regression_block:false`) — 다만 그 커맨드는 `DATABASE_URL` 없이는 크래시했고, 그게 이번 패스의 medium patch 중 하나다.
- **4차 리뷰의 새 검사·수정된 검사는 전부 양방향으로 깨서 확인했다(이번 패스가 배운 것 그 자체):** ① 락스텝 → `GRAY_ALLOWED` 행 삭제 red · 큐리셋에 gray 질의 추가 red · 행 수 상수 불일치는 **수집 시점 ERROR** · 전부 복원 green. ② 기준선 가드 → `main()` 가드 제거 시 2건 red **이면서 기준선 md5 불변**(이전 구현에선 red와 동시에 두 파일이 0항목으로 파괴됐다 — md5로 실측 확인) · `capture()` 직접 호출 거부 신규 테스트 green. ③ 과차단 대조군 → 가드를 `docs/` 전체 차단으로 회귀시키니 red(이전엔 초록이었다) · 복원 green. 뮤테이션 5건(`demo_queries.py`·`ai-ab-test-queryset.json`·`run_phase_b.py` 2형태)은 전부 원복했고 `git diff -- api/scripts` 확인·기준선 3파일 항목 수(47/3/47) 재확인했다.
...
- **(4차 리뷰) G2 회귀 판정은 축이 하나일 뿐 아니라 방향도 검증되지 않는다**(DW-637) — `--raw`에 준 **파일 순서**가 baseline/candidate 배정을 결정하는데, 리포트의 `baseline`/`candidate` 필드에 들어가는 것은 파일 경로가 아니라 **모델명**이고 13.8은 같은 모델을 자기 자신과 비교하므로 양쪽이 동일 문자열이다. 즉 순서를 뒤집어 채점하면 진짜 하락(0.894→0.269)도 `regression_block:false`로 통과하는데 **산출물만 봐서는 그 사실을 알 수 없다**(리뷰가 저하판으로 양방향 채점해 실증). 지금은 두 캡처가 같은 코드 상태라 실해가 없지만, 13.9가 재기준선을 뜨면 순서 실수의 대가가 실제로 생긴다.
- **(4차 리뷰) 보호 가드의 범위가 여전히 파일명 열거다**(DW-635·DW-636) — `_PROTECTED_BASELINES`는 기준선 2개만 덮고, 13.8의 AC2 증거인 `g2-exit-gate-2026-08-02.json`과 Manual checks의 유일한 기준점인 `g2-baseline-report.json`은 보호 밖이다. 후자는 아예 다른 스크립트(`score_ab.py`) 소관인데 그쪽엔 검사가 전혀 없고 독스트링이 그 파일을 `--out` 예시로 제시한다. 여기에 "0건 캡처도 exit 0"이 겹치면 조용한 파괴 경로가 된다(복구는 `git restore`로 되지만, exit 0이라 아무도 시도하지 않는다).
- **(4차 리뷰) "red를 확인했다"는 기록이 검사의 완결성을 뜻하지 않는다** — 이번 패스의 high 2건은 둘 다 *3차 패스가 일부러 깨서 red를 봤다고 정직하게 기록한 검사*였다. 3차의 기록은 사실이었고 조작이 없었다. 문제는 **깨는 방식을 한 가지만 시험한 것**이다: 락스텝은 "허용경로가 좁아지는" 방향만, 과차단 대조군은 애초에 대조 대상이 아닌 경로만, 기준선 가드는 "거부하는가"만 보고 "그 과정에서 파일이 다치는가"는 안 봤다. 13.9가 DW-626·627·630을 실행되는 검사로 바꿀 때 **양방향(또는 다방향) 뮤테이션**을 관례로 삼지 않으면 같은 층이 한 겹 더 쌓인다.
- **(4차 리뷰) 열린 항목 4건은 담당 스토리가 없다**(DW-640) — DW-620·625·627·632의 `trigger:`가 백로그에 없는 스토리를 조건으로 건다. 특히 **DW-627은 뮤테이션으로 실증된 FR11 보안 공백**(`doc_rag_node`의 `WHERE status='on_sale'`를 `OR true`로 무력화해도 CM-B 커맨드 전량 초록)인데 고칠 자리가 지정돼 있지 않다. 기존 항목 수정이 이번 실행에서 금지돼 신규 항목으로만 남겼으므로, **다음 sweep이 실제로 재지정해야 한다**(CLAUDE.md B8).
