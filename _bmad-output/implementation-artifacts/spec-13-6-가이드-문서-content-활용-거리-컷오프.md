...
- `cd api && RUN_LIVE_SMOKE=1 DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:55322/postgres .venv/bin/python scripts/run_phase_b.py --subset B1,B2,B3,B4,B5,B6,B7,G1,G4 --out /tmp/g13-6-check.json && .venv/bin/python scripts/score_ab.py --queryset docs/ai-ab-test-queryset.json --raw /tmp/g13-6-check.json --out /tmp/g13-6-report.json` -- expected: `doc_hit` recall이 기존 g2-baseline 대비 하락 없음(Block-If 조건 실측).
  ...
  **AC1의 가드가 아니었다**(변이 실측: `_GUIDE_BLOCK_TEMPLATE`를 통째로 비워 질의확장을 죽여도 그
  ...
  ✎ **2026-07-31 실측 완료(DW-604·605·607·608 종결)** — 로컬 Supabase(pgvector)가 뜬 세션에서
  Block-If를 실제로 실행했다. 위 1-raw 명령은 DW-608대로 A/B 비교를 하지 못하므로 **쓰지 말 것**.
  실제로 동작하는 절차는 다음과 같다(같은 코드에서 컷오프만 껐다 켜므로 분모·교란 문제가 없다):
  ```
  # 두 팔 캡처: ON = 현재 코드(0.3), OFF = doc_rag_node._GUIDE_DISTANCE_CUTOFF를 99.0으로 몽키패치
  #   (hybrid_rag_node는 상수가 아니라 find_relevant_guide 함수를 import하므로 모듈 전역만 바꾸면 양쪽에 적용)
  RUN_LIVE_SMOKE=1 DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:55322/postgres \
    .venv/bin/python <래퍼> {on|off} B1,B2,B3,B4,B5,B6,B7,G1,G4,G2,G3,G6,M4,M7 /tmp/arm_{on|off}.json
  # 채점: score_ab.doc_hit()를 직접 호출해 per-item으로 센다(summary엔 이 지표가 없다 — DW-608)
  ```
  결과: `doc_hit` recall **ON 0/10 · OFF 0/10 → 하락 없음(HALT 미충족)**. 단 이 0/0은 컷오프가
  아니라 **doc_refs 항목 10건 전부가 CLARIFY/REJECT로 라우팅돼 `doc_rag_node`에 도달하지 못하기
  때문**이다(13-4 이후 B1·B3·G1·G4·M4·M7이 CLARIFY로 흡수 — 07-30 baseline에선 4건이 route B로
  도달했다). 이 지표는 지금 구조상 항상 0이며 subset 재구성으로도 살릴 수 없다(DW-607→DW-609).
  ✎ **임계값 0.3은 실측으로 확정됐다** — 에픽 13.6 AC("하드코딩 상수가 아니라 Phase B 질의셋으로
  튜닝해 확정")를 이행해 큐리셋 44항목 전체의 최근접 가이드 거리를 실측했다: 기대 가이드 있음 10건
  = 0.1944~0.3042(중앙 0.2301) · 없음 34건 = 0.2418~0.4914(중앙 0.3168). 후보별 (정답/무관 통과)
  = 0.25 → 6/10·1/34 · **0.3 → 9/10·8/34** · 0.35 → 10/10·25/34 · 0.4 → 10/10·31/34. 0.35↑는
  노이즈 급증, 0.25↓는 정답 4건 손실이므로 **0.3 유지**(값 변경 없음). 무관 질의 최상단(C2 김치찌개
  0.4914·C1 날씨 0.4750)은 어느 후보값에서도 차단된다.
  ✎ **AC1은 매물 수준에서 확인됐다** — `run_search("3천만원 이하로 무난한 패밀리카")` 실행 결과:
  가이드 거리 0.2550 통과 → 추출 조건 `price <= 30000000 AND body_type IN ('준중형차','중형차',
  'SUV','RV')` → 반환 5건이 전부 SUV(3)·중형차(2). 같은 가격대 풀에 있는 경차 8·화물차 3·소형차 3·
  경승합차 2는 전부 배제됐다. answer도 `조건에 맞는 매물 5건을 찾았어요. (참고: 패밀리카로 무난한
  차종 고르기)`로 결정론 템플릿과 글자 그대로 일치(AC2). `pytest tests/test_live_smoke.py -q` = 5 passed.
