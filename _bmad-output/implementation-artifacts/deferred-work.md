...
resolution: 커밋 ba4b8c3 — 학습표+대상의 model 문자열을 정수 코드로 인코딩해 11번째 특징으로 추가, TabPFNRegressor(categorical_features_indices=[10]). 실매물 300건 hold-out 검증(.logs/encar_eval/20260905/report.md §8): q10~q90 적중 84.3%(기대 80), 판정 분포 8·15·54·15·8%(기대 10·15·50·15·10). 시연 매물 2건(GN7 하이브리드 2024/2025)이 4,034만 동일에서 3,988만(저렴)/4,029만(높음)으로 갈라짐.
...
update(2026-09-09 18:08, 재검증 2차): 회귀 MT5는 통과(2회 연속 실패 → 통과). 그러나 100건에서 멀티턴 wrong_reference 5건 남음(C50·C51·C53·C57·C59) — 원인은 DW-872 update의 (a)~(d). 수정 묶음 2 후 3차 재실행에서 멀티턴 wrong_reference 0이면 DW-872와 함께 닫는다.
update(2026-09-09 3차 재검증): 재검색은 코드로 막혔고 직전 목록 id도 문맥에 명시되는데 멀티턴 wrong_reference 4건 잔존(C48 목록의 K8·K5 하이브리드를 '없다'고 답함, C50 스파크 2대 혼동, C59 '첫 번째' 순서 오독, C60 '더 저렴한 쪽' 오독) — 모델이 문맥의 목록을 잘못 읽는 유형. 다음 후보: 순번('첫 번째')·극값('제일 싼/최신/짧은')·비교('더 저렴한 쪽') 지칭을 코드가 직전 목록(카드 순서·가격·연식·주행)에서 결정적으로 풀어 해당 id를 도구 인자로 넘기기, 또는 최종 selected_listing_ids를 직전 목록과 교집합. 트리거: 다음 에이전트 수정 때 위 후보 구현 후 100건 재실행, 멀티턴 wrong_reference 0이면 닫음.
update(2026-09-14, H 회차 — .logs/chatbot_eval/20260914_h): 1차 코드 aeba16a(순번·극값·비교 지칭을 코드가 직전 목록에서 id로 해석 → 도구 인자 교체·최종 id 교집합) 라이브 100건: 회귀 30/31(S16 데이터 의존), 전체 GOOD 93(변화 없음), 멀티턴 11/15 그대로 — 남은 4건의 원인은 지칭 해석이 아니었다: C48 '그거 말고 하이브리드만'은 후속 재검색 차단이 거절, C50·C59는 모델이 도구 원문(무사고·흰색)을 반대로 읽음, C60은 search_listings가 로컬 6건을 0건으로 돌려줌(HNSW 사후 필터 → DW-882). 2차 코드 d95d135: 제외어 후속은 첫턴 조건 합쳐 재검색(예산 패턴 파싱), 속성 좁힘(사고·색상·연료)을 코드가 직전 목록에서 계산해 최종 id 강제, HNSW iterative_scan. 단위 719 passed. **2차 라이브 확인(멀티턴·가이드 30건, driver2.sh)은 Gemini 크레딧 소진으로 미실행.**
update(2026-09-14 밤, 운영 실측으로 재개방): 시세 카드 8건 캡처 중 시드 '아반떼MD'(5d0d1114, 2014)가 비교군 0대·학습표 0건 → 카드가 '표본 부족'. 0038 백필로 generation='아반떼 MD'는 채워졌지만 비교군 사다리 마지막 단과 학습표 SELECT는 여전히 `_base_model(model)`의 ILIKE '%아반떼MD%'를 쓰므로 세대 열이 있어도 못 찾는다 — 9월 14일 닫음은 '특징 코드'만 보고 '조회 경로'를 안 본 오판. 고치는 자리: _base_model이 generation을 우선 쓰거나 한글+영문 붙은 토큰을 분리. 데이터는 시드 1건이라 영향은 작지만 사용자 등록 매물이 같은 표기를 쓰면 재발.
status: done 2026-09-15
resolution: adccafc — _base_model이 한글 바로 뒤 영문/숫자 앞에 공백을 넣어 토큰 분리('아반떼MD'→'아반떼'). 운영 재캡처: 시드 아반떼MD 2014 비교군 0→200대, 범위 594~761만, 판정 저렴. 근본 구조(모델군·세대 참조 테이블)는 DW-886.
...
status: done 2026-09-05
resolution: 3단계(ba4b8c3) 세대 특징 추가 후 두 시연 매물 예측이 39,880,000 / 40,290,000으로 분리. 실매물 300건에서 LOO 대 hold-out 예측 차이 평균 0.9%·판정 일치 93.3% — 표본이 수백 건이면 이웃 1건 효과는 작다(report.md §8.2). 카드 대표값(mean vs q50)은 DW-862 개편 때 결정.
...
status: done 2026-09-05
resolution: 커밋 ba4b8c3 — _train_rows_query(): ORDER BY (model=대상) DESC, (fuel=대상) DESC, |year차|, |mileage차|, id LIMIT 120. 단위 2건·실DB 통합 1건(red→green), 시드 스윕 위반 0. 실매물 하네스(api/scripts/eval_market_price_encar.py) LOO·hold-out 두 방식 모두 측정: TabPFN 오차율 중앙값 5.8%/5.7%, 방식 간 차이 0.9%.
...
### DW-862: 시세 진단 카드가 예측 내용보다 기계적 정보(비교군 SQL 통계·계산·판정 기준)를 앞세운다 — 사용자 시연 피드백, 화면 구성 개선 필요

origin: 사용자 피드백(2026-09-05) — 시연 피드백 후속 논의
location: web 시세 진단 카드(MarketDiagnosis 계열 컴포넌트, 착수 시 파일 확정) · app 동일 카드; API 응답(diagnose() dict)은 그대로
severity: medium
reason: 카드가 "비교군 9건과 비교, 하위 22%, 중앙값, 분위수" 같은 산출 과정을 먼저 보여주고, 사용자가 실제로 원하는 "이 차의 적정가 범위는 얼마이고 왜 그런가(연식·주행·옵션이 각각 얼마나 작용했나)"는 숫자 하나로 끝난다. 엔카 실매물 검증(1~4단계) 결과가 나오기 전엔 문구와 수치 표현을 확정할 수 없어 미룬다. 목업 초안은 2026-09-05 아티팩트로 사용자에게 제시했다.
trigger: 4단계 검증 수치가 나온 뒤, 포트폴리오 문안과 함께 — (1) 헤드라인 = 적정가 범위(q25~q75)와 한 문장 판정("비슷한 조건 100대 중 N대보다 높음"), (2) 근거 = 연식·주행·옵션 what-if 민감도 3줄(모델 실측값), (3) 비교군 통계·산점도는 접힘 영역으로, (4) "호가 기준" 표기. 웹·앱 동시 반영, 화면 문구는 E2E 1건으로 검증.
status: done 2026-09-14
resolution: 2026-09-14 웹(def6bc0)·앱(다음 커밋) 동시 반영 — 확정 설계(2026-09-05): 헤드라인 '이런 조건이면 보통 A~B만원'(TabPFN q25~q75, 없으면 stats q1~q3), 한 문장 판정 '열에 N이 이 매물보다 쌉니다'(실제 대수 표현 금지), 가격 축 하나짜리 AI 가격 곡선(q10~q90 종 모양)+비슷한 차 실제 가격 점+이 매물 세로선(SVG/CustomPainter, 라이브러리 없음), 타일 2개(비슷한 차 N대 중간 가격 / AI가 본 적정가)+'AI 적정가보다 N% 높음' 하나, 비교군 통계·사다리 단계·산점도·비교 목록은 '자세히' 접힘(기본 닫힘, '이런 차와 비교했어요:' 라벨), '기준 완화 없음' 삭제, '호가 기준'·TabPFN 소문구, 민감도 수치 없음. 웹 화면 문구는 LLM 답변이 아니라 서버 수치로 직접 계산(LLM 문장이 도구 요약을 그대로 새던 원인 제거). 두 진입점(상세 버튼→/ai 프리필, AI 검색)은 웹·앱 모두 같은 컴포넌트 1개. 검증: 웹 vitest 417·빌드 통과, 앱 analyze 0·test 599, red/green 각 2종; 웹 E2E(로컬 3000 + 운영 백엔드, seller@, 그랜저 GN7 2b759cad) 실측 — 헤드라인 2,822~3,437만원·판정 '거의 전부'·105대 중간 2,989만·AI 적정가 3,129만·'17% 높음'·자세히 기본 닫힘→펼치면 비교 목록 60건, '대 중' 표현 0(.logs/g_card_20260914/g-card-web-collapsed.png). 앱은 실기기 미확인(위젯 테스트까지). 다건 진단 표(MarketDiagnosisTable)는 범위 밖 그대로.

### DW-863: TabPFN 사고 특징이 무사고/그 외 이진(accident_free)이라 단순교환과 사고를 구분하지 못한다 — 실매물 검증에서 사고 매물 오차 2배

origin: 4단계 실매물 hold-out 검증(2026-09-05, .logs/encar_eval/20260905/report.md §8.2)
location: api/app/market_price.py(_tabpfn_features 10번째 칸 int(accident_free)), listings.accident_status(무사고/단순교환/사고)
severity: medium
reason: 300건 검증에서 오차율 중앙값이 무사고 5.2%·단순교환 5.3%인데 사고 10.4%. 실매물 감가는 단순교환 −80~−250만, 사고 −200~−700만으로 크게 다른데 특징은 둘을 같은 값(0)으로 넣는다. 다음 수업 일정상 이번엔 특징 변경을 미룬다(변경 시 재검증 1.5시간).
trigger: 4단계 하네스가 있으니 특징을 3단계 서열(무사고 2·단순교환 1·사고 0) 또는 원핫으로 바꾸고 같은 300건으로 재측정해 사고 매물 오차가 줄고 나머지가 유지되는지 확인한 뒤 채택.
update(2026-09-09, 300건 LOO cases_loo.csv 사고 상태별 재집계): 무사고 164건 MdAPE 5.2% · 단순교환 95건 5.3% · 사고 41건 10.4%. 즉 오차가 큰 건 '사고'뿐이고 단순교환은 무사고와 같은 수준인데, 현재 이진 특징 accident_free=(무사고)는 단순교환을 사고와 한 묶음으로 넣는다 — 3단계 특징(또는 사고만 1인 이진)으로 바꿀 근거. DW-864 1,000건 검증 때 전후 비교로 채택 결정.
status: done 2026-09-13
resolution: TabPFN 10번째 특징을 3단계 서열(무사고 2·단순교환 1·사고 0, accident_status NULL이면 종전 이진으로 폴백)로 개정 — api/app/market_price.py `_ACCIDENT_LEVEL`·`_accident_level`, `_COMP_COLUMNS`에 accident_status 추가. 같은 300건 LOO 3회(.logs/encar_eval/20260913_f/summary.md, encar_eval DB에 0038 적용 후): 기준선(현 운영 이진)=09-05 수치와 동일(전체 5.8%/±10% 69.7). 3단계: 사고 10.4→9.7%, ±10% 46.3→51.2, ±20% 78.0→87.8, q10~q90 65.9→73.2(개선 34·악화 7); 단순교환 5.3→5.9%(±10% 73.7 동일, 개선 35·악화 59); 무사고 5.2→5.3%; 전체 5.8→5.8%, ±20% 94.3→95.3, 판정 일치 89.7%. 대안 '사고만 구분 이진'(무사고·단순교환 같은 값)은 전체 6.4%·단순교환 6.8%·판정 일치 73%로 탈락 — 단순교환 가격은 무사고보다 사고 쪽에 가깝게 움직인다는 뜻. 채택 근거: 사고 개선 폭 > 단순교환 손실, 전체 지표 유지. 검증: 단위(test_accident_level…, 이진으로 되돌리면 red) 679 passed, 불변식 스윕(verify_market_engine --sample 40, 로컬 7,239건) 위반 0. 비교군 사다리 0·1단의 '무사고 = / <>' 조건은 손대지 않았다(학습표 선택도 사고 정보 미사용). 남은 것: 단순교환 손실(+0.6pp)과 사고 3단계를 비교군 조건에도 적용할지는 DW-864 1,000건에서 함께.

### DW-864: 시세 엔진 최종 검증을 300건이 아닌 약 1,000건(새 표본)으로 한 번 더 돌린다 — 최종 시연 점검표·시연 시나리오·포트폴리오 PDF 전에

origin: 사용자 결정(2026-09-08) — 4단계 300건 검증표 검토 후
location: api/scripts/build_eval_db.py(--split-per-target·--seed), api/scripts/eval_market_price_encar.py, .logs/encar_eval/20260905/(실매물 7,081건, 삭제 금지)
severity: medium
...
update(2026-09-10, 사람 판정 50건 대조 — report.md §9): 5단 정확 일치 52%, 한 칸 이내 90%, 3단 62%. 불일치는 사람이 더 싸다고 본 쪽이 16 대 8로 치우침(사람 근거: 렌트·영업 이력 없는 매물 기준 범위). 1,000건 검증 설계에 넣을 것: (a) 수집 데이터의 usageChangeTypes(렌트·영업 이력)로 층을 나눠 오차·판정 편향 비교, (b) 사람 판정 표본을 100건으로 늘려 판정 경계(q25/q75) 조정 근거 확보, (c) 두 칸 이상 어긋난 5건 유형 재확인.
status: done 2026-09-13
resolution: 2026-09-13 새 표본 955건(이전 300건 제외, seed 20260913, 세대당 최대 67; 작은 두 세대 17·67) LOO를 F 직전 커밋(사고 이진)과 94d0de2(3단계) 두 코드로 실행(.logs/encar_eval/20260913_i/summary.md·analysis.txt·verification_955.xlsx). 3단계: MdAPE 6.6%(95% 6.1~7.1) · ±10% 68.9%(65.9~71.8) · ±20% 93.4% · q10~q90 82.7% · 사고 9.8→8.1%(개선 82·악화 45) · 단순교환 6.2→6.6%(141·176) · 판정 일치 87.1% → DW-863 채택 유지. 300건 5.8%는 955건 구간 밖(쉬운 표본) — 대표 수치는 955건. 세대별 4.9~10.0%(300건 20건 추정과 최대 5pp 차이 = 이 항목의 근거 확인). (a) 렌트 이력 층: MdAPE 차이 없음(6.3 vs 6.7)이나 부호 편향 +3.3% vs +0.3%(예측>실제 64% vs 51%) — 사람 판정 16:8 치우침의 렌트 가설 확인 → DW-880. (b) 사람 판정 100건은 사용자 작업이라 DW-879로 분리(verification_955.xlsx '사람판정' 시트). (c) 두 칸 이상 어긋난 5건 재확인은 (b)와 함께. hold-out은 300건에서 차이 0.9%라 생략(trigger대로). 소요 건당 12초·2프로세스 병렬 5.5시간.
...
resolution: 지역·색상 인자 신설(f8bf999), 차종 별칭·복수값·models 목록·seats_min(6649404), 질문에 명시된 연료·차종·제조사·좌석을 도구 인자에 자동 주입(f6fdaa3, 부정어 보호). 3차 재검증(.logs/chatbot_eval/20260909c, 같은 100건, 판정기 v3): 검색 GOOD 37%→97%, constraint_violated 19→1(잔여 1건은 '대형 SUV' — body_type 어휘에 SUV 크기 축 없음, DW-872 resolution 참조), bad_tool_args 7→0. 멀티턴 참조 잔여는 DW-855에서 계속.

### DW-866: 운영(시연) DB 시드 매물 가격이 현실과 동떨어진 건이 있다(예: 쉐보레 크루즈 2천만원) — 보정된 3개 모델군 외 매물의 가격 수준 점검

origin: 사용자 지적(2026-09-08)
location: supabase/seed-local/data/listings.json(운영 스냅샷), api/scripts/seed_depreciation.py(아반떼·쏘렌토·그랜저만 엔카 실측 중앙값에 보정), 그 외 제조사·모델 매물(구 시드)
severity: low
reason: 2026-08-30 보정은 3개 모델군(17그룹)만 실측에 맞췄고 나머지 모델(쉐보레·르노·수입차 등)은 초기 시드 값 그대로라 시세와 어긋난다. 시연 현실감 문제이며 검증(실매물 기반)과는 무관. 사용자 결정으로 **가장 나중**에 본다.
trigger: 포트폴리오 PDF 이후, 시드 v4(DW-860 (2)) 결정과 함께 — (1) 운영 매물 전수(166건)의 모델별 가격을 엔카 실측 중앙값(2단계 표 또는 소량 재수집)과 대조해 ±30% 밖 건 목록화, (2) 보정 또는 삭제 결정은 사용자 승인(운영 데이터 변경).
status: open

### DW-867: 지역(region) 정보를 매물 등록 화면에서 입력하거나 검색에서 조건으로 쓸 수 있는 UI가 없다(사용자 지적) — 등록 폼·검색 필터·AI 에이전트 파라미터 세 곳 점검

origin: 사용자 지적(2026-09-08), 챗봇 검증 v2에서 지역 조건 위반 7건(DW-865)과 같은 축
location: web/src/app/(user)/sell/SellForm.tsx(등록 폼), web/src/app/(user)/search/page.tsx(검색 필터), app/(Flutter 동일 화면), api/app/graph/agent_tools.py(search_listings region 파라미터), listings.region(NOT NULL, 17개 CHECK)
severity: medium
reason: DB엔 region 열(17개 시·도)이 필수로 있고 시드엔 값이 채워져 있으나, 사용자 관찰로는 등록 폼에 입력 칸이 없고 검색에도 지역 필터가 없다. 에이전트 도구엔 region 파라미터가 있지만 프롬프트가 안 넘긴다(DW-865). 등록·검색·AI 셋을 같이 봐야 한 축으로 맞는다. 착수 시 실제 화면에서 존재 여부를 먼저 실측한다(추측 금지).
trigger: DW-865 수정 때 함께 — (1) 등록 폼에 지역 선택(17개 CHECK 목록과 동일) 추가 여부 확인·추가, (2) 검색 페이지 지역 필터, (3) 에이전트가 region 파라미터를 쓰도록 수정 후 챗봇 검증 100건 재실행. 웹·앱 동시.
status: open

### DW-868: 에이전트가 예산을 원 단위로 오독해("1500이하"→price_max=1500원) 매물이 있는데도 "조건에 맞는 매물 없음"이라 답한다 + 제조사 별칭(쌍용→KG모빌리티) 미매핑

origin: 챗봇 답변 검증 v3(2026-09-09) — 0건 응답 4건을 로컬 시드 DB SQL로 검산
location: api/app/graph/agent.py(_SYSTEM_PROMPT 단위 규칙), api/app/graph/agent_tools.py(search_listings price_min/max·manufacturer 인자), 검증 데이터 .logs/chatbot_eval/20260908/
severity: high
...
origin: 챗봇 답변 검증(2026-09-08~09) 사람 라벨 — 시세 10건 중 4건 + 멀티턴 2건에서 사용자가 지적
location: api/app/graph/agent.py(_SYSTEM_PROMPT 시세 설명 규칙), api/app/graph/agent_tools.py(_format_market_diagnosis: sample_count는 넘기지만 주의 지시 없음)
severity: medium
...
origin: 챗봇 답변 검증(2026-09-08) — C51·C53
location: api/app/graph/agent.py(최종 답변 생성 지시), agent_tools.py(_format_listing_summary·compare 텍스트에 id 포함)
severity: low
...
origin: 챗봇 답변 검증(2026-09-08) 사람 라벨 — C61·C67
location: api/app/graph/agent.py(_SYSTEM_PROMPT), api/corpus(가이드 문서: 일반 지식과 제품 기능을 구분하지 않음)
severity: medium
...
update(2026-09-09 저녁, 사용자 재검토 + 수정 후 실측 추가 근거): (1) 사람 라벨 12건 재검토 — 멀티턴 C46·C48·C50(직전 목록 무시 재검색)·C51·C53(id 노출)·C47·C79(표본 2건 단정) 7건을 사용자 결정으로 GOOD→BAD 반영(.logs/chatbot_eval/20260908/human_labels.json·rereview_20260909.json, 원본은 backup_pre_rereview_20260909/), 사람 vs 판정기 v3 일치율 85%→92%; 검색 C01·C03·C18·C28·C29는 근거 제시 후 사용자 결정 대기(초안 rereview_20260909.json의 pending_user_decision). (2) 수정 전 검색 5건을 로컬 시드 DB 정답집합과 대조: C28은 SUV 2대(셀토스·XM3)를 준중형으로 추천하고 준중형차 크루즈 2세대를 빠뜨린 실제 오답, C01·C03·C18·C29는 결과는 맞았으나 조건이 query_text(정렬용)에만 실려 모집단이 20건 상한 아래(14·4·16건)였거나 유사도 상위 20 안에 우연히 다 들어와(C29: 65건 중 K3/아반떼 9건) 맞은 것. (3) 수정 후(20260909) C18 신규 결함: '세단'을 body_type='중형차' 하나로 좁혀 대형차인 K8·E250(비흡연, 3천만원대)을 빠뜨림(정답 4건 중 2건만) — 판정기 v3는 DB를 모르니 GOOD으로 통과시킴. trigger (2)에 `body_types: list[str]`(OR) 또는 '세단'→IN(준중형차,중형차,대형차) 매핑을 추가한다. (4) Gemini 크레딧 충전(2026-09-09) → 회귀 31건·100건 재배치·판정 v3를 .logs/chatbot_eval/20260909b/driver.sh로 순차 실행(결과는 다음 update에).
update(2026-09-09, 이어서): 검색 5건(C01·C03·C18·C28·C29)도 사용자 확인으로 GOOD→BAD 반영 → 사람 라벨 GOOD 61·BAD 39, 사람 vs 판정기 v3 일치율 97%(범주별 검색 97·멀티턴 100·시세 100·가이드 93·경계 80). 남은 불일치 3건은 C27(운영/로컬 데이터 드리프트, DW-866)·C61·C99(판정기 한계)로 재검토 대상 아님. 엑셀 두 파일(사용자 표·v3 대조표) 셀 단위 diff로 12행 외 무변경 확인.
update(2026-09-09 17:35, 재검증 2차 시도 결과): 사용자가 크레딧을 충전한 뒤 driver.sh로 회귀 31건을 돌렸으나 2 PASS/29 FAIL(소요 1,036초) — 29건 전부 gemini-3.1-flash-lite 429 RESOURCE_EXHAUSTED 'Your project has exceeded its monthly spending cap'(월 지출 한도, 크레딧 잔액과 별개 설정). 드라이버가 이 문자열을 감지해 100건 배치·판정은 시작하지 않음(비용 0). 로그 .logs/chatbot_eval/20260909b/regression.log. 다음: 사용자가 AI Studio/Cloud Billing에서 해당 프로젝트의 월 지출 한도를 올리거나 해제한 뒤 driver.sh 재실행(회귀부터).
update(2026-09-09 18:08, 재검증 2차 완료 — 월 지출 한도 해제 후 driver.sh 재실행, .logs/chatbot_eval/20260909b/): 회귀 31건 28 PASS/3 FAIL(S15=로컬 시드에 쏘렌토 UM 없음(운영엔 13대), S16=조건 전부 만족하는 차가 로컬·운영 모두 0대라 되묻기가 정답(예전 통과는 느슨한 검색의 가짜 통과 — 기대값을 clarify_ok로 바꿀지 사용자 결정), MT6='세단' 어휘 부재로 1턴 0건). 100건 판정 v3: 전체 88%(1차와 동일; 검색 73→77, 멀티턴 73→67), constraint_violated 8→6, wrong_reference 3→5. 가드 보강은 작동(C50·C58 2턴 재검색이 코드에서 차단됨). 새로 확인된 결함: (a) 결과 줄에 사고·색상·지역이 없어 재검색이 막히면 에이전트가 "모두 무사고"라고 지어냄(C50, 실제 스파크 1대 단순교환), (b) '여기서 흰색만'은 가드 지칭 패턴 밖이라 재검색(C59), (c) 카드로 보여준 5대 밖의 도구 결과(6대째)를 직전 목록으로 취급(C51), (d) 재검색이 막히자 '목록에 없다'고 답함(C53, 시세는 market_price_stats(id)로 가야 함), (e) '세단' 오류 후 되묻기·중형차 축소·차종 누락(C06·C48·C18·C15), (f) 'SUV'·'하이브리드'를 여전히 query_text에만(C02·C03). 조치: 수정 묶음 2(코드 강제) 진행 중 — 차종 별칭·복수값, models·seats_min, '르노', 결과 줄 속성, 가드 패턴 보강('여기서'·'이 매물들' 등), 거절 문구에 compare_listings/market_price_stats 안내, 질문에 명시된 연료·차종·제조사·좌석 조건을 도구 인자에 자동 주입(부정어 '말고/제외/빼고' 보호). 완료 후 같은 100건·회귀 31건 재실행(3차).
update(2026-09-09 저녁, 수정 묶음 2·3 커밋): 묶음 2 = 6649404(차종 별칭·복수값 '세단'→준중형차·중형차·대형차 ANY, models 목록 OR, seats_min, '르노' 별칭, 결과 줄 사고·색상·지역, 거절 문구 compare_listings 안내; 로컬 DB 실측 세단 18건·모델목록 9건·7인승 SUV 9건). 묶음 3 = 다음 커밋(가드 지칭어 11개, 직전 목록 id 명시, 시세→market_price_stats 안내, 질문 명시 조건 자동 주입 infer_missing_args — 부정어 '말고/빼고/제외' 보호, '삼성' 단독은 지명 오탐 때문에 자동 주입 어휘에서 제외). 단위 658 통과(+26). 남은 한계: 직전 목록 id 명시는 프롬프트 보강이라 하드 필터는 아님(최종 선택 id를 직전 목록과 교집합하는 코드 강제는 미구현). 3차 재검증(.logs/chatbot_eval/20260909c/)으로 확인.
status: done 2026-09-09
resolution: 수정 묶음 2(6649404)·3(f6fdaa3) 완료. 3차 재검증(.logs/chatbot_eval/20260909c): 전체 GOOD 62→93%(검색 97·멀티턴 73·가이드 87·시세/거절/경계/추천 100), constraint_violated 1, bad_tool_args 0, wrong_reference 4(→DW-855). 회귀 31건 28 PASS — MT5·MT6 통과; S15(로컬 시드에 쏘렌토 UM 없음, 운영 13대)·S16(조건 전부 만족 0대 → 되묻기가 정답)·MT3(로컬에 쏘렌토 하이브리드 0대, 운영 13대 → 연료 조건을 제대로 걸자 0건)는 회귀 질의셋이 운영 시드를 전제한 데이터 의존 — 기대값 수정 또는 로컬 시드 정비는 사용자 결정. 미해결 한계: '대형/중형 SUV' 크기 구분은 데이터 모델에 축이 없어(C26) 시드 v4(DW-860) 검토 시 함께. 가이드 C63·C66은 이전 3회 GOOD → 모델 비결정성으로 분류(추적만).

### DW-873: 가이드(구매 지식) 질문 15건 중 13건이 search_guides를 부르지 않음 — 답이 가이드 코퍼스·리랭커 경로를 거치지 않고 모델 자체 지식으로 나옴, 판정기는 질의셋 expected와 어긋나게 채점

origin: 3차 재검증 가이드 GOOD 15→13 원인 분석(2026-09-10, .logs/chatbot_eval/20260909c) — C63(침수차 팁: "안내해 드렸습니다"라고만 하고 내용 없음)·C66(리스 승계: 범위 밖 안내 후 유도). 4회 실행(20260908·09·09b·09c) 모두 가이드 15건 중 search_guides 호출은 2건뿐
location: api/app/graph/agent.py(_SYSTEM_PROMPT — 지식 질문에 search_guides를 먼저 부르라는 규칙이 없거나 약함; 도구 디스패치 루프), api/scripts/chatbot_eval/judge.py:226-227(expected를 전달하지만 판정 규칙에서 우선하지 않음), api/scripts/chatbot_eval/queries.json(C63·C66 expected = "범위 밖 안내 + 대안 유도")
severity: medium
reason: (1) 가이드 답이 코퍼스(guide_documents 32절, 침수는 3절에 언급·리스/렌트는 없음)에 근거하지 않아 리랭커 경로가 실제 트래픽에서 거의 안 쓰이고, 답 품질이 모델 변동에 흔들린다(3차 C63은 팁을 준다고 말만 하고 내용이 빠짐). 문서 05가 설명하는 "가이드 검색→리랭커 재정렬" 흐름이 실측으로는 가이드 질문의 13%에서만 돈다. (2) C66의 3차 답("금융 계약 승계는 범위 밖, 조건을 말해 주세요")은 queries.json expected와 일치하는데 판정기 v3는 over_refused로 BAD — 판정기가 expected보다 자기 기준을 우선한 오판. 2차의 C63·C66 GOOD은 모델이 자기 지식으로 팁을 쓴 결과라 ungrounded인데 통과.
trigger: (1) 지식 질문(매물 조건 없음 + 지식 키워드 또는 라우터의 가이드 분류)이면 코드가 search_guides를 먼저 호출해 결과를 컨텍스트로 넣고, 코퍼스에 없으면 "범위 밖" 안내로 유도(자동 주입과 같은 자리, DW-872 방식). (2) judge 프롬프트에 "expected와 일치하면 GOOD 우선" 규칙 명시(판정기 v4) 후 사람 라벨 100건과 일치율 재확인(현재 97%). (3) 가이드 15건 ×2회 재실행으로 search_guides 호출율(목표 ≥13/15)·GOOD율 확인. 시점: DW-864(1,000건) 전후 어느 쪽이든 되나, 포트폴리오에 "리랭커 경로" 수치를 쓰기 전에는 반드시.
status: done 2026-09-12
resolution: agent.py:450-476 — 첫 스텝 응답에 도구 호출이 없으면(키워드 판별 없이 이 행동 자체가 트리거) 코드가 search_guides(원문 질의)를 강제 실행, 가이드 1건 이상이면 ToolMessage로 넣고 LLM 재호출, 0건이면 첫 답 유지(NO_GUIDES_FOUND_TEXT 상수, agent_tools.py). 판정기 v4 = api/scripts/chatbot_eval/judge_v4.py(v3 + 'expected 1순위' 한 문단, v3 보존). 재실행(.logs/chatbot_eval/20260912): 가이드 15건 2회 모두 search_guides 15/15(이전 4회 2/15), GOOD 13/15·11/15, C66 두 번 다 GOOD; 거절10+경계5 중 9건에 강제 호출이 끼었으나 15/15 GOOD; 회귀 28/31(S15·S16·MT3 기존 데이터 의존 그대로, 셋 다 첫 스텝에 search_listings를 불러 이번 로직 미발동). 단위 664 passed; 강제 호출을 `if False:`로 끄면 test_agent_loop 2 failed(red) → 원복 green. 한계: (c) '이미 도구 부른 턴 미적용' 테스트는 로직을 지워도 통과한다. 남은 BAD의 원인(게이트가 주제 밖 문서 통과·정직성·되묻기)은 DW-876.

### DW-874: TabPFN '세대' 특징이 실제로는 model 문자열 전체의 코드라, 표기가 제각각인 운영 시드에서 같은 세대가 다른 값이 되거나 세대 정보가 아예 없다

origin: 사용자 질문(2026-09-12, "모델과 세대를 어떻게 구분해서 넣나") 확인 중 운영 시드 model 표기 조회
location: api/app/market_price.py:156(_base_model — 모델군은 '더 뉴/올 뉴' 뗀 첫 토큰, 학습표 SQL의 ILIKE 필터로만 쓰임), :321(_tabpfn_features_with_model — 11번째 칸 = codes[row["model"]]), :360(codes = model 문자열 전체를 정렬해 붙인 정수). 데이터: listings.model 한 열에 모델군·세대·부분변경('더 뉴')·배기량 트림('3.3')·연료 접미('하이브리드')가 섞여 있고 세대 전용 열이 없다.
severity: medium
...
origin: 사용자 결정(2026-09-12, 포트폴리오 전 기능 보완 범위 확정 대화)
location: n/a (각 항목의 location 참조)
severity: low
reason: 시연·포트폴리오 핵심 기능(시세 엔진·챗봇·시세 카드)과 무관하거나 드문 상황(관리자 정지 경계·앱/웹 화면 차이 등)이고 주간 토큰이 부족해 사용자가 이번 범위에서 제외. 대상 — 선택: DW-843·844(가이드 코퍼스 확대)·846(웹 카드 사진 회색)·850(/ai 첫 입력 무시)·852(요청당 ~8초)·867(지역 입력 UI); 이전 에픽 잔여: DW-800~842 중 status: open 전부. 각 항목은 open 그대로 두고 여기서 일괄 이월만 기록한다. 포트폴리오 PDF에는 "알려진 제약"으로 기재.
trigger: 포트폴리오 PDF 완료 후 별도 정리 스프린트에서 이 목록부터 연다. 그 전에 시연 중 DW-846·850이 재현되면 그 항목만 앞당긴다.
status: open

### DW-876: 가이드 게이트가 주제 밖 문서도 통과시켜 "가이드 찾음"이 "관련 가이드 있음"을 뜻하지 않는다 — 코퍼스에 없는 주제(침수·명의이전·리스 승계·보증)에 엉뚱한 가이드가 붙고, 답변 정직성 결함 2건·과도한 되묻기 1건이 남음

origin: DW-873 재실행 분석(2026-09-12, .logs/chatbot_eval/20260912 guide_run1·2 + 가이드 15건 search_guides 직접 호출)
location: api/app/graph/doc_rag_node.py(상대 게이트 _GUIDE_TOP_K·_GUIDE_MARGIN·_GUIDE_DISTANCE_CEILING=0.45), api/app/graph/multi_query.py(find_relevant_guides_fused), api/app/graph/agent.py:450-476(강제 호출 — 가이드 1건 이상이면 무조건 재답변)
severity: medium
reason: 가이드 15건 직접 호출 결과 15/15가 "찾음"이지만 주제 일치는 일부뿐 — C65 "명의이전 절차"→[전기차 충전·보조금] 가이드, C62 "무사고 vs 단순교환"→[예산·가성비] 가이드, 대부분이 [중고차 신뢰성과 구매 체크포인트] 한 문서로 몰림. 질의셋 expected가 C62·C63을 "범위 밖 안내"로 둔 것은 코퍼스에 해당 주제 문서가 없다는 사실과 맞다. 결과: C62 모델 자체 지식으로 상세 설명(판정기: 성능점검상 단순교환도 무사고로 분류된다는 사실과 어긋남), C63 "자가 점검 팁을 안내했다"며 실제 내용 없음(2회 모두), C67 도구 출력에 없는 "성능점검을 마쳤다" 단정, C71 '세단' 조건이 있는데 예산·제조사 되묻기(2회 모두). 포트폴리오 전 범위(DW-855와 같은 챗봇 재검증 회차)로 사용자 결정(2026-09-12) 안에 있으나 DW-873 수정 범위 밖이라 분리. 거리 실측(2026-09-12, 로컬 코퍼스): '명의이전 절차가 복잡한가요?' 최근접 0.390 [전기차 충전·보조금 — 보조금과 의무운행], '무사고 vs 단순교환' 최근접 0.325 [예산·가성비], '침수차 구별 팁' 최근접 0.301 [신뢰성 체크포인트] — 상한 0.45라 전부 통과하고, 주제가 맞는 문서와 안 맞는 문서의 거리가 0.30~0.39로 겹쳐 상한 조정만으로는 가르기 어렵다.
trigger: DW-855 작업 회차에서 함께 — (a) 게이트 관련도 기준 강화(리랭커 점수 하한 등) 또는 강제 호출 결과가 주제 밖이면 "범위 밖" 안내로 유도, 가이드 15건 재실행으로 C62·C63·C65 확인 (b) 정직성: 답변이 "안내했다/확인했다"고 말하는데 근거가 도구 출력에 없으면 잡는 규칙 (c) C71 조건 하나라도 있으면 검색 먼저. 코퍼스 보강(침수·명의이전 문서 추가)은 DW-843(이월, DW-875)과 묶어 사용자에게 묻는다.
update(2026-09-14, H 회차): 리랭커 점수 하한은 불가(로컬에 리랭커 없음, 벡터 거리로는 주제 일치·불일치 겹침 재확인) → 어휘 근거 게이트(aeba16a, agent_tools `_guides_lexically_grounded`: 질의 내용어가 가이드 본문에 하나도 없으면 NO_GUIDES) + 0건이면 범위 밖 안내 조기 반환. 라이브 100건: 가이드 13→14/15(C63 침수 해결, C62·C65 유지), 그러나 조기 반환이 추천 질의(C36 '드라이브 좋은 차')까지 거절하고 C71(주행거리+세단)은 모델이 search_guides만 불러 강제 검색 분기가 안 걸림 → 2차 d95d135: 조기 반환은 매물 의도(has_listing_intent) 없을 때만, 의도 있는데 search_listings 미호출이면 사후 강제. 정직성(b) 규칙은 게이트가 근거 없는 문서를 걸러 간접 해소(별도 규칙 없음). 코퍼스 보강(침수·명의이전 문서)은 DW-843/875 그대로.
status: done 2026-09-14
resolution: H 회차: 어휘 근거 게이트(질의 내용어가 가이드 본문에 없으면 NO_GUIDES → 매물 의도 없을 때만 범위 밖 안내, 있으면 search_listings 강제)로 C63(침수)·C36 해결. 같은 판정기 4차 가이드 13/15(1차 13/15와 동수 — C62는 코퍼스가 사고 등급을 언급해 게이트가 통과시키고 모델이 설명함(expected는 범위 밖, 데이터 정의 문제), C71은 DW-883 유형). 리랭커 점수 하한은 로컬에 리랭커가 없고 벡터 거리로 못 갈라 미채택(실측). 정직성 규칙(b)은 별도 없음 — C67·C74 '성능점검' 언급은 잔존(모델 프롬프트 수준, 추적만). 코퍼스 보강은 DW-843/875.

### DW-877: 판정기가 같은 답변에 다른 판정을 낸다 — temperature=0인데도 재채점에서 2/15가 뒤집혀 GOOD율이 13/15와 11/15로 갈렸다

origin: DW-873 재실행 대조(2026-09-12, .logs/chatbot_eval/20260912 guide_run1 vs guide_run2)
location: api/scripts/chatbot_eval/judge_v4.py:287(ChatGoogleGenerativeAI temperature=0), judge.py(v3 동일 구조)
severity: medium
...
update(2026-09-14, 실측): 같은 응답 100건(20260909c)을 판정기 v4로 재채점 — 원판정(v3) 대비 5건 차이(C26·C56·C66·C67·C71, GOOD 93→92), 2회차는 33건에서 한도로 중단됐고 그 33건 중 1건(C33) 차이. 즉 채점 잡음 ≈ 100건당 3~5건(±5%p). 3회 다수결은 판정 비용(pro 모델, 1회 ≈ 전체 실행비의 대부분)이 예산(3,000원)을 넘어 도입하지 않음(사용자 결정 2026-09-14, 1회 채점 유지).
status: done 2026-09-14
resolution: 잡음 폭을 실측해 문서 05 5절(b)에 '채점 잡음 ±5건(100건당)'으로 명시하고, GOOD율 비교는 5건 이내 차이를 '변화 없음'으로 읽는 규칙을 둔다. 다수결 채점은 비용상 미도입 — 뒤집힌 사례의 공통점(C66·C67·C71: 되묻기 vs 범위 밖 안내의 경계, C26: 대형/중형 SUV 어휘)은 루브릭의 'over_refused' 경계가 흔들리는 것으로, 기준 문장 보강은 다음 판정기 손볼 때.
...
### DW-879: 사람 판정 100건 대조(5단 판정 경계 q25/q75 조정 근거) — 사용자 작업, 955건 검증표의 '사람판정' 시트

origin: DW-864 (b)·(c) 분리(2026-09-13) — 50건 대조는 정확 일치 52%·한 칸 이내 90%(report.md §9)
location: .logs/encar_eval/20260913_i/verification_955.xlsx('사람판정' 시트, 빈칸), api/app/market_price.py(_verdict_by_quantiles 경계), .logs/encar_eval/20260905/eval/verification_human.xlsx(50건 원본)
severity: low
reason: 판정 경계(q25/q75)를 사람 감각에 맞춰 조정하려면 50건은 부족(정확 일치 52%의 95% 구간이 ±14%p). 50건에서 두 칸 이상 어긋난 5건 유형도 재확인 안 됨. 렌트 이력 편향(DW-880)이 그 불일치의 일부를 설명하므로 DW-880 결정 뒤에 하는 게 효율적.
trigger: 사용자가 시트 100건을 채우면 → Agreement%·불일치 방향·렌트 이력 층별 재집계 → 경계 조정 여부 결정. 포트폴리오 PDF 전 필수는 아님(선택).
status: open

### DW-880: 렌트 이력이 있는 차를 시세 엔진이 실제 호가보다 약 3.3% 높게 본다 — listings에 이력 열이 없어 특징으로 넣을 수 없음

origin: DW-864 955건 재검증(2026-09-13) 부호 편향 층별 분석(.logs/encar_eval/20260913_i/summary.md)
location: api/app/market_price.py(_tabpfn_features 10칸 — 이력 특징 없음), supabase/migrations(listings에 usage/rent 열 없음), .logs/encar_eval/20260905/listings_eval.csv(usage_change 열: 렌트 2,203·영업용 2·없음 4,876 — 수집 데이터에는 있음)
severity: medium
reason: (예측−실제)/실제 중앙값이 렌트 이력 330건 +3.3%(예측>실제 64%) vs 없음 625건 +0.3%(51%). 오차 크기(MdAPE)는 6.3 vs 6.7로 차이 없지만 방향이 한쪽으로 쏠려 렌트 차엔 '적정'이 실제로는 '다소 높음'인 경우가 많다. 사람 판정 50건에서 사람이 더 싸게 본 쪽 16:8 치우침의 가설(렌트·영업 이력)을 데이터로 확인. 단순교환 +2.2%·사고 +3.1%·1,500만 미만 +4.7%도 같은 방향(예측 과대)이나 이력 편향과 별개 축.
trigger: 사용자 결정 — (a) 시드 v4(DW-860)·등록 폼에 '렌트·영업 이력' nullable 열 추가(마이그레이션 더하기만) + 엔카 로더가 usage_change를 채움 + 특징 12칸째로 추가 → 955건 재측정; (b) 열 없이 두고 문서 05·시연에서 한계로만 설명. 판정 경계 조정(DW-879)은 이 결정 뒤에.
update(2026-09-14, 측정 완료·사용자 결정=후순위): 열은 추가했다 — 0039 `listings.usage_history`(없음/렌트/영업용, 엔카 용어 '용도변경 이력' usageChangeTypes 코드 1=렌트·2=영업용, 현재 택시 매물은 수집 때 제외돼 영업용 2건뿐) 운영 적용, 로더 `--backfill-usage`로 운영 엔카 7,081건 채움(렌트 2,203·없음 4,876·영업용 2, 멱등), 커밋 3dac116. 엔진 변형 2종을 같은 955건으로 측정(.logs/encar_eval/20260913_i/): (a) 12번째 특징 0/1 → 렌트 편향 +3.3→+2.1%이나 전체 MdAPE 6.6→6.8·사고 8.1→8.7·판정 일치 84.8%; (b) 학습표 정렬 3순위에 이력 일치(277건 중간) → 렌트 차 9.3→7.7이나 이력 없는 차 7.4→8.0, 전체 제자리, 판정 일치 74.7% — 중단. 둘 다 전체를 못 올려 **엔진은 3단계(94d0de2 상태)로 되돌림**(특징·정렬 미채택). 사용자 결정(2026-09-14): 렌트 효과가 눈에 띄는 크기가 아니라 후순위 — 추후 논의.
trigger(재지정): 사용자가 다시 꺼낼 때 — 후보는 (c) 편향만 사후 보정(렌트 이력 대상이면 예측 분위수를 세대별 회귀 계수(−2~−13%, 중앙 −6.9%)만큼 내림, 학습표는 그대로) 또는 (d) 카드에 '렌트 이력' 표시만. 데이터·열은 준비돼 있어 코드만 바꾸면 955건으로 즉시 재측정 가능(2시간).
status: done 2026-09-14
resolution: 2026-09-14 사용자 결정(정정) — 렌트 편향을 줄이는 특징 버전(3dac116 엔진, 12칸: 11번째 `_usage_flag` 렌트·영업용=1)을 채택. 955건: 렌트 차 편향 +3.3→+2.1%(예측>실제 64→59%), 전체 MdAPE 6.6→6.8%(95% 구간 6.2~7.2 안), ±10% 69.0 동일, ±20% 93.4→93.5, 사고 8.1→8.7·단순교환 6.6→7.0. 학습표 정렬 변형(b)은 미채택. 사용자 취지: 편향 축소는 취하되 이 속성이 가격 판정을 눈에 띄게 좌우하진 않게 — 특징은 모델이 데이터로 비중을 정하므로 별도 가중치 없음. 열·운영 데이터·로더는 3dac116, 문서 05 3절 특징표·5절(a′) 대표 수치 갱신. 등록 폼 입력란(웹·앱)은 DW-881로 분리.

### DW-881: 매물 등록·수정 폼(웹·앱)에 '용도변경 이력'(usage_history) 입력란이 없다 — 사용자 매물은 항상 NULL(미입력)

origin: DW-880 채택(2026-09-14) — 열·엔진 특징은 들어갔으나 등록 경로가 값을 못 채움
location: web/src/app/(seller)/sell 폼·app/lib/features/listings/sell_screen.dart(accident_status 입력란 옆), supabase/migrations/0011(anon 컬럼 화이트리스트 — 카드·상세 표시하려면 anon SELECT GRANT 필요, §9.3 (b) 사용자 승인 사항)
severity: low
reason: 엔진은 NULL을 '없음'과 같이 0으로 보므로 사용자 매물은 종전과 동일하게 동작(회귀 없음). 다만 렌트 이력이 있는 사용자 매물도 '없음'으로 예측돼 +3% 높게 볼 수 있고, 카드·상세에 이력을 보여주지도 않는다.
trigger: G(DW-862 카드 개편) 또는 시드 v4 때 — 폼에 3값 셀렉트(없음/렌트/영업용, 기본 미선택=NULL) + 카드·상세 표시(anon GRANT는 승인 후 0040으로). 웹 E2E 1건·앱 위젯 테스트 1건.
status: open

### DW-882: 엔카 7,081건 적재 후 선택적 조건 검색(search_listings)이 HNSW 사후 필터 때문에 0건을 돌려준다 — 운영 API는 수정 코드 미배포 상태

origin: H 2차(2026-09-14) C60 "쏘나타나 K5 중에 무사고" 0건 실측 — 로컬 DB에 조건 맞는 매물 6건이 있는데 벡터 ORDER BY를 얹으면 0건
location: api/app/graph/agent_tools.py(search_listings similarity 분기 → `_run_similarity_select`: 트랜잭션 안 `SET LOCAL hnsw.iterative_scan = relaxed_order`), supabase/migrations(listings_embedding_hnsw m=16 ef_construction=200), 운영 pgvector 0.8.0(iterative_scan 지원, 기본 off — 2026-09-14 읽기 전용 확인)
severity: high
...
origin: H 회차 4차 잔여 3건(2026-09-14, .logs/chatbot_eval/20260914_h/run4) — 3차(25de00b)에서 '검색 결과 있으면 상위 5건 반드시 제시' 재확정을 넣자 C37·C39가 도구 결과에 없는 매물을 본문에 지어내 4차에서 제거
location: api/app/graph/agent.py(_finalize — 기존 '찾았다 서술+빈 목록' 재시도만 남음), 시스템 프롬프트 되묻기 규칙
severity: low
reason: 모호한 추천 질의("골프백 실을 수 있는 차", "사고 이력 없는 차", "적당한 주행거리 세단")에서 모델이 search_listings 20건을 받고도 카드를 비우고 조건을 되묻는다. 판정기는 unhelpful. 3건/45(6.7%)이고 사용자에게 해롭지는 않으나 기대 행동(매물 제시)과 어긋난다.
trigger: 포트폴리오 뒤 정리 스프린트 — 접근 후보: (a) 재확정 지시를 '지어내지 말고 도구 결과의 id만'으로 좁히고 id 검증 실패 시 카드만 붙이는 대신 본문에 '아래 N건' 한 줄을 코드가 덧붙임, (b) 되묻기 허용 조건을 '결과 0건 또는 20건 상한 도달'로 제한. 어느 쪽이든 45건 재실행(flash 판정, 소액)으로 확인.
status: open

### DW-884: 시세 카드의 '비슷한 차 실제 가격 점'이 비교군 중 가장 싼 60대만 찍혀 분포가 왼쪽으로 치우쳐 보인다

origin: 시세 카드 8건 캡처(2026-09-14, .../portfolio/shots/cards/02) — 쏘렌토 MQ4 HEV: 비교군 186대, AI 곡선 중심 3,150만인데 점은 2,450~2,800만에만 몰림
location: api/app/market_price.py(comps SELECT `ORDER BY price LIMIT MAX_COMPS(60)` — 산점도용 상한이 "가장 싼 60대"가 됨), web/src/components/ai/MarketDiagnosisPriceChart.tsx·app market_diagnosis_price_chart.dart(comps를 그대로 점으로 찍음)
severity: medium
reason: 비교군이 60대를 넘는 세대(엔카 적재 후 흔함)에서 점이 실제 분포가 아니라 하위 60대만 보여 "AI 곡선이 실제보다 높게 잡혔다"는 오해를 준다. 옛 산점도도 같은 데이터를 썼으나 이제 곡선과 나란히 놓여 어긋남이 눈에 띈다. 통계(중앙값·사분위)는 전체 비교군으로 계산돼 정확하고, 점만 편향.
trigger: 카드 곡선 개편(덱 5쪽 스타일 논의) 때 함께 — (a) comps를 가격순 상위 60이 아니라 전체에서 균등 표본(예: 분위수 간격) 또는 무작위 60으로 바꾸거나, (b) 점 대신 비교군 히스토그램/사분위 띠로 표현. API 응답 필드는 유지(더하기만).
status: done 2026-09-15
resolution: adccafc — comps LIMIT 60 제거, MAX_COMPS 500·초과 시 가격순 균등 표본(_sample_comps_evenly, 최저·최고 보존), 웹·앱은 점 전부 찍고 목록만 60건+외 N대. 운영 재캡처(cards_v2·v4): 쏘렌토 MQ4 HEV 186대가 2,100~3,800만 전 구간에 찍힘.

### DW-885: 시세 카드의 한 문장 판정("싼 차가 거의 없습니다")과 배지("다소 저렴")가 극단 구간에서 서로 다른 강도를 말한다

origin: 시세 카드 8건 캡처(2026-09-14, cards/07) — 그랜저 IG 2020 사고 191,618km 1,099만: 문장은 백분위 기준(열에 0 → "거의 없음"), 배지는 분위수 판정(q10 근처 → "다소 저렴")
location: web/src/components/ai/MarketDiagnosis.tsx(buildJudgementSentence: percentile 반올림 10분위), api/app/market_price.py(_verdict_by_quantiles: q10/q25/q75/q90 경계)
severity: low
reason: 두 값의 출처가 다르다 — 문장은 percentile(비교군 내 위치)을 10분위로 반올림, 배지는 TabPFN 분위수 경계. 백분위 0.05는 '거의 없음'이지만 가격이 q10 바로 안쪽이면 배지는 '다소 저렴'. 고객에겐 "거의 다 비싼데 왜 '다소'?"로 읽힌다.
trigger: 덱 5쪽 스타일(5색 구간) 적용 시 문장과 배지를 같은 축(분위수 구간)에서 만들고, 문장은 구간별 고정 문구 5개로. 웹·앱 동시.
status: done 2026-09-15
resolution: adccafc — tabpfn.cdf_at_price(분위수 5점 선형 보간) 추가, 웹·앱 한 문장 판정이 이 값으로 N=round(cdf×10)을 계산(없으면 percentile 폴백)해 배지와 같은 축. 운영 재캡처: 그랜저 IG '열에 둘+다소 저렴', 쏘렌토 '열에 일곱+적정' 일치. 조사 '하나가' 수정(078345e·fa0d365).
