...
severity: medium
reason: 재캡처 47/47에서 라우팅 오답 3건 중 2건이 같은 뿌리다. `S6 "제일 싼 차 뭐야?"` → **CLARIFY**(매물 0건, 되묻기 문구), 반면 `S7 "가장 비싼 매물 하나 보여줘"` → **SQL**(1건 정답). 멀티턴에서도 `M1.t2 "제일 싼 거 하나만 알려줘"`(앞선 두 턴에서 SUV·3천만원·서울로 좁힌 상태) → **CLARIFY**로 새서, 3턴 좁히기 사슬의 마지막 top-N 정확도가 관측되지 않는다. 라우터 프롬프트는 갈래를 "명시조건 유무 + 용도·느낌 유무"로만 가르는데 **최상급·정렬 표현(제일 싼/가장 비싼/최신)은 어느 쪽도 아니다** — "보여줘"류 동사가 붙으면 SQL로, 아니면 CLARIFY로 갈리는 것으로 보인다(같은 의미에 다른 경로 = 재현 가능한 불일치). 사용자 경험상으로도 답할 수 있는 질문에 되묻기를 돌려주는 쪽이 나쁘다.
trigger: 라우터 프롬프트에 최상급·정렬 표현을 SQL(구조형)로 명시하는 규칙·예시를 추가하고, `S6`·`M1.t2`가 SQL로 라우팅되는지 재캡처로 확인한다 → 다음 라우팅 손질 스토리의 인수조건으로 심는다(B8: 자리를 지정한다). 그 전까지 이 두 문항의 오답은 기준선(0.894/54·57)에 포함된 상태이며, 고치면 기준선이 **올라가는** 방향이라 13-8의 회귀 게이트를 막지 않는다.
status: open
...
trigger: DW-611의 라우터 손질과 같은 스토리에서 처리한다 — "아니 …로 바꿔줘"처럼 **명시 차종이 들어 있는 교체 요청**이 SQL로 가도록 규칙·예시를 보강하고, `M6.t1`이 SQL로 라우팅돼 오염 게이트가 실제로 발화 가능한 상태가 되는지 확인한다(발화 가능 = 일부러 오염 데이터를 넣었을 때 red가 되는지까지 본다, B4). 그 전까지 `M6`의 오염 커버리지는 없는 것으로 간주한다.
status: open
