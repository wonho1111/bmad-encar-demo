...
trigger: **`doc_rag_node`의 컷오프 또는 가이드 검색 방식을 다음에 손대는 스토리** — 없다면 사용자가 "가이드 인용률"을 품질 목표로 삼기로 결정하는 시점. 지금은 결과 정확도에 영향이 없어 즉시 고칠 필요는 없으나, **가이드 인용 축(현재 18/28)을 기준선으로 삼는 이상 이 값이 왜 64%인지 모르는 채로 두면 안 된다**.
status: open
