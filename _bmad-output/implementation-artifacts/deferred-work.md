...
- **해소 방향(그때)**: 각 테이블 마이그에 명시 GRANT 추가 → 프렐류드의 `alter default privileges` 한 줄 제거 → 게이트가 *"맨 Postgres에서도 선다"* 를 증명. **판정규칙 (a) 해당**(원격은 이미 부여돼 있어 델타 0 → 안전). **비용 ≈ 테이블당 1~2줄.** `docs/tech-debt.md`에도 항목으로 박제(8.6 Task 6b).
