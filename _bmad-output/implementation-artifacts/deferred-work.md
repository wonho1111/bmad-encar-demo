...
resolution: H 회차 4차(5e78bc1)에서 멀티턴 15/15 GOOD(같은 판정기 gemini-3.8-flash로 1차 11/15 → 4차 15/15, .logs/chatbot_eval/20260914_h/run4). 코드: 순번(복수 포함)·극값·비교 지칭을 answer_guards.resolve_list_reference가 직전 목록 카드에서 결정적으로 풀어 market_price_stats 인자 강제·compare_listings는 2건 이상일 때 강제, 속성 좁힘 후속('그 중 무사고만/여기서 흰색만')은 LLM 없이 코드가 답·카드 생성(narrow_by_attribute 템플릿), 제외어 후속('그거 말고 하이브리드만')은 첫턴 조건 합쳐 재검색, HNSW iterative_scan(DW-882). 3차의 '매물 강제 제시' 규칙은 없는 매물을 지어내(C37·C39) 4차에서 제거. 회귀 30/31(S16 데이터 의존). 채점 잡음 ±5건(DW-877) 감안해도 +4건은 실질 개선. 남은 유형(검색됐는데 미제시·되묻기 C42·C68·C71)은 DW-883.
