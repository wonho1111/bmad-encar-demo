...
- ❌ **찜 목록의 하단 여백 변화는 눈으로 못 봤다** — 로그인이 필요한데 운영 계정 자격증명이 없다. 지금 근거는 **형제 3개 탭과의 코드 일치**뿐이고, 시스템 인셋을 실제로 넣고 재는 검사는 이 화면에 없다(`FakeViewPadding`을 쓰는 테스트는 `listing_detail_screen_test.dart` 하나뿐).
