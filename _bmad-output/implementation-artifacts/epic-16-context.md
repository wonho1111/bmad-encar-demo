...
- FR58: 비로그인도 매물 열람 가능. 로그인 게이트는 화면이 아니라 "행동"(비용 발생 여부) 기준 — AI 검색은 호출당 실제 과금이 나가는 행동이라 로그인 필수, 열람은 서버 자원 읽기뿐이라 무관.
  - ✎ **결정 2026-08-09(사용자) — 이 FR은 웹·앱 공통이다. 앱도 비로그인 열람을 연다**(DW-738의 ②안). 지금 앱은 `app_router.dart`의 `redirect`가 미인증 사용자를 **모든 경로에서** `/login`으로 보내 이 FR이 앱에서 성립하지 않는다(실측). 열람 경로(`/home`·`/search`·상세)를 화이트리스트로 통과시키고 행동 지점은 기존 `requireUser` 게이트로 막는 작업이 **16.6의 범위**다.
...
- 사진 업로드 계약(**정본은 `docs/conventions.md` §10.1** — 웹 9.3이 원격 실측으로 확정, 앱도 동일): 경로 `{user_id}/{listing_id}/{filename}`(첫 세그먼트=소유자, DB 트리거가 강제) · 업로드 전 클라가 긴 변 ≤1600px·WebP·q0.82로 재인코딩 · 대표=`sort_order` 0번(`is_cover`는 파생 결과) · **파일명은 uuid이고 `x-upsert`는 쓰지 않는다** · `listing_images` 행 INSERT는 순차. 삭제 순서는 **무엇을 지우느냐로 갈린다** — 사진 1장 제거는 ①Storage 오브젝트 → ②`listing_images` 행(①이 실패하면 ②를 하지 않는다), **매물 전체 삭제는 ①`listings` 행 → ②오브젝트 정리로 반대**다. `sold` 매물 사진 추가/삭제는 DB 쓰기 정책으로 막는다(화면 방어 아님).
  - ✎ 2026-08-09 정정(DW-749) — 이 줄은 16.7 인수 커밋에서 *"파일명은 (매물+순번)로 결정론적이라 재시도가 덮어씀"*·*"삭제는 ①Storage→②DB 행 순서 고정"*으로 되살아나 있었다. 둘 다 spec-16-7이 **낡은 서술로 폐기한** 내용이라 §10.1에 맞춰 되돌린다.
