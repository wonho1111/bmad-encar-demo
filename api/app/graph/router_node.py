...
- SQL (구조형): 가격·차종·연식·색상·지역·주행거리·연료·옵션명(스마트키 등) 같은 명시적 조건만으로
    ...
    용도·느낌 조건이 없으면 SQL이다.
    **"제일"·"가장" 같은 최상급 부사가 "싸다"·"비싸다"·"저렴하다" 등과 함께 나타나면(예: "제일
    싼", "가장 저렴한") 그 결합 자체가 명시적 구조조건이다(정렬 기준 자체가 조건) — 다른 조건이
    없어도 이 결합만으로 SQL이다. 단, 최상급 부사("제일"·"가장") 없이 "저렴한"·"싸다"·"비싼"
    같은 형용사만 있으면 막연한 소프트 예산 표현이지 구조조건이 아니다 — 아래 CLARIFY 규칙대로
    분류한다("저렴한 차 추천해줘"는 CLARIFY, "제일 저렴한 차 추천해줘"만 SQL).**
    예) "3천만원 이하 흰색 SUV", "2020년 이후 제네시스", "10만km 미만 디젤", "서울 경차",
        "2천만원 이하 중형세단 스마트키 있는 거 추천해줘",
        "제일 싼 차 뭐야?"(최상급 부사+정렬 형용사 결합만 있어도 SQL), "가장 비싼 매물 하나 보여줘"
...
- CLARIFY (질적·의미형, 되묻기): 명시적 조건이 전혀 없고 용도·느낌만으로 묻는 질의.
    예) "패밀리카로 무난한 거", "초보운전자에게 좋은 차", "출퇴근하기 편한 차", "가성비 좋은 차",
        "사회 초년생이 탈만한 저렴한 차 추천해줘"(최상급 부사 없는 "저렴한"은 소프트 예산일 뿐
        구조조건이 아니다)
...
교체요청("~같은 ~로 바꿔줘"·"아니 ~로 바꿔줘"류, 직전 대화의 차종을 다른 차종으로 바꾸는 요청)은
  별도 다섯 번째 갈래가 아니다 — 위 SQL/HYBRID/CLARIFY 판정을 그대로 적용한다. 명시 차종·조건만
  있으면 SQL, 거기에 용도·느낌 표현이 섞이면 HYBRID다("교체요청"이라는 이유만으로 CLARIFY로
  보내지 않는다).
  예) "아니 쏘렌토 같은 SUV로 바꿔줘" → SQL (명시 차종 "SUV"만 있음)
  예) "가족이 타기 좋은 SUV로 바꿔줘" → HYBRID (차종 "SUV" + 느낌 "가족이 타기 좋은")
...
- 최상급 부사("제일"·"가장")가 정렬 형용사(싸다·비싸다 등)와 함께 있으면 다른 조건이 없어도
  그 자체로 명시적 조건이다 — CLARIFY로 보내지 않는다. 최상급 부사 없이 정렬 형용사만
  있으면(예: "저렴한 차 추천해줘") 소프트 예산 표현이므로 이 규칙이 적용되지 않는다(CLARIFY 대상).
- 교체요청("~같은 ~로 바꿔줘"류)은 명시 조건만 있으면 SQL, 느낌이 섞이면 HYBRID다 —
  교체요청이라는 이유만으로 CLARIFY로 보내지 않는다.
