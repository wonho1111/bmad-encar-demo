...
[옵션 동의어 그룹 — 아래 표현은 DB 실측 옵션 문자열이 여러 개로 나뉘어 있어, 계열 전체를
 `options && ARRAY['…','…']`(배열 겹침 연산자)로 묶어 찾는다. 단일 옵션이면 위 `= ANY(options)`를
 그대로 쓴다(2026-08-29 사용자 검수·승인. DB 실측 옵션 문자열 기준 — E2E에서 '크루즈컨트롤'
 단일 매칭이 1건 vs 계열 전체 11건이던 문제).
- 크루즈컨트롤/크루즈 → options && ARRAY['크루즈컨트롤','어댑티브크루즈','스마트크루즈']
  (오토파일럿은 포함하지 않는다)
- 주차보조/주차센서 → options && ARRAY['주차센서','후방센서','후방감지센서']
- HUD/헤드업(디스플레이) → options && ARRAY['헤드업디스플레이','HUD','증강현실HUD']
- 선루프 → options && ARRAY['선루프','파노라마선루프'] (단 "파노라마"를 명시하면
  '파노라마선루프' = ANY(options) 단일 매칭만 쓴다)
- 어라운드뷰/서라운드뷰 → options && ARRAY['어라운드뷰','서라운드뷰']
- 가죽시트 → options && ARRAY['가죽시트','나파가죽','나파가죽시트']
- 급속충전 → options && ARRAY['급속충전지원','초고속충전']
- 프리미엄 사운드/사운드 좋은 → options && ARRAY['하만카돈','렉시콘사운드','뱅앤올룹슨']
- 후방카메라 → '후방카메라' = ANY(options) 단일 매칭만 쓴다(주차센서류와 섞지 않는다)]"""
