...
v4(judge.py=v3에서 복사, v3는 그대로 둔다, DW-873): queries.json의 `expected`(질의셋이 미리
정의한 기대 행동)가 있으면 그것을 1순위 채점 기준으로 삼는 규칙 하나만 시스템 프롬프트에
추가했다 — origin: 3차 재검증(20260909c) C66에서 답변이 expected("범위 밖 안내 + 대안 유도")와
정확히 일치하는데도 v3가 자기 세부 규칙(6번, over_refused)을 우선해 BAD로 오판했다. v3와의
차이는 이 규칙 문단 하나뿐, 나머지 로직(모델 선택·재시도·payload 조립)은 동일하다.

실행:
  cd api && .venv/bin/python scripts/chatbot_eval/judge_v4.py
"""
...
[1순위 규칙 — expected 우선] payload에 `expected`(질의셋이 이 질의에 미리 정의해 둔 기대
행동)가 있으면, 그것을 이번 판정의 1순위 기준으로 삼는다. 답변의 실제 내용·행동이 `expected`가
요구하는 방향과 일치하면(예: "범위 밖 안내 + 대안 유도"가 expected인데 답변이 실제로 범위 밖임을
안내하며 대안으로 유도했다면), 표현 방식이나 구체적 어휘가 아래 세부 규칙(특히 6번 over_refused·
7번 가이드 지식 판정)과 다르게 읽히더라도 그 세부 규칙보다 `expected` 일치를 우선해 GOOD으로
판정한다. `expected`가 없거나, 있어도 답변이 그 기대와 명백히 어긋날 때만 아래 1~9번 세부 규칙
으로 판정한다 — expected 일치가 다른 위반(예: hallucinated_listing·constraint_violated처럼
사실관계 자체가 틀린 경우)까지 면제해 주지는 않는다.

아래 기준을 모두 만족해야만 GOOD이다. 하나라도 위반하면 BAD다(단, 위 1순위 규칙이 우선한다):
...
def main() -> None:
    ...
    (OUT_DIR / "judge_meta.json").write_text(
        json.dumps({
            ...
            # (judgments_v1.jsonl=rubric v1, judgments_v2.jsonl=rubric v2, judge.py=v3,
            # 이 파일(judge_v4.py)=v4 — v3에 "expected 1순위" 규칙 한 문단만 추가, DW-873)
            "judge_version": 4,
