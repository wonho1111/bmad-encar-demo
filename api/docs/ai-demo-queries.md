...
> **이 표들의 "기대 분류"가 어디서 왔나** (13.8 3차 리뷰 추가, 13.9가 나머지 행 실측으로 보강):
> `3천만원 이하 흰색 SUV`(SQL) · `패밀리카로 무난한 거`(CLARIFY) · `오늘 날씨 어때?`(REJECT)는
> `tests/test_live_smoke.py`가 라이브로 route를 단언하고, `출퇴근용으로 편한 차 추천해줘`는
> 회귀 큐리셋 CL4(CLARIFY)와 같은 문자열이다. **나머지 8행도 13.9(2026-08-03)가 라우터
> 프롬프트 갱신 후 `router_node()`를 직접 호출해 실측 확인했다**(라이브 1회, 각 문항의
> "기대 분류"와 일치) — 결정론 테스트는 여전히 `_patch_route`로 경로를 강제 주입하므로
> 이 실측은 상시 회귀 게이트가 아니라 **이 문서를 고친 시점의 스냅샷**이다(라우터 프롬프트가
> 다시 흔들리면 재확인이 필요하다, 열린 항목 DW-576). 표 ③(회색지대)은 세 행 모두 큐리셋
> 실측 기반이다.
...
|---|---|---|---|
| user "3천만원 이하 서울 SUV 보여줘" / assistant "조건에 맞는 매물 3건을 찾았어요." | "제일 싼 거 하나만 알려줘" | "3천만원 이하 서울 지역 SUV 중 가장 저렴한 매물 하나만 알려줘" | SQL (최상급 표현은 구조조건이다, M1.t2) |
...
> ⚠️ 셋째 행의 **원 질의**(재작성 전)는 표 ②가 CLARIFY로 규정한 것이다 — 모순이 아니다.
> 맥락 재작성이 조건을 붙여 주기 때문에 후속 질의는 다른 갈래로 간다. 그게 이 절의 요지다.
> 첫째 행은 M1.t2(2026-08-02 재캡처 오답)를 그대로 옮긴 것으로, 13.9(2026-08-03)가
> `contextualize_node._REFINE_MARKERS`에 최상급 어휘를 추가한 뒤 `contextualize_query`→
> `router_node`를 실제로 이어 호출해 위 재작성·경로를 그대로 실측 확인했다(DW-611 종결).
> 교체요청("~같은 SUV로 바꿔줘")의 사분면 귀속(명시 조건만=SQL, 느낌이 섞이면 HYBRID)도
> 같은 세션이 `router_node()` 직접 호출로 실측 확인했다(이 표에 별도 행은 없음, DW-612 참조).
